...
Traversing "catering":
  'c' → exists
  'a' → exists
  't' → exists, is_end=True ("cat") → FOUND! Return "cat"
...
Traversing "catering":
  'c' → exists
  'a' → exists
  't' → is_end ("cat"), continue
  'e' → exists
  'r' → is_end ("cater"), continue
  'i' → NOT exists, stop
...
| **Replace all words** | O(n * l + s * w) | O(n * l) | s sentences words |
...
> "Trie gives us O(m) per lookup compared to O(n * m) checking against all dictionary words. The trade-off is O(n * l) space for the trie."
...
> **💡 Key Insight:** Shortest prefix returns on first word match. Longest prefix tracks the best match and keeps going until the path ends. Same traversal, different return logic.

> **🔗 Related:** [StartsWith](../02-Trie-Operations/3.3-StartsWith-Prefix.md) | [Search](../02-Trie-Operations/3.2-Search-Operation.md) | [Word Collection](./4.3-Collect-All-Words.md)
