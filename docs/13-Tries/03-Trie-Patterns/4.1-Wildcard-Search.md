...
- Pattern: `addWord("bad")`, `search("b.d")` → True
...
- [DFS/Backtracking](../.../02-Recursion-Backtracking/02-Recursion-Backtracking.md) - Required for wildcards
...
Wildcard search: `'.' can match ANY character` → must try ALL children!
...
When we hit '.':
  ...
  - If ANY path leads to valid word → return True
```
...
Search "b.d":
  'b' → Found, move to 'b' node
  '.' → Try ALL children: 'a', 'e', 'i'
        'a' + 'd' → Found, is_end=True ✓
  Result: True (at least one path works)

Search "b.g":
  'b' → Found
  '.' → Try all children: 'a', 'e', 'i'
        'a' + 'g' → Found, is_end=True ✓
  Result: True

Search "b..":
  'b' → Found
  '.' → Try all ('a', 'e', 'i')
        '.' on each → Try all children
...
class WordDictionary:
    ...
    def search(self, word: str) -> bool:
        ...
        Time: O(m) for no wildcards, O(26^k * m) worst case
...
| **One '.'** | O(26 * m) | O(m) | Try 26 branches |
| **k wildcards** | O(26^k * m) | O(m) | Exponential! |
| **All '.'** | O(n * m) | O(m) | n = words in trie |
...
> "Without wildcards, search is O(m). Each '.' adds a branching factor of up to 26. So with k wildcards, worst case is O(26^k * m). However, in practice it's much better because real tries are sparse."
...
|---------|-----------|-------|
| **Meta** | ⭐⭐⭐ | LC 211 is common |
| **Amazon** | ⭐⭐ | Occasionally |
| **Google** | ⭐⭐ | As part of larger problems |
...
> **💡 Key Insight:** Wildcard '.' is DFS on the trie - try all children and return true if ANY path succeeds. Regular characters follow one path, wildcards fork to all paths.

> **🔗 Related:** [Search Operation](../02-Trie-Operations/3.2-Search-Operation.md) | [Trie + Backtracking](./4.2-Trie-Plus-Backtracking.md) | [DFS Patterns](../.../02-Recursion-Backtracking/02-Recursion-Backtracking.md)
