...
- Given m*n board and list of words, find all words that exist

**Why Trie + Backtracking?**
- Backtracking alone: O(n * m * 4^L) per word * W words
- With Trie: O(n * m * 4^L) total - search all words simultaneously!
...
- [Backtracking template](../.../02-Recursion-Backtracking/02-Recursion-Backtracking.md) - Explore + undo
- [Grid DFS](../.../10-Graphs/10-Graphs.md) - 4-directional exploration
...
Time: O(W * N * M * 4^L) where W = words, L = word length
```
...
Time: O(N * M * 4^L) - words don't multiply!
```

### Key Insight: Pruning

The Trie tells us immediately if the current path can lead to any word:
- If `startsWith(current_path)` is False → prune this branch
- If `is_end` is True → found a word, but keep going (might find longer words!)
...
Start at 'o':
  Path: "o" → In trie? Yes, continue
  Path: "oa" → In trie? Yes ("oath" branch)
  Path: "oat" → In trie? Yes
  Path: "oath" → In trie? Yes, is_end=True! Found "oath"!

Start at 'e':
  Path: "e" → In trie? Yes ("eat" branch)
  Path: "ea" → In trie? Yes
  Path: "eat" → In trie? Yes, is_end=True! Found "eat"!
  
Start at 'k':
  Path: "k" → In trie? No! Prune immediately.
```
...
| Aspect | Value | Notes |
|--------|-------|-------|
| **Time** | O(N * M * 4^L) | L = max word length |
| **Space** | O(W * L + L) | Trie + recursion stack |

**Why this complexity:**
- Start from each cell: N * M
...
| Approach | Time | Notes |
|----------|------|-------|
| Naive (word by word) | O(W * N * M * 4^L) | W words |
| With Trie | O(N * M * 4^L) | All words at once |
...
> "We have a grid and multiple words to find. Searching each word separately would be O(W * N * M * 4^L). Using a Trie lets us search all words simultaneously - we follow the trie as we explore the grid, pruning paths that can't lead to any word."
...
|---------|-----------|-------|
| **Amazon** | ⭐⭐⭐⭐ | Classic interview problem |
| **Google** | ⭐⭐⭐ | Tests trie + backtracking |
| **Meta** | ⭐⭐⭐ | Common |
...
> **💡 Key Insight:** The Trie acts as a "smart dictionary" that guides your DFS - instead of searching for each word separately, you search for all words at once, pruning branches that can't lead to any word. This changes O(W * grid) to O(grid).

> **🔗 Related:** [Wildcard Search](./4.1-Wildcard-Search.md) | [Backtracking Template](../.../02-Recursion-Backtracking/02-Recursion-Backtracking.md) | [Grid DFS](../.../10-Graphs/10-Graphs.md)
