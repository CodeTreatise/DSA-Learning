...
def build_trie(words: list[str]) -> Trie:
    ...
    Time: O(n * m) where n = num words, m = avg length
    Space: O(n * m) worst case (no shared prefixes)
...
| **Build from n words** | O(n * m) | O(n * m) | Total characters |
...
Typical case:
  Nodes ≈ total_unique_characters * sharing_factor
```
...
- [ ] [Implement Trie](https://leetcode.com/problems/implement-trie-prefix-tree/) - LC 208 ⭐ Core problem
...
> "Insert is O(m) where m is the word length. In the worst case, we create m new nodes, so space is also O(m) per word. For building from n words, it's O(n * m) total."
...
| Situation | Advice |
|-----------|--------|
| Building from list | Mention O(n * m) total time |
...
> **💡 Key Insight:** Insert is just a path-building operation. For each character, ensure the path exists (create if needed), then walk down. At the end, plant a flag (is_end = True). That's all there is to it.

> **🔗 Related:** [TrieNode Structure](../01-Trie-Fundamentals/2.1-TrieNode-Structure.md) | [Search Operation](./3.2-Search-Operation.md) | [Delete Operation](./3.4-Delete-Operation.md)
