...
## ❌ Search vs StartsWith - Critical Difference
...
search("app")      → False (no word "app" inserted)
...
**Search "app":**
```
Step 1: At root, look for 'a' → Found, move down
Step 2: At 'a', look for 'p' → Found, move down
Step 3: At 'p', look for 'p' → Found, move down
Step 4: At 'p', no more chars, check is_end → True ✓
Result: True
```
...
Step 4: At 'p', look for 'l' → Found, move down
Step 5: At 'l', look for 'e' → Found, move down
Step 6: At 'e', no more chars, check is_end → True ✓
Result: True
```
...
Step 5: At 'l', no more chars, check is_end → False ✗
...
Step 6: At 'e', look for 'i' → NOT FOUND ✗
...
| Linked List | O(n * m) | Must check all |
...
> **💡 Key Insight:** Search = Traverse + Check is_end. The traversal ensures the path exists; the is_end check ensures it's a complete word, not just a prefix of something longer.

> **🔗 Related:** [Insert Operation](./3.1-Insert-Operation.md) | [StartsWith Operation](./3.3-StartsWith-Prefix.md) | [Wildcard Search](../03-Trie-Patterns/4.1-Wildcard-Search.md)
