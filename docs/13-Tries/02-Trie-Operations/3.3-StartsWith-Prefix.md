...
## ❌ StartsWith vs Search (Critical Distinction)

| Trie contains: ["apple", "app"] | search() | startsWith() |
|---------------------------------|----------|--------------|
| Query: "app" | **True** ✓ | **True** ✓ |
| Query: "apple" | **True** ✓ | **True** ✓ |
| Query: "ap" | **False** ✗ | **True** ✓ |
| Query: "application" | **False** ✗ | **False** ✗ |

**Key difference:**
- `search("ap")` → False (no word "ap" exists)
- `startsWith("ap")` → True (words "app", "apple" start with "ap")
...
**Search:** Must reach END of a complete word → checks `is_end`
**StartsWith:** Just needs PATH to exist → no `is_end` check
...
Trie structure:
    (root)
       ...
       e ← is_end ("apple")   c
                              ...
                              o
                              │
                              n ← is_end ("application")
```

**startsWith("app"):**
```
Step 1: At root, look for 'a' → Found ✓
Step 2: At 'a', look for 'p' → Found ✓
Step 3: At 'p', look for 'p' → Found ✓
Step 4: Reached end of prefix, path exists → True ✓
```
...
Step 1: At root, look for 'a' → Found ✓
Step 2: At 'a', look for 'p' → Found ✓
Step 3: At 'p', look for 'l' → NOT FOUND ✗
...
class Trie:
    ...
    def search(self, word: str) -> bool:
        ...
        return node is not None and node.is_end  # ← Checks is_end
    
    def startsWith(self, prefix: str) -> bool:
        """Check if any word starts with prefix."""
        return self._traverse(prefix) is not None  # ← No is_end check!
```
...
| **Hash Set** | O(n * m) - check every word! |
...
> **💡 Key Insight:** StartsWith is Trie's killer feature. While Hash Maps need O(n) to find words with a prefix, Tries do it in O(m + k) - that's the difference between unusable and instant for large dictionaries.

> **🔗 Related:** [Search Operation](./3.2-Search-Operation.md) | [Collect All Words](../03-Trie-Patterns/4.3-Collect-All-Words.md) | [Autocomplete System](../06-Applications/7.1-Autocomplete-System.md)
