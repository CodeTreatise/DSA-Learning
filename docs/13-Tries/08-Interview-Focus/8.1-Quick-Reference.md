...
| Build from n words | O(n * avg_len) | O(total_chars) |
| Wildcard search | O(26^dots * m) | O(m) recursion |
...
├── Prefix-based queries → TRIE
│   ├── Exact word → search()
│   ├── Has prefix → startsWith()
│   ├── Wildcard (.) → DFS
│   └── Collect words → DFS from prefix node
├── XOR optimization → BIT TRIE
│   └── Maximum XOR → Greedy opposite bit
└── Not prefix-based → Consider Hash Map
```
...
> **💡 Remember:** Trie = "Prefix Tree". If the problem involves prefixes, Trie is probably the answer.
