...
- Regular characters → follow normal trie path
- `.` wildcard → try ALL children (DFS/backtracking)
...
class WordDictionary:
    ...
    Time Complexity:
        ...
        - search without '.': O(m)
        - search with '.' : O(26^k * m) worst case, k = number of dots
...
| **search (k dots)** | O(26^k * m) | O(m) stack |
| **search (all dots)** | O(n * m) | O(m) stack |
...
3. **Complexity:** "Without dots, O(m). With k dots, worst case O(26^k * m), but real tries are sparse so it's usually much better."

4. **Optimization:** "I could bucket by word length so '...' only searches 3-letter words."

**Common follow-ups:**
- "What if '*' means zero or more?" → Much harder, needs DP
- "How to handle case insensitivity?" → Normalize on insert/search
- "Support multiple wildcards?" → Already handled!
...
> **💡 Key Insight:** The '.' wildcard turns search into DFS. For regular chars, follow one path. For '.', fork to all children and return true if ANY path succeeds.

> **🔗 Related:** [Wildcard Search Pattern](../03-Trie-Patterns/4.1-Wildcard-Search.md) | [Basic Trie](./6.1-Implement-Trie.md)
