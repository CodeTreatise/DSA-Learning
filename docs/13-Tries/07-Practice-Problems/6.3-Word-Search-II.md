...
- Naive: Search each word separately → O(W * M * N * 4^L)
- Optimized: Build Trie, search all words at once → O(M * N * 4^L)
...
Total: O(W * M * N * 4^L) - Too slow for W = 30,000!
```
...
Total: O(M * N * 4^L) - W doesn't multiply!
```
...
3. **DFS from cell:**
   - If char not in Trie children → prune (return)
   - If found word → add to result, mark as found
   - Mark cell visited
...
class Solution:
    ...
    Space: O(W * L) for Trie + O(L) recursion stack
    """
...
| **Space** | O(W * L + L) | Trie + recursion |

**Why this is efficient:**
- Without Trie: O(W * M * N * 4^L) - W multiplies everything
- With Trie: O(M * N * 4^L) - W only affects Trie building
...
Starting at (0,0) 'o':
  ...
  'oa' path (0,1)? Yes
  'oat' path (1,1)? Yes
  'oath' path (2,1)? Yes, word found! → result = ["oath"]

Starting at (1,0) 'e':
  'e' in trie? Yes
  'ea' path (0,0)? No (not 'a')
  'ea' path (1,1)? No (it's 't')
  'ea' from (2,2)... eventually find 'eat' → result = ["oath", "eat"]
```
...
**Common follow-ups:**
- "Why store word instead of is_end?" → Easy retrieval without tracking path
- "How to handle duplicates?" → Set word to None after finding
- "Can we optimize more?" → Node removal, early termination
...
> **💡 Key Insight:** The Trie acts as a "smart dictionary" guiding your DFS - instead of searching for each word, you search once and the Trie tells you which words you've found. This is why complexity loses the W factor.

> **🔗 Related:** [Trie + Backtracking Pattern](../03-Trie-Patterns/4.2-Trie-Plus-Backtracking.md) | [Word Search I (LC 79)](../.../02-Recursion-Backtracking/02-Recursion-Backtracking.md)
