...
- At most 3 * 10⁴ calls to insert, search, and startsWith
...
1. **TrieNode stores:**
   - Children (mapping char → child node)
...
2. If any character missing → False
3. If traversal complete → return node.is_end
```

**StartsWith:**
```
1. Same as search, but...
2. If traversal complete → return True (no is_end check)
```
...
class Trie:
    ...
    Space Complexity:
        - O(n * m) for n words of avg length m
    """
...
trie.search("apple")  →  True  (path exists, is_end = True)
trie.search("app")    →  False (path exists, is_end = False)
trie.startsWith("app") → True  (path exists, don't check is_end)
...
trie.search("app") → True (now is_end = True at 'p')
```
...
**Common follow-ups:**
- "Add delete operation" → See [Delete Operation](../02-Trie-Operations/3.4-Delete-Operation.md)
- "Support wildcards" → See [Wildcard Search](../03-Trie-Patterns/4.1-Wildcard-Search.md)
- "Get all words with prefix" → See [Collect Pattern](../03-Trie-Patterns/4.3-Collect-All-Words.md)
...
> **💡 Key Insight:** The _traverse helper is crucial - it eliminates code duplication between search and startsWith. The only difference between them is the is_end check at the end.

> **🔗 Related:** [TrieNode Structure](../01-Trie-Fundamentals/2.1-TrieNode-Structure.md) | [Trie Class Design](../01-Trie-Fundamentals/2.2-Trie-Class-Design.md)
