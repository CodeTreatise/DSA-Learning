...
class Solution:
    """
    Time: O(n * l * log(n) + m * 3) 
        = O(n * l * log(n)) for sorting and building
...
| Phase | Time | Space |
|-------|------|-------|
| Sort | O(n * l * log(n)) | O(log n) |
| Build Trie | O(n * l) | O(n * l) |
| Search | O(m) | O(1) |
| **Total** | **O(n * l * log(n))** | **O(n * l)** |
...
|-------|------|-------|
| Sort | O(n * l * log(n)) | O(log n) |
| Search | O(m * (log(n) + l)) | O(1) |
| **Total** | **O(n * l * log(n))** | **O(1) extra** |
...
Prefix "m":   → node.suggestions = ["mobile","moneypot","monitor"]
Prefix "mo":  → node.suggestions = ["mobile","moneypot","monitor"]
Prefix "mou": → node.suggestions = ["mouse","mousepad"]
Prefix "mous":→ node.suggestions = ["mouse","mousepad"]
Prefix "mouse":→ node.suggestions = ["mouse","mousepad"]
```
...
**Common follow-ups:**
- "What if products update frequently?" → Trie is better for dynamic data
- "What if we need top K by popularity?" → Add frequency, use heap
- "Memory constraints?" → Binary search approach
...
> **💡 Key Insight:** Sorting products first is crucial - it ensures our DFS/binary search naturally returns results in lexicographic order. Store up to 3 suggestions at each Trie node for O(1) query time.

> **🔗 Related:** [Collect All Words](../03-Trie-Patterns/4.3-Collect-All-Words.md) | [Autocomplete System](../06-Applications/7.1-Autocomplete-System.md)
