...
Query for 5 (00101):
- Bit 4: 5 has 0, want 1 → GO 1 ✓ (found path to 25!)
  Result so far: 1xxxx
- Bit 3: 5 has 0, want 1 → GO 1 ✓
  Result: 11xxx
- Bit 2: 5 has 1, want 0 → GO 0 ✓
  Result: 110xx
- Bit 1: 5 has 0, want 1 → no 1, GO 0
  Result: 1100x
- Bit 0: 5 has 1, want 0 → GO 0 (only option)
  Result: 11001
...
|----------|------|-------|
| Brute Force | O(n²) | O(1) |
| Bit Trie | O(n * max_bits) | O(n * max_bits) |
| HashSet | O(n * max_bits) | O(n) |

**For typical constraints:**
- n = 2*10^5, max_bits = 31
- Bit Trie: ~6.2 million operations ✓
- Brute Force: 4*10^10 operations ✗
...
"Building and querying the Trie is O(n * 31), which is effectively O(n). Space is also O(n * 31) for storing all paths."

**Follow-ups:**
- "What if numbers can be added/removed?" → Add count field
- "Maximum XOR subarray?" → Use prefix XOR + Bit Trie
...
> **💡 Key Insight:** XOR gives 1 when bits differ. To maximize XOR, greedily pick the opposite bit at each position, starting from the most significant bit (because 1 in MSB beats all lower bits combined).

> **🔗 Related:** [Bit Trie Fundamentals](5.1-Bit-Trie-Fundamentals.md) | [Maximum XOR With Element](5.3-Max-XOR-With-Element.md)
