...
Step 1: Bit 2 (MSB) = 1 → go right
Step 2: Bit 1 = 0 → go left  
Step 3: Bit 0 (LSB) = 1 → go right
...
print(trie.get_max_xor(5))   # Best XOR with 5 → 5^10=15
print(trie.get_max_xor(10))  # Best XOR with 10 → 10^5=15
```
...
Bit 3: 5 has 0 → want 1 → go to 1 ✓ (will get 1xxx)
Bit 2: 5 has 1 → want 0 → go to 0 ✓ (will get 10xx)
Bit 1: 5 has 0 → want 1 → go to 1 ✓ (will get 101x)
Bit 0: 5 has 1 → want 0 → go to 0 ✓ (will get 1010)
...
| Build from n numbers | O(n * max_bits) | O(n * max_bits) |
...
- Total: O(n * max_bits) in worst case
...
```python
# ❌ Wrong: Assumes Trie always has path
...
```python
# ❌ Wrong: 32 bits for non-negative numbers
max_bits = 32  # Sign bit is always 0

# ✅ Correct: 31 bits for non-negative
...
**Common follow-ups:**
- "What if numbers can be added/removed?" → Track count at nodes
- "What about negative numbers?" → Handle sign bit separately
- "Memory optimization?" → Use bit count based on max value
...
> **💡 Key Insight:** The maximum XOR problem becomes greedy with Bit Trie: at each bit position, we try to go the opposite way to maximize the resulting XOR. MSB matters most, so we process from high to low bits.

> **🔗 Related:** [Maximum XOR](5.2-Maximum-XOR.md) | [Bit Manipulation](../../15-Bit-Manipulation.md)
