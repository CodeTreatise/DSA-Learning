...
> **Extension of Maximum XOR with an added constraint: the number must be ≤ a threshold. Combine Bit Trie with offline processing.**
...
Output: [3,3,7]
Explanation:
- Query [3,1]: nums[j] <= 1 are {0,1}. Max XOR = 3^1=2, 3^0=3 → 3
- Query [1,3]: nums[j] <= 3 are {0,1,2,3}. Max XOR = 1^2=3 → 3  
- Query [5,6]: All nums apply. Max XOR = 5^2=7 → 7
```
...
- For each query, we'd need to find numbers ≤ mi and query Trie
...
- [3, 1] → threshold 1, original index 0
- [1, 3] → threshold 3, original index 1  
- [5, 6] → threshold 6, original index 2
...
| Build Trie | O(n * 31) | O(n * 31) |
| Query | O(q * 31) | O(1) |
| **Total** | **O(n log n + q log q + 31(n+q))** | **O(n * 31)** |

**Simplified:** O((n + q) log(n + q)) for sorting + O((n + q) * 31) for Trie operations.
...
**Follow-up questions:**
- "What if queries come in streaming order?" → Use min-value tracking at nodes
- "What if we need exact equals, not <=?" → Modify threshold comparison
...
> **💡 Key Insight:** Offline processing transforms a constraint problem into an incremental building problem. Sort by constraint, add data incrementally, and the Trie only ever grows.

> **🔗 Related:** [Maximum XOR](5.2-Maximum-XOR.md) | [Count Pairs With XOR](5.4-Count-XOR-Pairs.md)
