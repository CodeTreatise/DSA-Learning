...
class Solution:
    ...
    Time: O(n * 15) for nums up to 2*10^4 (15 bits)
    Space: O(n * 15)
    """
    
    def countPairs(self, nums: list[int], low: int, high: int) -> int:
        root = TrieNode()
        max_bits = 15  # For values up to 2*10^4
...
Bit 2: num=0, limit=1
  ...
  - Same bit path (0) → if exists, add its count
...
Bit 1: num=0, limit=1
  - limit_bit = 1!
  - Same bit (0) path → Trie has 4 which is 0 here... no, 4 = 0100
...
| Total for n numbers | O(n * max_bits) | O(n * max_bits) |

**For this problem:**
- max_bits = 15 (since values ≤ 2*10^4)
- n ≤ 2*10^4
- Total: ~300K operations ✓
...
```python
# ❌ Wrong: Opposite when should be same
if limit_bit == 1:
    opposite_bit = 1 - num_bit  # Wrong for counting!

# ✅ Correct: Same bit for XOR = 0
if limit_bit == 1:
...
**Common follow-up:**
"What if we need XOR in multiple ranges?" → Multiple difference queries
"Optimize for memory?" → Use dictionary-based nodes for sparse tries
...
> **💡 Key Insight:** When limit bit is 1, any path giving XOR bit = 0 is guaranteed to produce a result < limit at that bit position. Count all such paths, then continue looking for XOR bit = 1.
