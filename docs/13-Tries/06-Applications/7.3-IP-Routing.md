...
Lookup:
  192.168.1.130 → Gateway C (matches /25, most specific)
  192.168.1.1   → Gateway B (matches /24)
  192.168.2.1   → Gateway A (matches /16)
  10.0.0.1      → Default (no match)
```
...
Trie structure:
       root
        ...
        0
        ...
       (16 bits: 192.168.x.x → Gateway A)
        |
       ... (24 bits: 192.168.1.x → Gateway B)
```
...
class IPRoutingTable:
    ...
    def _parse_cidr(self, cidr: str) -> tuple:
        """Parse CIDR notation: "192.168.1.0/24" → ("192.168.1.0", 24)"""
        parts = cidr.split('/')
        ip = parts[0]
...
Traverse:
  Bit 0-15: Match /16 → best=A
  Bit 16-23: Match /24 → best=B
  Bit 24: Match /25 (bit=1) → best=C
...
|-----------|------|-------|
| Add route | O(prefix_len) ≤ O(32) | O(prefix_len) |
| Lookup | O(32) = O(1) | O(1) |
| Delete | O(prefix_len) | O(1) |
| Build n routes | O(n * avg_prefix) | O(n * avg_prefix) |
...
**Follow-ups:**
- "How to handle updates?" → Add/remove routes dynamically
- "Performance for large tables?" → Multi-bit Trie, LC-Trie
- "IPv6?" → Same concept, 128 bits
...
> **💡 Key Insight:** Longest prefix matching means we don't stop at the first match - we continue traversing and update best_match each time we hit a route endpoint. The deepest match wins.

> **🔗 Related:** [Bit Trie](../04-Bit-Trie/5.1-Bit-Trie-Fundamentals.md) | [Prefix Matching](../03-Trie-Patterns/4.4-Prefix-Matching.md)
