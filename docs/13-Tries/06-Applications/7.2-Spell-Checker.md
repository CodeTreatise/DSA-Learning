...
check("hello") → True (exists)
check("helo")  → False, suggestions: ["hello"]
check("wrld")  → False, suggestions: ["world"]
```
...
class SpellChecker:
    ...
    Exact check: O(m)
    Suggestions: O(26^k * m) where k is edit distance
    """
...
|-----------|------|-------|
| Build dictionary | O(n * m) | O(n * m) |
| Exact check | O(m) | O(1) |
| Suggest (k=1) | O(26 * m) | O(m) recursion |
| Suggest (k=2) | O(26² * m) | O(m) |

### Generate-and-Check Approach

| Operation | Time | Space |
|-----------|------|-------|
| Edits distance 1 | O(26 * m) | O(26 * m) |
| Edits distance 2 | O(26² * m²) | O(26² * m²) |
...
**Follow-ups:**
- "How to handle proper nouns?" → Separate dictionary or skip check
- "Scale to millions of words?" → Compress Trie, limit distance
- "Real-time as-you-type?" → Cache prefix suggestions
...
> **💡 Key Insight:** The Trie structure lets us prune the search space early - if a prefix doesn't exist, we don't explore further, unlike generate-and-check which creates all candidates first.

> **🔗 Related:** [Search Operation](../02-Trie-Operations/3.2-Search-Operation.md) | [Autocomplete](7.1-Autocomplete-System.md)
