...
class AutocompleteSystem:
    ...
    input: O(n * log(n)) for sorting n matching sentences
    """
...
|-----------|------|-------|
| Constructor | O(total_chars) | O(total_chars * n) |
| input (navigate) | O(1) | O(1) |
| input (get top 3) | O(n log n) | O(n) |
| Save sentence | O(m) | O(m * n) |
...
| Approach | Pros | Cons |
|----------|------|------|
| Every node | O(1) retrieval | O(n*m) space |
...
**Follow-up questions:**
- "What if we need real-time updates?" → Already handled
- "Scale to millions of sentences?" → Consider distributed Trie
- "Personalization?" → Per-user frequency tables
...
> **💡 Key Insight:** Storing sentences at every prefix node trades space for O(1) retrieval time. For large-scale systems, use DFS from prefix node instead.

> **🔗 Related:** [Collect All Words](../03-Trie-Patterns/4.3-Collect-All-Words.md) | [Search Suggestions](../07-Practice-Problems/6.4-Search-Suggestions.md)
