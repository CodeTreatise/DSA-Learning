...
## 1️⃣ HashMap vs Array Comparison
...
# Memory: 26 * 8 bytes = 208 bytes per node (pointers)
...
trie.insert("hello👋world")  # Works fine
```
...
```
HashMap: ~50 bytes + 26*16 bytes = ~466 bytes per node
...
## 6️⃣ Memory-Optimized Implementations
...
├── Yes → Is data dense (most letters used)?
│         ├── Yes → Use Array[size]
│         └── No  → Use HashMap
└── No (Unicode/variable) → Use HashMap

Is performance more critical than memory?
├── Yes → Use Array (better cache)
└── No  → Use HashMap (less memory)

Is the constraint lowercase English only?
├── Yes → Use Array[26]
└── No  → Evaluate based on actual charset
```
...
```python
# ❌ Wrong: Assumes 'a' starts at 0
idx = ord(char)  # 'a' = 97, way out of bounds!

# ✅ Correct: Normalize to 0-25
idx = ord(char) - ord('a')
```
...
```python
# ❌ Wrong: Complex optimization for small data
...
> **💡 Key Insight:** HashMap is almost always the right default choice. Only use Array[26] when you know the alphabet is fixed and dense, or when benchmark shows cache issues.

> **🔗 Related:** [Trie Class Design](../01-Trie-Fundamentals/2.2-Trie-Class-Design.md) | [Trie Variations](4.5-Trie-Variations.md)
