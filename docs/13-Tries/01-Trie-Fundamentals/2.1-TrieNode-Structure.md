...
- [Hash Maps](../.../09-Hashing/09-Hashing.md) - For HashMap implementation
- [Classes/OOP basics](../.../00-Prerequisites/00-Prerequisites.md) - Object design
...
```
        TrieNode Structure
        ┌──────────────────────────┐
        │ children: {char → Node}  │  ← Links to child nodes
        │ is_end: boolean          │  ← Word completion marker
        │ count: int (optional)    │  ← Number of words ending here
        │ word: str (optional)     │  ← Store complete word
        └──────────────────────────┘
```
...
|----------|-------------------------|-------------------------|
| HashMap | N * (2 * ptr + overhead) | N * (10 * ptr + overhead) |
| Array[26] | N * (26 * ptr) | N * (26 * ptr) |

**Rule of thumb:**
- Less than ~4 children per node → HashMap is more space-efficient
- More than ~4 children per node → Array might be similar
- If you need speed → Array is always faster
...
> **💡 Key Insight:** The TrieNode design should match your problem constraints. HashMap gives flexibility, Array gives speed. Adding metadata like `word` or `count` unlocks solutions for more complex problems.

> **🔗 Related:** [What Is a Trie](./1.1-What-Is-Trie.md) | [Trie Class Design](./2.2-Trie-Class-Design.md) | [Insert Operation](../02-Trie-Operations/3.1-Insert-Operation.md)
