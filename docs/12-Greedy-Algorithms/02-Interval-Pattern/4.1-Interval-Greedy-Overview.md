...
> **Definition:** Interval problems involve collections of ranges `[start, end]` where we need to optimize some property—maximize non-overlapping, merge overlapping, or find minimum to cover.
...
Question types:
• How many can happen without overlap? (Activity Selection)
• Can we merge overlapping ones? (Merge Intervals)
• Minimum resources for all? (Meeting Rooms)
• Minimum arrows to pop? (Min Arrows)
```
...
## ❌ When NOT to Use Interval Greedy
...
- [Weighted Job Scheduling (DP)](../.../11-Dynamic-Programming/11-Dynamic-Programming.md)
...
## 📐 The Three Core Interval Patterns
...
Timeline:
  [1,4]   [====]
  [2,3]    [=]          ← Pick this (ends earliest)
  [3,5]      [===]      ← Pick this (starts after [2,3])
  [4,7]       [====]
...
Timeline:
  [1,3]   [===]
  [2,4]    [===]       ← Overlaps! Extend end to 4
  [5,7]         [===]  ← No overlap, new interval
...
1. **Wrong sorting criterion**
   - ❌ Sort by start for activity selection
   - ✅ Sort by END for activity selection
...
4. **Modifying input while iterating**
   - ❌ `intervals.sort()` changes original
   - ✅ Use `sorted()` if you need original later

5. **Forgetting empty input**
   - ❌ Crash on empty list
   - ✅ Handle `if not intervals: return ...`
...
|---------|-----------|-----------------|
| Amazon | ⭐⭐⭐⭐ | Meeting Rooms, Merge Intervals |
| Google | ⭐⭐⭐⭐ | Interval scheduling with constraints |
| Meta | ⭐⭐⭐ | Calendar merge problems |
| Microsoft | ⭐⭐⭐ | Standard interval problems |

**Interview Communication:**

```
"This is an interval problem. Let me identify the type:
- If we need max non-overlapping → sort by end time
- If we need to merge → sort by start time
- If we need room count → use heap or sweep line
...
> **💡 Key Insight:** Most interval problems are solved by sorting. The question is just: sort by START (for merging/processing left-to-right) or sort by END (for maximizing non-overlapping)?

> **🔗 Related:** [Sort by End Pattern](./4.2-Sort-By-End-Pattern.md) | [Sort by Start Pattern](./4.3-Sort-By-Start-Pattern.md) | [Meeting Rooms II](./4.4-Interval-Practice/Meeting-Rooms-II.md)
