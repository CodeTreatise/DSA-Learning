...
> **Definition:** The Exchange Argument proves greedy optimality by showing that any optimal solution differing from greedy can be transformed into greedy's solution without worsening the objective—therefore greedy is also optimal.
...
Greedy G:     [Y₁] [Y₂] [Y₃] [Y₄] [Y₅]
               ↕    ↕    ↕    ↕    ↕
Optimal O:    [Y₁] [X₂] [Y₃] [X₄] [Y₅]  (X₂ ≠ Y₂, X₄ ≠ Y₄)
               │    │    │    │    │
Step 1:       [Y₁] [Y₂] [Y₃] [X₄] [Y₅]  (swap X₂ → Y₂, still valid)
               │    │    │    │    │
Step 2:       [Y₁] [Y₂] [Y₃] [Y₄] [Y₅]  (swap X₄ → Y₄, still valid)
               │    │    │    │    │
...
def job_scheduling_with_proof(jobs):
    ...
    Suppose G ≠ O. Find the first slot t where they differ:
    - G has job g in slot t
    - O has job o in slot t (or empty)
    
    Case 1: O has job o at slot t, but g ∉ O
      - Since greedy picked g over o, profit(g) ≥ profit(o)
      - We can swap o with g in O
      - New solution O' has profit ≥ profit of O
      - O' agrees with G at slot t
    
    Case 2: O has job o at slot t, and g is in O at slot t' > t
      - Swap g and o in O
      - Both still meet deadlines (g moved earlier, o moved later but o's deadline ≥ t)
...
Suppose G ≠ O. Let i be the first position where G and O differ.
...
**Quality of O':**
[Show quality(O') ≥ quality(O)]
- Either profit increases, or
- Profit stays same (o and g have equal value), or  
- Profit could only decrease, but g was chosen greedily so profit(g) ≥ profit(o)

**Induction:**
After this exchange, O' agrees with G at positions 1, ..., i.
Repeat for remaining positions. After at most n exchanges:
- O^(n) = G
- quality(O^(n)) ≥ quality(O) at each step
- Therefore, quality(G) ≥ quality(O)

Since O was optimal and G is at least as good, G is also optimal. ∎
```

---

## 📊 Classic Example: Huffman Coding
...
- Goal: Minimize weighted path length (Σ freq * code_length)
...
3. Huffman always combines two lowest → matches optimal structure

4. After combining, problem reduces to n-1 characters
   (new node has combined frequency)

5. By induction: Huffman is optimal at each step → optimal overall
```
...
1. **Not showing exchange preserves validity**
   - ❌ "Swap them, it's still valid"
   - ✅ "After swap, deadline constraints still satisfied because..."

2. **Not showing exchange doesn't worsen objective**
   - ❌ "Swapping is fine"
   - ✅ "profit(g) ≥ profit(o) by greedy's selection criterion"

3. **Forgetting to argue repeated exchanges work**
   - ❌ Proving one exchange
   - ✅ Showing each exchange moves O closer to G

4. **Not handling edge cases in swap**
   - ❌ Ignoring what happens if swapped element is elsewhere
   - ✅ Consider all cases: element present, absent, at different position
...
2. At least as good (X has profit ≥ Y)
...
> **💡 Key Insight:** The exchange argument works backwards—instead of showing greedy builds optimally, it shows any optimal solution can be transformed into greedy's solution. If you can always "swap in" greedy's choice without losing quality, greedy must be optimal.

> **🔗 Related:** [Greedy Stays Ahead](./2.1-Greedy-Stays-Ahead.md) | [Classic Greedy](../05-Classic-Greedy/7.1-Classic-Problems-Overview.md) | [When Greedy Works](./1.2-When-Greedy-Works.md)
