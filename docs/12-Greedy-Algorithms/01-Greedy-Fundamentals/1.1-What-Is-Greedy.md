...
- [Basic Complexity Analysis](../.../00-Prerequisites/00-Prerequisites.md) - Understanding O(n log n) from sorting
...
- [Dynamic Programming](../.../11-Dynamic-Programming/11-Dynamic-Programming.md) - When greedy fails
...
Step 1: Pick A [1,3]  ✓ (first to end)
Step 2: Skip B [2,5]  ✗ (overlaps with A)
Step 3: Pick C [4,6]  ✓ (starts after A ends)
Step 4: Pick D [6,8]  ✓ (starts when C ends)
Step 5: Skip E [5,9]  ✗ (overlaps with D)
Step 6: Pick F [8,10] ✓ (starts when D ends)
...
1. **Assuming greedy works without proof**
   - ❌ "It seems like picking the biggest first should work"
   - ✅ Try to find a counterexample or prove correctness

2. **Wrong sorting criteria**
   - ❌ Activity selection: sort by start time
   - ✅ Activity selection: sort by END time

3. **Not handling ties**
   - ❌ Ignoring what happens when values are equal
   - ✅ Define tie-breaking rules explicitly

4. **Forgetting edge cases**
   - ❌ Empty input, single element, all overlapping
...
|---------|-----------------|--------------|
| Amazon | ⭐⭐⭐ | Intervals, scheduling |
| Meta | ⭐⭐⭐ | Intervals, arrays |
| Google | ⭐⭐ | Often mixed with DP |
| Microsoft | ⭐⭐ | Classic problems |
...
> **💡 Key Insight:** Greedy works when making the locally optimal choice at each step doesn't cut off the globally optimal solution. The hard part is proving this is true—or finding a counterexample that proves it's false.

> **🔗 Related:** [When Greedy Works](./1.2-When-Greedy-Works.md) | [Greedy vs DP](./1.3-Greedy-vs-DP.md) | [Proof Techniques](./2.1-Greedy-Stays-Ahead.md)
