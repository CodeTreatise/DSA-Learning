...
Iterator: 3 → 7 → 9 → 15 → 20 (inorder sequence)
```
...
while stack or current:
    ...
    current = stack.pop()
    process(current)          ← We want to PAUSE here
...
```python
# ❌ Wrong: Only popping, not exploring right subtree
def next(self):
    node = self.stack.pop()
    return node.val  # Forgot to push right subtree's left path!

# ✅ Correct: After visiting, explore right subtree
def next(self):
    node = self.stack.pop()
...
# ✅ Better: Assume hasNext() is called first (per problem spec)
...
2. "next() is O(1) amortized — each node pushed and popped once"
3. "This is essentially iterative inorder, paused between elements"

**Follow-up questions:**
- "Can you do it with O(1) space?" → Morris traversal (modifies tree temporarily)
- "What about prev()?" → Maintain two stacks or parent pointers
...
|---------|-----------|-------|
| Meta | ⭐⭐⭐⭐⭐ | Very common |
| Google | ⭐⭐⭐⭐ | Iterator patterns |
| Amazon | ⭐⭐⭐⭐ | Standard BST |
| Microsoft | ⭐⭐⭐ | Design question |
...
- **Concept:** [Inorder Traversal](../../06-Trees/03-Tree-Traversals/3.1-DFS-Traversals/01-Inorder-Traversal.md)
