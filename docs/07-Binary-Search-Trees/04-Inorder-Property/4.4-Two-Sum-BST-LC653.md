...
- Only O(h) space — could be O(log n) for balanced tree
...
```python
# ❌ Wrong: Might use node.val + node.val = k
if node.val * 2 == k:
    return True  # This uses the same node twice!

# ✅ Correct: Ensure two DIFFERENT nodes
...
```python
# ❌ Wrong: Not checking if iterators overlap
while True:  # Infinite loop possible!
    ...

# ✅ Correct: Stop when left and right meet
...
**Discuss trade-offs:**
1. "Inorder + two pointers is O(n) time and space — simple to implement"
2. "Hash set is also O(n) time and space — doesn't use BST property"
3. "Two iterators is O(n) time but O(h) space — best when tree is balanced"

**Follow-up questions:**
- "Which approach is best?" → Depends on tree balance and interview requirements
- "What about three sum?" → Add another iterator/pointer
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Common two-sum variant |
| Meta | ⭐⭐⭐ | Tests BST understanding |
| Google | ⭐⭐⭐ | May ask for optimal |
