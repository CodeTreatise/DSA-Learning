...
- Reverse inorder (right → node → left) visits in descending order
...
- [Inorder Traversal](../../06-Trees/03-Tree-Traversals/3.1-DFS-Traversals/01-Inorder-Traversal.md)
...
For each node, we need sum of all nodes with value ≥ current.
...
```python
# ❌ Wrong: Left → Node → Right gives prefix sum, not suffix
def convert(node):
    if not node:
        return
    convert(node.left)
...
# ✅ Correct: Right → Node → Left for suffix sum
def convert(node):
    if not node:
        return
...
```python
# ❌ Wrong: Excluding current value from sum
def convert(node):
    convert(node.right)
...
```python
# ❌ Wrong: Local variable doesn't persist across calls
def convert(node):
    running_sum = 0  # Resets each call!
    ...

# ✅ Correct: Use instance variable or nonlocal
self.running_sum = 0
...
**Opening statement:**
"For each node, I need the sum of all values greater than or equal to it. If I traverse in reverse inorder (right → node → left), I visit nodes in descending order. I maintain a running sum, and each node's new value is that running sum at the time of visit."
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐ | BST transformation |
| Google | ⭐⭐⭐ | Traversal variation |
| Microsoft | ⭐⭐ | Less common |
| Meta | ⭐⭐ | Occasionally asked |
