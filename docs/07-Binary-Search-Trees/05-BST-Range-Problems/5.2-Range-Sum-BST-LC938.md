...
At 10: 7 ≤ 10 ≤ 15 → ADD 10, explore both
At 5:  5 < 7 → Don't add, but check right (might have 7+)
At 3:  3 < 7 → Don't add, don't check left (all smaller)
At 7:  7 ≤ 7 ≤ 15 → ADD 7
At 15: 7 ≤ 15 ≤ 15 → ADD 15, don't check right (all larger)
...
Range [7, 15]:
        ...
        10          ← In range: ADD (10)
       /  \
      5    15       ← 5 < 7: skip value, check right
     / \     \         15 in range: ADD (15), prune right
    3   7    18     ← 3 < 7: prune, 7 in range: ADD (7)
...
- Range DFS: O(h + k) — much better when k << n
...
```python
# ❌ Wrong: Always explores both subtrees (O(n) always)
...
```python
# ❌ Wrong: Goes left when value is too small (should go right!)
if root.val < low:
    return rangeSumBST(root.left, low, high)  # WRONG!

# ✅ Correct: Go right to find larger values
if root.val < low:
    return rangeSumBST(root.right, low, high)
```
...
```python
# ❌ Wrong: Exclusive range (misses boundary values)
...
- "What if this is called frequently?" → Preprocess with range tree or segment tree
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Easy warm-up |
| Meta | ⭐⭐⭐ | Tests BST basics |
| Google | ⭐⭐ | Often follow-up |
| Microsoft | ⭐⭐⭐ | Common question |
...
> **BST turns O(n) traversal into O(h+k) range query.** By pruning subtrees outside [low, high], we only visit nodes that might be in range. This is the power of ordered data structures — they enable intelligent search space reduction.
