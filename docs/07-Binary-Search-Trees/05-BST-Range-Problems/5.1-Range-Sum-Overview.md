...
- If `low ≤ node.val ≤ high`: Include node, explore both
...
At 10: 6 ≤ 10 ≤ 14 → INCLUDE, check both subtrees
At 5:  5 < 6 → SKIP left subtree, check right only
At 7:  6 ≤ 7 ≤ 14 → INCLUDE
At 15: 15 > 14 → SKIP right subtree, check left only
At 12: 6 ≤ 12 ≤ 14 → INCLUDE
...
```python
# ❌ Wrong: Strict inequality misses boundary nodes
if node.val >= low:  # Should be > for pruning
    dfs(node.left)

# ✅ Correct: Use > for pruning (don't prune when equal)
if node.val > low:
    dfs(node.left)
```
...
# ✅ Correct: Continue to both subtrees
if low <= node.val <= high:
    result += node.val
...
> **Range queries on BST are O(h + k), not O(n).** The BST property lets us prune entire subtrees that can't contain values in range. This is like binary search combined with traversal — we skip branches that can't have answers.
