...
- Otherwise → current is the LCA (split point)
...
- If both are smaller → both in left subtree → go left
- If both are larger → both in right subtree → go right
- If one is smaller, one is larger → they split here → LCA found!
- If current equals p or q → current is LCA (ancestor of itself)
...
Step 1: At 6 → both < 6 → go left
Step 2: At 2 → both > 2 → go right
...
```python
# ❌ Wrong: Only checks for split, not equality
if p.val < root.val and q.val < root.val:
...
# ✅ Correct: The else case handles it
...
# ✅ Correct: Smaller values are in LEFT subtree
if p.val < root.val and q.val < root.val:
    return self.LCA(root.left, p, q)
```
...
**Opening statement:**
"Since this is a BST, I can use the ordering property. The LCA is the first node where p and q diverge — where one goes left and one goes right. I'll navigate down: if both are smaller, go left; if both are larger, go right; otherwise, I've found the LCA."
