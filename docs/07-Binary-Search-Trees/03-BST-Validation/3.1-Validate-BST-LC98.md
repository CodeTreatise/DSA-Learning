...
- [DFS Traversals](../../06-Trees/03-Tree-Traversals/3.1-DFS-Traversals/01-Inorder-Traversal.md)
...
Each node has a valid range it must fall within:
        ...
        5 (-∞, +∞)      Root can be any value
       / \
      1   4             Wrong! 4 should be in (5, +∞)
     /   / \
    0   3   6

For left child: range becomes (-∞, parent)
For right child: range becomes (parent, +∞)
...
INVALID BST that passes parent-only check:
      ...
      3   8
       \
        7   ← 7 > 3 (OK with parent) but 7 > 5 (violates grandparent!)
...
- Valid BST → inorder traversal is strictly increasing
...
# ✅ Correct: Strict inequality (standard BST has unique values)
if node.val <= min_val or node.val >= max_val:
    return False
```
...
```python
# ❌ Potential issue in languages with fixed-size integers
min_val = -2147483648 - 1  # Overflow!

# ✅ Use float('inf') or separate null checks
min_val = float('-inf')
max_val = float('inf')
```
...
**Opening statement:**
"I'll use a range-based DFS. Each node has a valid range it must fall within. For the root it's (-∞, +∞), and the range narrows as I go down — left children must be less than their ancestor chain, right children must be greater."

**Common clarifications:**
- "Are there duplicate values?" → Standard BST has unique values
- "Can I modify the tree?" → No, just validate
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Microsoft | ⭐⭐⭐⭐ | Fundamental BST |
| Meta | ⭐⭐⭐⭐ | Often as warm-up |
| Google | ⭐⭐⭐ | Usually combined with other |
