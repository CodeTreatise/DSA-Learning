...
First violation: 6 > 3 → first = 6 (previous), second = 3
Second violation: 5 > 2 → update second = 2
...
```python
# ❌ Wrong: Only looking for two separate violations
for i in range(len(nodes) - 1):
    if nodes[i].val > nodes[i + 1].val:
        violations.append((nodes[i], nodes[i + 1]))
# If adjacent, there's only 1 violation!

# ✅ Correct: Set second at first violation, update at second
if first is None:
    first = prev
...
# ✅ Correct: Update prev after visiting
def inorder(node):
    if node:
        inorder(node.left)
        if self.prev and self.prev.val > node.val:
...
```python
# ❌ Wrong: Trying to swap nodes (complex pointer manipulation)
def swap_nodes(a, b):
    # Would need to update parent pointers, children, etc.
    ...

# ✅ Correct: Just swap values (simple and sufficient)
first.val, second.val = second.val, first.val
```
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐ | Medium-hard BST |
| Microsoft | ⭐⭐⭐ | Tests BST understanding |
| Google | ⭐⭐ |" May ask for O(1) space "|
| Meta | ⭐⭐ | Less common |
...
- **Uses:** [Inorder Traversal](../../06-Trees/03-Tree-Traversals/3.1-DFS-Traversals/01-Inorder-Traversal.md)
