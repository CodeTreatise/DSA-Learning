...
- If node < low → entire left subtree is also < low
- If node > high → entire right subtree is also > high
...
If current node value < low:
  ...
  - Entire LEFT subtree has values < current < low → remove all
  - RIGHT subtree might have valid values → trim and return

If current node value > high:
  - Current node must be removed
  - Entire RIGHT subtree has values > current > high → remove all
  - LEFT subtree might have valid values → trim and return
...
Step 1: At node 3 (in range [1,3]) → Keep
...
Step 4: At node 2 (in range) → Keep
        Trim left (1) and right (null)

Step 5: At node 1 (in range) → Keep
...
var trimBST = function(root, low, high) {
    ...
    // Value too small → skip to right subtree
    if (root.val < low) {
        return trimBST(root.right, low, high);
    }
    
    // Value too large → skip to left subtree
    if (root.val > high) {
        return trimBST(root.left, low, high);
    }
    
    // Value in range → keep node, trim children
...
```python
# ❌ Wrong: Not returning when skipping node
if root.val < low:
...
```python
# ❌ Wrong if bounds are inclusive
if root.val < low:  # Should be root.val <= low for exclusive

# ✅ Correct for inclusive bounds [low, high]
if root.val < low:
    return self.trimBST(root.right, low, high)
if root.val > high:
    return self.trimBST(root.left, low, high)
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐ | Tree modification |
| Google | ⭐⭐⭐ | Clean recursion |
| Microsoft | ⭐⭐ | BST operations |
| Meta | ⭐⭐ | Less common |
...
> **The BST property turns range filtering from O(n) checking into O(n) pruning with O(log n) best case.** We don't just skip individual nodes — we skip entire subtrees because we know all their values are on one side of the boundary.
