...
Output: 5  (inorder: 1,2,3,4,5,6 → successor of 4 is 5)
```
...
- Right subtree exists → leftmost in right
- No right subtree → lowest ancestor where node is in left subtree
...
```
Find successor of 4:
        5   ← Successor! (4 is in left subtree of 5)
       / \
      3   6
     / \
    2   4   ← Start here, no right child

Walk up: 4→3 (4 is right child of 3, keep going)
        3→5 (3 is left child of 5, FOUND!)
...
Node 4: No right subtree → search from root
Node 3: Has right subtree (4) → go right, then left
```
...
# ✅ Correct: Successor must be strictly greater
while current:
    if p.val < current.val:
        successor = current
        current = current.left
...
```python
# ❌ Wrong: Not handling when p is the maximum
def inorderSuccessor(root, p):
    # ... search logic
    return successor  # Could return wrong value if p is max

# ✅ Correct: Initialize successor to None
def inorderSuccessor(root, p):
...
```python
# ❌ Inefficient: Always searching from root
def inorderSuccessor(root, p):
    # Always does full search, even if p has right child

# ✅ Efficient: Check right subtree first
def inorderSuccessor(root, p):
    if p.right:
...
**Opening statement:**
"To find the inorder successor, there are two cases. If the node has a right subtree, the successor is the leftmost node in that subtree. If not, I search from the root and track the last node where I went left — that's the successor."
...
**Follow-up questions:**
- "What if the node is the maximum?" → Return null, no successor
- "Can you do it with parent pointers?" → Yes, walk up until left child
- "How is this used in deletion?" → Replace deleted node's value with successor's
...
|---------|-----------|-------|
| Microsoft | ⭐⭐⭐⭐ | Very common |
| Amazon | ⭐⭐⭐ | Part of BST operations |
| Facebook | ⭐⭐⭐ | Iterator pattern |
| Google | ⭐⭐ | Combined with other problems |
...
> **The successor is the "next" in sorted order, which BST encodes spatially.** If there's a right subtree, the next larger is the minimum there. Otherwise, it's the nearest ancestor where we're "coming from the left" — meaning we haven't yet visited that ancestor in inorder.
