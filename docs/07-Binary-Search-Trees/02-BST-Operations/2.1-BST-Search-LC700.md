...
Path: 4 → 2 → 3 (3 comparisons for 5 nodes)
```
...
3. **If reach null → value not found**
     ...
     \
      2
       \
        3
         \
          4   ← Searching for 4 requires O(n) steps
```
...
```python
# ❌ Wrong: Return True/False instead of node
def searchBST(root, val):
    while root:
        if val == root.val:
...
**Follow-up questions:**
- "What if the tree is skewed?" → O(n) worst case
- "Recursive or iterative?" → Iterative for O(1) space
- "What if value doesn't exist?" → Return null
...
|---------|-----------|-------|
| All companies | ⭐⭐⭐⭐⭐ | Foundation for all BST problems |
