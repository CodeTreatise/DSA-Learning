...
- All values in left subtree are still smaller ✓
- All values in right subtree are still larger ✓
...
```python
# ❌ Wrong: Forgetting leaf case or one-child case
def deleteNode(root, key):
    if root.val == key:
        successor = findMin(root.right)  # Crashes if no right!
        ...

# ✅ Correct: Check cases in order
def deleteNode(root, key):
    if root.val == key:
        if not root.left:
            return root.right
...
```python
# ❌ Wrong: Not reassigning the child
def deleteNode(root, key):
    if key < root.val:
        self.deleteNode(root.left, key)  # Lost the returned value!
    ...

# ✅ Correct: Reassign the returned subtree
def deleteNode(root, key):
    if key < root.val:
...
```python
# ❌ Wrong: Forgetting to delete successor after copying value
def deleteNode(root, key):
    if found:
...
# ✅ Correct: Delete the successor from right subtree
def deleteNode(root, key):
    if found:
        successor = findMin(root.right)
        root.val = successor.val
...
**Follow-up questions:**
- "Why successor and not just any node?" → Must maintain BST ordering
- "Could you use predecessor?" → Yes, same approach but max of left
- "What about iterative?" → More complex but O(1) space
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Common BST question |
| Microsoft | ⭐⭐⭐⭐ | Fundamentals of BST |
| Google | ⭐⭐⭐ | May ask about balancing |
...
> **BST deletion's complexity comes from maintaining the invariant.** The successor (or predecessor) is the only value that can replace a two-child node without violating the BST property — it's the closest value on one side, ensuring all comparisons still work.
