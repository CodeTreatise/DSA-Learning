...
Inserting 5 into BST:
     ...
     / \                Step 3: Left child is NULL → INSERT HERE!
    1   3               
...
Result:
      ...
      2   7
     / \ /
    1  3 5   ← New node inserted as leaf
```
...
```python
# ❌ Wrong: Not using BST property correctly
def insertIntoBST(root, val):
    if val < root.val:
        root.left = TreeNode(val)  # Overwrites existing left child!
    ...

# ✅ Correct: Navigate to null position
def insertIntoBST(root, val):
    if val < root.val:
        if root.left:
...
**Follow-up questions:**
- "What if you insert sorted values?" → Tree becomes skewed, O(n) operations
- "How to keep it balanced?" → AVL or Red-Black tree rotations
- "Iterative or recursive?" → Iterative for O(1) space
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Common building block |
| Microsoft | ⭐⭐⭐⭐ | Part of BST operation set |
| Google | ⭐⭐⭐ | May ask about balancing |
| Meta | ⭐⭐⭐ | Foundation for harder problems |
...
> **BST insert always adds a leaf.** The search determines WHERE, and the insert simply attaches a new node at a null position. This is why unbalanced insertions (sorted data) create skewed trees — each new node extends the longest path.
