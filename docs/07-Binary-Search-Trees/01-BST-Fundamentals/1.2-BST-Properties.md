...
## ❌ When NOT to Use BST Properties
...
```
        15
       /  \
      10   20    ← Both subtrees are valid BSTs
     /  \    \
    5   12   25

Left subtree rooted at 10: All values < 15 ✓
Right subtree rooted at 20: All values > 15 ✓
```

### Property 5: Value Ranges in Subtrees

```
At node 15 with range (-∞, +∞):
├── Left subtree (10): range (-∞, 15)
│   ├── Left (5): range (-∞, 10)
│   └── Right (12): range (10, 15)
└── Right subtree (20): range (15, +∞)
    └── Right (25): range (20, +∞)
...
def is_height_balanced(root: TreeNode) -> bool:
    ...
    difference between left and right subtrees is ≤ 1.
...
```python
# ❌ Wrong: Assuming balanced
def operation(root):
    # "This is O(log n) because BST"
    ...

# ✅ Correct: State the assumption
def operation(root):
    """
    Time: O(h) where h is height
    - O(log n) if balanced
...
```
        15        ← depth 0, height of this subtree = 3
       /  \
      10   20     ← depth 1
     /
    5             ← depth 2, height of this subtree = 1
   /
  3               ← depth 3, height of this subtree = 0
...
```python
# ❌ Slow: Full traversal to find min
...
**Questions to expect:**
- "What's the time complexity?" → State O(h), then clarify balanced vs skewed
- "Can you optimize this?" → Use BST properties to prune
- "What if the tree is skewed?" → Discuss O(n) worst case
...
> **Every BST property comes from the recursive definition:** left < node < right. The min being leftmost, max being rightmost, inorder being sorted — all are consequences of this single invariant applied recursively.
