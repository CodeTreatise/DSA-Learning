...
**Case 2: Node has no right subtree → Successor is first ancestor where node is in left subtree**

```
Finding successor of 17:
              20     ← Successor! (17 is in left subtree of 20)
            /
          10
            \
             15
               \
                17   ← Start here, no right child
                
Walk up: 17→15 (17 is right child, keep going)
        15→10 (15 is right child, keep going)
        10→20 (10 is left child, FOUND!)
```
...
**Case 2: Node has no left subtree → Predecessor is first ancestor where node is in right subtree**

```
Finding predecessor of 25:
              20     ← Predecessor! (25 is in right subtree of 20)
                \
                 30
                /
               25    ← Start here, no left child
```
...
def successor_from_node(root: TreeNode, target: TreeNode) -> TreeNode:
    ...
    # Case 1: Right subtree exists → leftmost in right
    if target.right:
...
def inorder_predecessor(root: TreeNode, target: TreeNode) -> TreeNode:
    ...
    1. If left subtree exists → rightmost in left
    2. Otherwise → lowest ancestor where target is in right subtree
...
def predecessor_from_node(root: TreeNode, target: TreeNode) -> TreeNode:
    ...
    # Case 1: Left subtree exists → rightmost in left
    if target.left:
        return find_max(target.left)
    
    # Case 2: No left subtree → search from root
    predecessor = None
    current = root
...
| **Floor/Ceiling** | Largest ≤ target, Smallest ≥ target | Approximate search |
...
```python
# ❌ Wrong: Not handling when target is maximum
def successor(root, target):
    # ... search logic
    return successor  # Could be None!

# ✅ Correct: Explicitly handle None case
def successor(root, target):
...
```python
# ❌ Wrong: Always looking in right subtree
def successor(root, target):
...
- "Why is successor useful?" → BST deletion, iterators
- "What's the time complexity?" → O(h), mention balanced vs skewed
- "Can you do it without root access?" → Need parent pointers

**Red flags to avoid:**
- ❌ Doing inorder traversal to find successor (O(n) instead of O(h))
- ❌ Forgetting the "no right subtree" case
- ❌ Not handling edge cases (max element has no successor)
...
|---------|-----------|-------|
| Microsoft | ⭐⭐⭐⭐ | LC 285 is common |
| Amazon | ⭐⭐⭐ | BST deletion |
| Google | ⭐⭐⭐ | Iterator pattern |
| Meta | ⭐⭐ | Combined with other patterns |
...
> **Successor and predecessor are the "next" and "previous" in sorted order.** Since BST encodes sorted order spatially, finding next/previous is just navigating the tree structure — either into a subtree or up to an ancestor.
