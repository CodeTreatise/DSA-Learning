...
- [Tree Traversals](../../06-Trees/03-Tree-Traversals/3.1-DFS-Traversals/01-Inorder-Traversal.md) - Inorder, preorder, postorder
- [Binary Search](../.../03-Sorting-Searching/03-Sorting-Searching.md#binary-search) - The algorithm BST is based on
...
Inorder: 20, 30, 40, 50, 60, 70, 80
         ↑ sorted! This is the KEY insight!
```

> **💡 Key Insight:** Inorder traversal of a BST always produces a sorted sequence. This is the foundation for many BST algorithms.
...
```
❌ WRONG understanding: Only check left < node < right

        5
       / \
      3   7
     / \
    1   6  ← This violates BST! 6 > 5 but in left subtree

✅ CORRECT: ALL nodes in left subtree must be < node
```

### 2. Off-by-One with Equal Values

```python
# ❌ Wrong: Using <= allows duplicates in unexpected places
if val <= root.val:
    root.left = insert(root.left, val)

# ✅ Correct: Standard BST has strictly less than
if val < root.val:
    root.left = insert(root.left, val)
elif val > root.val:
    root.right = insert(root.right, val)
...
```python
# ❌ Wrong assumption: "BST search is always O(log n)"
# Reality: Sorted input creates O(n) search

# ✅ Correct: Use balanced BST or randomize input order
import random
...
**Red flags to avoid:**
- ❌ Treating BST like a regular binary tree (missing optimization)
- ❌ Not mentioning O(n) worst case
- ❌ Confusing BST property (subtree vs immediate child)
...
|---------|-----------|-----------------|
| Amazon | ⭐⭐⭐⭐ | Validate BST, LCA in BST |
| Microsoft | ⭐⭐⭐⭐ | BST operations, k-th smallest |
| Google | ⭐⭐⭐ | Often combined with other patterns |
| Meta | ⭐⭐⭐ | Inorder iterator, two sum in BST |
