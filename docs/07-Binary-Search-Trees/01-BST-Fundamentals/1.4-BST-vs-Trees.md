...
## ❌ When NOT to Use BST Algorithms
...
```python
# ❌ Wrong: Treating any tree as BST
...
# ✅ Correct: Check if BST first, or use general search
def find_in_tree(root, target):
    if not root:
        return None
    if root.val == target:
...
```python
# ❌ Slow: Using general approach when BST property helps
...
# ✅ Fast: Stop early using BST property
...
#     3   8
#      \
#       7  ← 7 > 5, but it's in left subtree!
