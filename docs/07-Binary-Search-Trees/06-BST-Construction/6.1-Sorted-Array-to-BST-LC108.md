...
**Why O(log n) space:**
- Tree is balanced → height is O(log n)
...
# ✅ Correct: Only skip when left > right
if left > right:
    return None
```
...
```python
# ❌ Wrong: Includes mid in subtrees (infinite loop)
