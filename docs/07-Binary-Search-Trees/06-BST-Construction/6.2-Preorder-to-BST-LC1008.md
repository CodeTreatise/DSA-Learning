...
So: 8 is root
    [5, 1, 7] are all < 8 → left subtree
    [10, 12] are all > 8 → right subtree
...
```python
# ❌ Wrong: Slicing creates copies, index gets lost
...
```python
# ❌ Wrong: Right before left
node.right = build(val, max_val)  # Consumes wrong elements!
node.left = build(min_val, val)

# ✅ Correct: Left before right (matches preorder)
...
```python
# ❌ Wrong: Creates node even when out of range
def build(min_val, max_val):
    val = preorder[self.index]
...
# ✅ Correct: Check range BEFORE advancing index
def build(min_val, max_val):
    if self.index >= len(preorder):
        return None
    val = preorder[self.index]
...
| Activity | Time | Notes |
|----------|------|-------|
| Understand problem | 3-4 min | Preorder → BST |
