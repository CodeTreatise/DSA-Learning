...
```mermaid
flowchart TD
    A["dividend = 43, divisor = 5"] --> B["Find largest 2^k where divisor*2^k ≤ dividend"]
    B --> C["5*8=40 ≤ 43, so subtract 40, add 8 to quotient"]
...
```python
# ❌ WRONG - XOR on booleans might not work as expected
negative = (dividend < 0) ^ (divisor < 0)  # Works in Python

# ✅ CLEARER - Use != for clarity
negative = (dividend < 0) != (divisor < 0)
```
...
```python
# ❌ WRONG - abs(INT_MIN) overflows in some languages
dividend = abs(dividend)  # In C/Java, abs(-2^31) = -2^31!

# ✅ CORRECT - Check and handle specially
if dividend == INT_MIN:
    if divisor == 1:
        return INT_MIN
    if divisor == -1:
...
> **💡 Key Insight:** Division is repeated subtraction, but we can subtract 2^k * divisor at once to speed it up exponentially. This is essentially binary search for the quotient.

> **🔗 Related:** [Addition Without Plus ←](./5.1-Addition-Without-Plus.md) | [Multiplication/Division →](./5.3-Multiplication-Division.md) | [Binary Search](../../03-Sorting-Searching.md)
