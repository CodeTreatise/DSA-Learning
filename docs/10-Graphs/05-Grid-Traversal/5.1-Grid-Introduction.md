...
>
> 🎯 **Interview Impact:** ~20% of all graph problems are grid-based | **Frequency:** Very High
...
Nodes: m * n cells
Edges: ~4 * m * n (each cell has up to 4 neighbors)
```
...
```python
# ❌ Wrong: Missing directions or duplicates
directions = [(1, 1), (1, 0), (0, 1)]  # Only 3 directions + diagonal

# ✅ Correct: All 4 cardinal directions
directions = [(0, 1), (0, -1), (1, 0), (-1, 0)]  # R, L, D, U
```
...
```python
# ❌ If problem says "don't modify input"
...
> **💡 Key Insight:** A grid is just a graph with `m * n` nodes and `~4mn` edges. Every grid problem translates to a graph algorithm: connected components, shortest path, or flood fill.

---

**Next:** [Island Pattern →](./5.2-Island-Pattern.md)
