...
>
> ⏱️ **Study Time:** 30 min | 📊 **Priority:** Medium | 🎯 **Pattern:** Dynamic Programming
...
| Large sparse graph | V * Dijkstra | O(V * E log V) < O(V³) |
| Unweighted | V * BFS | O(V * (V+E)) |
...
- [Dynamic Programming Basics](../.../11-Dynamic-Programming/11-Dynamic-Programming.md)
...
```
Key question: Can we improve path i→j by going through k?
...
- Dense graphs (E ≈ V²): Floyd-Warshall is simpler
- Need all pairs: V * Dijkstra = O(V * E log V)
  - Sparse: E ≈ V → O(V² log V) < O(V³) ✓
  - Dense: E ≈ V² → O(V³ log V) > O(V³), so Floyd-Warshall wins
...
def minimax_path(n, edges):
    ...
    for k in range(n):
        for i in range(n):
            for j in range(n):
                # Path i→k→j has max edge = max(max_edge(i,k), max_edge(k,j))
...
```python
# ❌ WRONG: inf + inf can cause issues
dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j])
...
```python
# ❌ WRONG: Forget to initialize diagonal
dist = [[INF] * n for _ in range(n)]
# dist[i][i] is INF!

# ✅ CORRECT: Self-distance is 0
for i in range(n):
    dist[i][i] = 0
```
...
> **💡 Key Insight:** Floyd-Warshall is **DP with a clever ordering**. By making k the outer loop, when we consider paths through k, we've already computed all optimal paths using nodes 0..k-1.

---

> **🔗 Related:** [Dijkstra](./8.2-Dijkstra.md) | [Bellman-Ford](./8.3-Bellman-Ford.md) | [Dynamic Programming](../.../11-Dynamic-Programming/11-Dynamic-Programming.md)

---

**Back:** [← Bellman-Ford](./8.3-Bellman-Ford.md) | **Next:** [Shortest Path Practice →](./8.4-Path-Problems.md/)
