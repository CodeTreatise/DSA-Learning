...
>
> ⏱️ **Study Time:** 30 min | 📊 **Priority:** Medium | 🎯 **Pattern:** Dynamic Programming on Edges
...
|----------|--------------|-----|
| Non-negative weights | Dijkstra | O(E log V) vs O(V * E) |
...
Edges: 0→1 (weight 5), 1→2 (weight -4), 0→2 (weight 2)
...
3. Pop 2 (dist=2): Node 2 "finalized" with dist=2
   But wait... we haven't explored 0→1→2 yet!
4. Pop 1 (dist=5): Try to update 2: 5+(-4)=1 < 2
   But node 2 is already "done"! Dijkstra ignores this.

Dijkstra's answer: dist[2] = 2 ❌
Correct answer:    dist[2] = 1 ✓ (via path 0→1→2)
...
Iteration 1 (relax all edges):
  Edge (0,1,5): dist[1] = min(∞, 0+5) = 5
  Edge (0,2,2): dist[2] = min(∞, 0+2) = 2
  Edge (1,2,-4): dist[2] = min(2, 5+(-4)) = 1 ✓
...
After V-1 iterations:
  One more pass: 1 →(-3)→ 0 still improves!
  This means infinite improvement → negative cycle
```
...
|------|------|-------|-------|
| Standard | O(V * E) | O(V) | V-1 passes over E edges |
| Sparse (E ≈ V) | O(V²) | O(V) | Reasonable |
| Dense (E ≈ V²) | O(V³) | O(V) | Consider Floyd-Warshall |
| With early termination | O(V * E) worst | O(V) | Often faster in practice |
...
```python
# ❌ WRONG: Cascading updates in same iteration
for u, v, w in edges:
    dist[v] = min(dist[v], dist[u] + w)
# Problem: An update to dist[u] affects dist[v] in same iteration!

# ✅ CORRECT: Use copy of previous distances
new_dist = dist.copy()
for u, v, w in edges:
    if dist[u] != float('inf'):
...
```python
# ❌ WRONG: Relax even when source distance is infinity
if dist[u] + weight < dist[v]:
...
```python
# ❌ WRONG: n iterations (should be n-1)
for _ in range(n):  # One too many for shortest path

# ✅ CORRECT: n-1 for shortest path, n-th for cycle detection
...
> **💡 Key Insight:** Bellman-Ford is essentially **dynamic programming on edges**. Each iteration allows paths to use one more edge. V-1 iterations cover all possible path lengths.

---

> **🔗 Related:** [Dijkstra](./8.2-Dijkstra.md) | [Floyd-Warshall](./8.4-Floyd-Warshall.md) | [Cycle Detection](../09-Cycle-Detection/9.1-Undirected-Cycles.md)

---

**Back:** [← Dijkstra](./8.2-Dijkstra.md) | **Next:** [Floyd-Warshall →](./8.4-Floyd-Warshall.md)
