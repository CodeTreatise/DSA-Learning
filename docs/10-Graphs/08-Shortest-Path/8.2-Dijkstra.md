...
>
> ⏱️ **Study Time:** 45 min | 📊 **Priority:** Critical | 🎯 **Pattern:** Greedy + Priority Queue
...
**Named after:** Edsger W. Dijkstra (1959) - Conceived in 20 minutes at a café!

```
Core Idea:
1. Start with source at distance 0, all others at ∞
...
| Dense graph, all pairs | Floyd-Warshall | O(V³) vs O(V * E log V) |
...
Graph:          Step 1: Start at A
    2                dist[A] = 0
A ───→ B            Process A, update neighbors
│       │   
1       3           dist = {A:0, B:2, C:1, D:∞}
↓       ↓           heap = [(1,C), (2,B)]
C ───→ D
    4
...
| Dense (E ≈ V²) | O(V² log V) | O(V) | Consider Floyd-Warshall |
| Sparse (E ≈ V) | O(V log V) | O(V) | Very efficient |
...
# Dijkstra says dist[2] = 1 (via 0→2)
# But 0→1→2 = 4 + (-3) = 1 would be found later!
...
```python
# ❌ WRONG: Process every heap entry
while heap:
    d, u = heapq.heappop(heap)
    for v, w in graph[u]:
...
```python
# ❌ WRONG: Assuming no parallel edges
graph[u] = (v, weight)  # Overwrites previous edge!

# ✅ CORRECT: Handle multiple edges
...
> **💡 Key Insight:** Dijkstra works because of the **greedy property** - once we've settled the shortest distance to a node (popped from heap), we can never improve it. This only holds when all weights are non-negative!

---

> **🔗 Related:** [BFS Pattern](../03-BFS-Traversal/3.1-BFS-Fundamentals.md) | [Bellman-Ford](./8.3-Bellman-Ford.md) | [Prim's MST](../11-MST/11.2-Prims-Algorithm.md)

---

**Back:** [← Shortest Path Overview](./8.1-Shortest-Path-Overview.md) | **Next:** [Bellman-Ford →](./8.3-Bellman-Ford.md)
