...
>
> ⏱️ **Study Time:** 45 min | 📊 **Priority:** High | 🎯 **Interview:** Meta, Google, Amazon
...
```
Problem Types:
├── Single Source (one start, all destinations)
│   ├── Non-negative weights → Dijkstra's O(E log V)
│   └── Negative weights → Bellman-Ford O(V * E)
├── All Pairs (every node to every node)
...
| **Bellman-Ford** | Any weights | O(V * E) | O(V) | Negative edges, detect neg cycles |
| **Floyd-Warshall** | Any weights | O(V³) | O(V²) | All pairs, small dense graphs |
...
**Visualization:**
```
Current state: dist[A] = 0, dist[B] = 10, dist[C] = ∞
...
Relax A→B: dist[B] = min(10, 0+5) = 5  ✓ Updated!
Relax B→C: dist[C] = min(∞, 5+3) = 8   ✓ Updated!
Relax B→D: dist[D] = min(∞, 5+4) = 9   ✓ Updated!
```
...
```python
# ❌ Wrong: Dijkstra gives wrong answer with negative weights
graph = {0: [(1, 5), (2, 2)], 1: [(2, -10)], 2: []}
# Dijkstra processes 2 before 1, misses the shorter path 0→1→2
...
```python
# ❌ Wrong: Processing outdated distances
while heap:
    d, u = heapq.heappop(heap)
    # Process even if we already found a better path

# ✅ Correct: Skip if we've found better
while heap:
    d, u = heapq.heappop(heap)
    if d > dist[u]:
...
```python
# ❌ Wrong: Assuming all nodes reachable
return dist[destination]  # Might be inf!

# ✅ Correct: Handle unreachable nodes
...
**Explain the choice:**
> "Dijkstra is O(E log V) but only works with non-negative weights. Bellman-Ford is O(V * E) but handles negatives."

**Red flags to avoid:**
- ❌ Using Dijkstra when weights can be negative
...
> **💡 Key Insight:** Shortest path is really about **relaxation** - repeatedly asking "can I do better by going through this node?" The algorithms differ only in *how* they explore nodes.

---

> **🔗 Related:** [BFS Pattern](../03-BFS-Traversal/3.1-BFS-Fundamentals.md) | [Dijkstra Deep Dive](./8.2-Dijkstra.md) | [MST Algorithms](../11-MST/11.1-MST-Introduction.md)

---

**Next:** [Dijkstra's Algorithm →](./8.2-Dijkstra.md)
