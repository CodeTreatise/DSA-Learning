...
>
> ⏱️ **Time:** 15-20 min | 📊 **Difficulty:** Intermediate | 🎯 **Interview Frequency:** High (Trade-off Discussion)
...
The Golden Rule:
┌───────────────────────────────────────────────────────────────┐
│   95% of interview problems → Use ADJACENCY LIST              │
│   Dense graphs (E ≈ V²) → Consider Adjacency Matrix           │
│   Edge-centric algorithms → Keep Edge List                    │
└───────────────────────────────────────────────────────────────┘
```
...
2. Is the graph dense (E ≈ V²) or sparse (E << V²)?
...
Start
 │
 ├─ Need to traverse neighbors? ─────────────────→ Adjacency List
 │
 ├─ Need O(1) edge check AND dense graph? ───────→ Adjacency Matrix
 │
 ├─ Need to sort/iterate all edges? ─────────────→ Edge List
 │
 └─ Default (most problems) ─────────────────────→ Adjacency List
```
...
| **BFS/DFS traversal** | O(V²) | ⭐ O(V + E) | O(V * E) |
| **Find degree** | O(V) | ⭐ O(1) | O(E) |
...
| Use When | Example |
|----------|---------|
| Dense graph (E ≈ V²) | Complete graph, small social network |
| Need O(1) edge check | "Is there a direct flight between A and B?" |
| Floyd-Warshall algorithm | All-pairs shortest path |
| Small V (≤ 1000) | Memory isn't a constraint |
...
- V is large (> 1000): O(V²) = 1,000,000+ memory
...
- BFS/DFS traversal: O(V * E) instead of O(V + E)
...
ADJACENCY MATRIX (5*5 = 25 cells)
     0  1  2  3  4
   ┌──────────────┐
 0 │ 0  1  0  1  1│   Space: O(V²) = O(25)
 1 │ 1  0  1  0  1│   For dense graph: efficient
 2 │ 0  1  0  0  1│   For sparse graph: wasteful
 3 │ 1  0  0  0  0│
 4 │ 1  1  1  0  0│
   └──────────────┘
...
ADJACENCY LIST (6 edges * 2 = 12 entries)

 0 → [1, 3, 4]                    Space: O(V + E) = O(5 + 6)
 1 → [0, 2, 4]                    Only stores actual edges
 2 → [1, 4]                       Efficient for traversal
 3 → [0]
 4 → [0, 1, 2]
...
List:   ████████████████████     List:   ████
        O(V + E) ≈ O(V²)                 O(V + E) ≈ O(V) EFFICIENT

Edge:   ████████████████         Edge:   ██
        O(E) ≈ O(V²)                     O(E) ≈ O(V) MOST COMPACT
```
...
|------------|---------------------|-----|
| Sparse (E << V²) | Adjacency List | O(V + E) space, O(degree) neighbors |
| Dense (E ≈ V²) | Adjacency Matrix | O(1) edge check, space similar |
...
```python
# ❌ Wrong: V = 10,000, E = 20,000 (sparse)
# Matrix uses 10,000 * 10,000 = 100 MILLION cells!
matrix = [[0] * 10000 for _ in range(10000)]  # Memory limit exceeded

# ✅ Correct: Use adjacency list
...
# ✅ Correct: Convert first, then O(V + E) traversal
...
```python
# ❌ Wrong: Trying to use edge list directly for traversal
edges = [[0,1], [0,2], [1,2]]
# "How do I get neighbors of node 0?"

# ✅ Correct: Convert immediately at start
def solve(edges, n):
...
> **💡 Key Insight:** Default to adjacency list (95% of problems). Only use matrix for dense graphs with O(1) edge checks, or edge list for edge-centric algorithms like Kruskal's.

> **🔗 Related:** [BFS Fundamentals](../03-BFS-Pattern/3.1-BFS-Fundamentals.md) | [DFS Fundamentals](../04-DFS-Pattern/4.1-DFS-Fundamentals.md)

---

**Previous:** [← 2.3 Edge List](./2.3-Edge-List.md)  
**Next:** [3.1 BFS Fundamentals →](../03-BFS-Pattern/3.1-BFS-Fundamentals.md)
