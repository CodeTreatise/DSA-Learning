...
>
> ⏱️ **Time:** 25-35 min | 📊 **Difficulty:** Beginner | 🎯 **Interview Frequency:** Foundation
...
```
    ...
    deg(B) = 3                 in-deg(B) = 1 (A→B)
    deg(A) = 2                 out-deg(B) = 2 (B→C, B→D)
    deg(C) = 3                 in-deg(D) = 1
```
...
**Handshaking Lemma:** Sum of all degrees = 2 * |E| (each edge counted twice)
...
|------|------------|--------|
| **Walk** | Sequence of vertices (can repeat) | A→B→C→B→D |
| **Trail** | Walk with no repeated edges | A→B→C→D (edges distinct) |
| **Path** | Walk with no repeated vertices | A→B→C→D (vertices distinct) |
| **Circuit** | Trail starting/ending at same vertex | A→B→C→D→A (trail + cycle) |
| **Cycle** | Path starting/ending at same vertex | A→B→C→A (path + closes) |
...
# ❌ Wrong thinking
"A and C are connected" → checking if edge (A,C) exists

# ✅ Correct
# A and C are adjacent if edge (A,C) exists
# A and C are connected if ANY path A→...→C exists
```

### 2. Path vs Walk

```python
# ❌ Wrong: Counting A→B→A→C as a valid path
# Paths don't repeat vertices!

# ✅ Correct
# A→B→A→C is a WALK (vertices can repeat)
# A→B→C is a PATH (no vertex repeats)
```

### 3. Degree Sum Property

```python
# ❌ Wrong: Expecting sum of degrees = |E|

# ✅ Correct: Handshaking lemma
...
|---------|------------|-------|
| [Find the Town Judge](https://leetcode.com/problems/find-the-town-judge/) | 🟢 Easy | In-degree, out-degree |
| [Find Center of Star Graph](https://leetcode.com/problems/find-center-of-star-graph/) | 🟢 Easy | Degree analysis |

### Medium (Path/Connected Analysis)
| Problem | Difficulty | Focus |
|---------|------------|-------|
| [Number of Connected Components](https://leetcode.com/problems/number-of-connected-components-in-an-undirected-graph/) | 🟡 Medium | Components |
| [Graph Valid Tree](https://leetcode.com/problems/graph-valid-tree/) | 🟡 Medium | Connected + Acyclic |
...
**Red flags to avoid:**
- ❌ Using "node" and "vertex" inconsistently
- ❌ Confusing "edge" with "path"
- ❌ Saying "connected" when you mean "adjacent"
...
> **💡 Key Insight:** Graph terminology is the language of graph algorithms. Mastering these terms makes learning BFS, DFS, Dijkstra, and other algorithms significantly easier.

> **🔗 Related:** [What Is a Graph?](./1.1-What-Is-Graph.md) | [Types of Graphs](./1.3-Types-Of-Graphs.md) | [Graph Representation](../02-Graph-Representation/2.2-Adjacency-List.md)

---

**Previous:** [← 1.1 What Is a Graph?](./1.1-What-Is-Graph.md)  
**Next:** [1.3 Types of Graphs →](./1.3-Types-Of-Graphs.md)
