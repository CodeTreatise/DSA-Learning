...
>
> ⏱️ **Study Time:** 25 min | 📊 **Priority:** Medium | 🎯 **Pattern:** Greedy + Union-Find
...
Key: Union-Find enables O(α(V)) cycle checking!
```
...
Step 2: Process edges
        Edge (0,2,1): 0 and 2 not connected → ADD
                      Union(0,2), MST weight = 1
        
        Edge (1,2,2): 1 and 2 not connected → ADD
                      Union(1,2), MST weight = 3
        
        Edge (0,1,4): 0 and 1 already connected (via 2) → SKIP
        
        Edge (1,3,5): 1 and 3 not connected → ADD
                      Union(1,3), MST weight = 8
        
        We have 3 edges (V-1) → DONE!
...
**Breakdown:**
- Sorting: O(E log E) = O(E log V) since E ≤ V²
...
```python
# ❌ WRONG: Assuming graph is connected
return total

# ✅ CORRECT: Verify we used V-1 edges
...
> **💡 Key Insight:** Kruskal's works because we process edges in sorted order. When we add an edge, it's guaranteed to be the minimum weight edge that could connect those two components.

---

**Back:** [← Prim's Algorithm](./11.2-Prims-Algorithm.md) | **Next:** [Advanced Topics →](../12-Advanced/12.1-Advanced-Overview.md)
