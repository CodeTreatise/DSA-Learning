...
>
> ⏱️ **Time:** 45-60 min | 📊 **Difficulty:** Medium-Hard | 🎯 **Interview Frequency:** ⭐⭐⭐⭐⭐
...
|------------|----------|---------------------|
| Connected Components | Count separate groups | ⭐⭐⭐⭐⭐ |
| Cycle Detection (Directed) | Course scheduling | ⭐⭐⭐⭐⭐ |
| Cycle Detection (Undirected) | Detect extra edge | ⭐⭐⭐⭐ |
| Topological Sort | Task ordering | ⭐⭐⭐⭐⭐ |
| Path Finding | Any path between nodes | ⭐⭐⭐⭐ |
| Clone Graph | Deep copy with DFS | ⭐⭐⭐⭐ |
...
Cycle exists ⟺ We encounter a GRAY node
...
When at B (came from C), seeing A IS a cycle
    ...
    A ── B    Visiting B from A, seeing A → NOT cycle
    │    │    
    └────┘    Visiting B from C, seeing A → CYCLE!
         C
```
...
DFS from A:
A → B → D (finish D, add D)
      → (back at B, finish B, add B)
  → C → E (finish E, add E)
      → (back at C, finish C, add C)
  → (back at A, finish A, add A)

Result: [D, B, E, C, A]
Reverse: [A, C, E, B, D] ← Topological order!
```
...
│       │         │ ╲     │
C(1) ── D(0)      │  ╲    │
                  C(1) ── D(?)  ← D needs 0 (from B) and 0 (from C)
                                  Can't satisfy both!
```
...
```python
# ❌ Wrong: Using undirected cycle detection for directed graph
def has_cycle_wrong(graph, n):
    visited = set()
    def dfs(node, parent):  # Parent tracking doesn't work for directed!
        # ...

# ✅ Correct: Use three-state for directed graphs
def has_cycle_directed(graph, n):
...
# ✅ Correct: Start from all unvisited
for node in range(n):
    if node not in visited:
        dfs(node)
```
...
> **💡 Key Insight:** Most DFS applications follow the same template with different state tracking: connected components track components, cycle detection tracks node states, topological sort tracks finish times.

> **🔗 Related:** [DFS Template](./4.2-DFS-Template.md) | [Topological Sort](../06-Topological-Sort/6.1-Topological-Basics.md) | [Union-Find](../07-Union-Find/7.1-Union-Find-Basics.md)

---

**Previous:** [← 4.2 DFS Template](./4.2-DFS-Template.md)  
**Next:** [DFS Practice Problems →](./4.3-DFS-Classic-Problems.md/)
