...
>
> ⏱️ **Study Time:** 30 min | 📊 **Priority:** High | 🎯 **Pattern:** DFS / Union-Find
...
1. DFS with parent tracking
   - When visiting neighbor, if visited AND not parent → cycle!
   
2. Union-Find
   - When adding edge, if both endpoints already connected → cycle!
```
...
| **Union-Find** | O(E * α(V)) | O(V) | Processing edges one by one |
...
If we see a visited neighbor that's NOT our parent → cycle!
...
DFS from 1:
  ...
  → Visit 2 (parent=1), mark visited
    → Visit 3 (parent=2), mark visited
      → See 4: not visited, go there
        Visit 4 (parent=3), mark visited
        → See 1: visited AND not parent(3) → CYCLE!
```
...
Process edges one by one:
  Edge (1,2): Union(1,2) → connected
  Edge (2,3): Union(2,3) → connected
  Edge (3,4): Union(3,4) → connected
  Edge (4,1): Find(4)=1, Find(1)=1, SAME! → CYCLE!
```
...
| **Union-Find** | O(E * α(V)) | O(V) | Nearly O(E) in practice |
...
```python
# ❌ WRONG: Any visited neighbor triggers cycle
if visited[neighbor]:
    return True  # But neighbor could be parent!

# ✅ CORRECT: Skip the parent
...
```python
# ❌ WRONG: Using directed graph logic for undirected
# In undirected, we need parent tracking
# In directed, we use three-color marking

# ✅ CORRECT: Use appropriate algorithm for graph type
```
...
> **💡 Key Insight:** In undirected graphs, a "back edge" (to a visited non-parent) means a cycle. Unlike directed graphs, we don't need three-color marking - just parent tracking!

---

> **🔗 Related:** [Directed Cycles](./9.2-Directed-Cycles.md) | [Union-Find](../07-Union-Find/7.1-Union-Find-Basics.md) | [DFS Pattern](../04-DFS-Traversal/4.1-DFS-Fundamentals.md)

---

**Next:** [Directed Graph Cycles →](./9.2-Directed-Cycles.md)
