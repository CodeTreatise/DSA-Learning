...
>
> ⏱️ **Time:** 30-40 min | 📊 **Difficulty:** Core | 🎯 **Interview Frequency:** ⭐⭐⭐⭐⭐
...
┌─────────────────────────────────────────────────────┐
│  1. INITIALIZATION                                   │
│     - Queue with starting state(s)                  │
│     - Visited set with starting state(s)            │
│     - (Optional) Distance/level tracking            │
├─────────────────────────────────────────────────────┤
│  2. BFS LOOP                                         │
│     while queue:                                    │
...
│         - For each valid unvisited state:           │
│           - Mark visited                            │
│           - Enqueue                                 │
├─────────────────────────────────────────────────────┤
│  3. RETURN RESULT                                    │
│     - Path found / distance / visited states        │
│     - Or -1 / empty if no solution                  │
└─────────────────────────────────────────────────────┘
```
...
- Multi-source BFS → Start with multiple nodes in queue
- Grid BFS → Start at (row, col)

**Modify "get neighbors":**
- Grid → 4 or 8 directions
- State machine → Valid transitions
- Word ladder → Words differing by 1 character

**Modify visited representation:**
- Simple graph → Set of nodes
- Grid → Set of (row, col)
- Complex state → Set of (state tuple)
...
def bfs_template(graph, start, target=None):
    ...
    Time: O(V + E) or O(states * transitions)
...
```
                    Start
                      ...
                      ▼
              ┌───────────────┐
              │ Shortest path │──Yes──► Template 2 (with distance)
              │ needed?       │
              └───────┬───────┘
                      │ No
                      ▼
              ┌───────────────┐
              │ Level-by-level│──Yes──► Template 3 (level processing)
              │ processing?   │
              └───────┬───────┘
                      │ No
                      ▼
              ┌───────────────┐
              │ 2D Grid?      │──Yes──► Template 4 (grid BFS)
              └───────┬───────┘
                      │ No
                      ▼
              ┌───────────────┐
              │ Multiple      │──Yes──► Template 5 (multi-source)
              │ start points? │
              └───────┬───────┘
                      │ No
                      ▼
              ┌───────────────┐
              │ Complex state?│──Yes──► Template 6 (state BFS)
              └───────┬───────┘
                      │ No
                      ▼
                Template 1 (basic)
...
| Grid | O(R * C) | O(R * C) | 2D matrix |
| Multi-Source | O(R * C) | O(R * C) | Multiple starts |
| With State | O(States * Transitions) | O(States) | Complex state |
...
```python
# ❌ Wrong: Using list (unhashable) for visited
state = [row, col, keys]
...
# ✅ Correct: Use while loop with popleft
while queue:
    item = queue.popleft()
...
> **💡 Key Insight:** The BFS template is remarkably consistent across problems. The only things that change are: (1) what you put in the queue (your "state"), (2) how you generate neighbors, and (3) when you stop.

> **🔗 Related:** [BFS Fundamentals](./3.1-BFS-Fundamentals.md) | [Multi-Source BFS](./3.3-Multi-Source-BFS.md) | [Grid Traversal](../05-Grid-Traversal/5.1-Grid-Introduction.md)

---

**Previous:** [← 3.1 BFS Fundamentals](./3.1-BFS-Fundamentals.md)  
**Next:** [3.3 Multi-Source BFS →](./3.3-Multi-Source-BFS.md)
