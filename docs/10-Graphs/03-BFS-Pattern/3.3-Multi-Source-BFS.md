...
>
> ⏱️ **Time:** 35-45 min | 📊 **Difficulty:** Medium | 🎯 **Interview Frequency:** ⭐⭐⭐⭐⭐ (Classic pattern)
...
All 0s spread simultaneously → Each cell gets distance to NEAREST 0!
```
...
| **Space** | O(m * n) | Queue + visited/result |
...
```python
# ❌ Wrong: Doesn't check if all oranges rotted
def orangesRotting(grid):
    # BFS...
    return max_time  # Might return time even if some can't rot

# ✅ Correct: Check if all reachable
def orangesRotting(grid):
...
> "The naive approach would run BFS from each source, giving O(sources * cells). But with multi-source BFS, I process all sources simultaneously in O(cells) total, because each cell is visited exactly once."

**Complexity discussion:**
> "Time is O(m * n) because each cell is processed once. Space is also O(m * n) for the queue in the worst case."
...
|---------|-----------|-------------------|
| Meta | ⭐⭐⭐⭐⭐ | Rotting Oranges |
| Amazon | ⭐⭐⭐⭐⭐ | 01 Matrix, Walls and Gates |
| Google | ⭐⭐⭐⭐ | Complex grid problems |
| Microsoft | ⭐⭐⭐⭐ | Spreading simulations |
...
> **💡 Key Insight:** When you need distance FROM multiple sources (or TO multiple targets), flip the perspective: start BFS from all targets simultaneously. Each cell will be reached first by its nearest target, giving you the answer in one pass!

> **🔗 Related:** [BFS Fundamentals](./3.1-BFS-Fundamentals.md) | [Grid Traversal](../05-Grid-Traversal/5.1-Grid-Introduction.md) | [Island Pattern](../05-Grid-Traversal/5.2-Island-Pattern.md)

---

**Previous:** [← 3.2 BFS Template](./3.2-BFS-Template.md)  
**Next:** [Rotting Oranges Practice →](./3.4-BFS-Practice/01-Rotting-Oranges-LC994.md)
