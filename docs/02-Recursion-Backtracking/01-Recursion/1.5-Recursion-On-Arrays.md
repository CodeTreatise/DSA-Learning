...
> **Interview Value:** ⭐⭐⭐⭐ - Foundation for divide-and-conquer sorting  
...
❌ **Wrong:** O(n) slicing at each step
```python
def sum(arr):
    if not arr: return 0
...
❌ **Wrong:** Recursion on 100K elements
```python
sum([1] * 100000)  # RecursionError!
```

✅ **Correct:** Use iteration or increase limit carefully
```python
sum(arr)  # Built-in
# Or
import sys
