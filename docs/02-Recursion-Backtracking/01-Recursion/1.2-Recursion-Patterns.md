...
> **Interview Value:** ⭐⭐⭐⭐⭐ - Recognize patterns → faster solutions  
...
## 📐 Pattern 3: Tree/Multiple Recursion
...
| Tree/Multiple | O(branches^depth) | O(depth) | Subsets O(2ⁿ) |
...
❌ **Wrong:** Using binary recursion for linear problem
```python
def array_sum(arr):
...
✅ **Correct:** Linear recursion for linear problem
```python
...
❌ **Wrong:** Non-tail when tail is possible
```python
...
> 1. Am I processing elements one at a time? → **Linear**
> 2. Am I dividing into two parts? → **Binary**
> 3. Am I generating all combinations? → **Tree/Multiple**
> 4. Can the recursive call be last? → **Tail**
> 5. Do states alternate? → **Mutual**
