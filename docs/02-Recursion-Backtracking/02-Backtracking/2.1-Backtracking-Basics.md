...
> **Interview Value:** ⭐⭐⭐⭐⭐ - Foundation for Subsets, Permutations, Combinations  
...
- [Core Patterns](./2.3-Core-Patterns/01-Subsets-Pattern.md) - Subsets, Permutations, etc.
...
```
1. CHOOSE  → Make a decision (add element, take path, etc.)
...
> "Since we're generating all solutions, the time is exponential—O(2^n) for subsets, O(n!) for permutations."
...
>
> The template is always: **CHOOSE → EXPLORE → UNCHOOSE**
