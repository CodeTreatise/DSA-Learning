...
> **Interview Value:** ⭐⭐⭐⭐⭐ - Top-tier interview problems  
...
Row 1: Can't use col 0 (same col), can't use col 1 (diagonal)
  - Place Q at (0,1) originally → Try (1,0)? No (diagonal from 0,0)
  - Try (1,2)? Valid!
...
## 📐 Problem 3: Valid Parentheses Generation
...
| Parentheses |" O(4ⁿ/√n) "| O(n) | Catalan number |
...
✅ **Correct:**
```python
diag1 = row - col  # Positive diagonal: ↗
diag2 = row + col  # Negative diagonal: ↘
```
...
> - **N-Queens:** Track attacked positions with sets → O(1) validity check
> - **Sudoku:** Precompute row/col/box constraints → faster pruning
>
...
- [Core Patterns](./2.3-Core-Patterns/01-Subsets-Pattern.md) - Subsets, Permutations
