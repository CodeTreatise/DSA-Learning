...
> **Interview Value:** ⭐⭐⭐⭐⭐ - Essential optimization technique  
> **Prerequisites:** [1.1 Recursion Basics](../01-Recursion/1.1-Recursion-Basics.md), [1.4 Basic Problems](../01-Recursion/1.4-Basic-Problems/01-Factorial-Fibonacci.md)
...
```
Without Memo: fib(5) → 15 calls, O(2^n)
With Memo:    fib(5) → 9 calls,  O(n)
```

The key insight: **If you've solved a subproblem before, don't solve it again—look it up!**
...
- [Dynamic Programming](../.../11-Dynamic-Programming/11-Dynamic-Programming.md) - Bottom-up approach
- [2D DP](../.../11-Dynamic-Programming/11-Dynamic-Programming.md) - Grid-based problems
- [State Machine DP](../.../11-Dynamic-Programming/11-Dynamic-Programming.md) - Complex state transitions
...
```
1. Check if result is in cache → return cached value
...
def memoized_recursion(n: int, memo: dict = None) -> int:
    ...
    Pattern: Check → Compute → Store → Return
    """
    if memo is None:
        memo = {}
...
- **Without memo:** Each subproblem solved multiple times → exponential
- **With memo:** Each subproblem solved once → polynomial
...
| **Space-optimized** | Only keep needed previous states |" O(n) → O(1) for Fibonacci "|
...
> "I notice this problem has overlapping subproblems—computing the same values multiple times. I'll use memoization to optimize."
...
- "Can you convert this to bottom-up DP?" → Yes, iterate from base case
- "Can you optimize space?" → Often can reduce to O(1) for linear dependencies

**Company Focus:**

| Company | Memoization Frequency |
|---------|----------------------|
| Google | ⭐⭐⭐⭐⭐ Often leads to DP |
| Amazon | ⭐⭐⭐⭐ Standard optimization |
| Meta | ⭐⭐⭐⭐ Bug-free implementation |
| Microsoft | ⭐⭐⭐ Know the pattern |
...
>
> This transforms O(2^n) → O(n) with just 3 lines of code!
