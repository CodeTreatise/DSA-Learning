...
> **Interview Value:** ⭐⭐⭐ - Understanding matters more than implementation  
...
```
Regular recursion:     return n * factorial(n-1)  ← Must wait for result
Tail recursion:        return factorial(n-1, n*acc)  ← Nothing after call
```
...
def tail_func(n, acc):
    return tail_func(n-1, n*acc)  # ✅ Nothing after return

# NOT TAIL CALL - operations after recursive call
def regular_func(n):
    return n * regular_func(n-1)  # ❌ Multiplication happens AFTER
```

**Languages with TCO:**
| Language | TCO Support |
|----------|-------------|
| Scheme/Lisp | ✅ Guaranteed |
| Scala | ✅ With @tailrec |
| JavaScript | ⚠️ Only Safari (ES6 spec, rarely implemented) |
...
- [Iteration Conversion](../.../00-Prerequisites/00-Prerequisites.md) - Manual loop conversion
...
Call Stack:
  factorial(4) → waiting for factorial(3) * 4
    factorial(3) → waiting for factorial(2) * 3
      factorial(2) → waiting for factorial(1) * 2
...
With TCO:
  factorial(4, 1) → factorial(3, 4)    [reuses same frame]
  factorial(3, 4) → factorial(2, 12)   [reuses same frame]
  factorial(2, 12) → factorial(1, 24)  [reuses same frame]
  factorial(1, 24) → returns 24
...
factorial(n):
  if n <= 1: return 1
  return n * factorial(n-1)     ← Compute after

factorial_tail(n, acc=1):
  if n <= 1: return acc         ← Return accumulator
  return factorial_tail(n-1, n*acc)  ← Compute before, pass forward
```
...
def factorial_bad(n):
    if n <= 1:
        return 1
    return n * factorial_bad(n - 1)  # ❌ Multiplication AFTER call
```
...
def factorial(n, acc=1):
    if n <= 1:
        return acc
    return factorial(n - 1, n * acc)  # ✅ Call is the LAST thing
```
...
def sum_tail(arr, acc=0):
    if not arr:
        return 0  # ❌ Throws away accumulated work!
```
...
> - Regular: `return n * f(n-1)` → needs stack to remember n
> - Tail: `return f(n-1, n*acc)` → passes result forward, stack unneeded
>
...
- [Dynamic Programming](../.../11-Dynamic-Programming/11-Dynamic-Programming.md) - Bottom-up iteration
