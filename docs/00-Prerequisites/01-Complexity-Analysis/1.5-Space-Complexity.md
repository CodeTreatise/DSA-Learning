...
1. **Avoid crashes** — Running out of memory kills your program
2. **Write efficient code** — Less memory often means faster execution (cache locality)
3. **Pass interviews** — You'll always be asked about both time AND space
4. **Make trade-offs** — Sometimes you trade space for time, or vice versa
...
```
factorial(5)  → Stack: [f(5)]
factorial(4)  → Stack: [f(5), f(4)]
factorial(3)  → Stack: [f(5), f(4), f(3)]
factorial(2)  → Stack: [f(5), f(4), f(3), f(2)]
factorial(1)  → Stack: [f(5), f(4), f(3), f(2), f(1)]
...
| Binary (2 * T(n/2)) | log n | O(log n) |
| Binary (2 * T(n-1)) | n | O(n) |
...
| Approach | Time | Space | Trade-off |
|----------|------|-------|-----------|
| Naive recursion | O(2ⁿ) | O(n) | Terrible time, moderate space |
...
def example(arr):
    seen = set()        
    for x in arr:
        seen.add(x)     # Adds up to n unique elements → O(n)
    
    result = []
    for x in arr:
        if x > 0:
            result.append(x)  # Could be all elements → O(n)
```
...
def example(arr, i=0):
    if i >= len(arr):
        return 0
    return arr[i] + example(arr, i + 1)  # n calls deep → O(n) stack
```
...
## 📊 Quick Reference: Common Algorithm Space
...
| Graph (Adj Matrix) | varies | O(V²) | V*V matrix |
...
**O(n)** — We create an array of size n.
...
**O(1)** — Only using two pointer variables, modifying in-place.
...
**O(log n)** — The recursion depth is log n. Even though there are 2 calls, they don't exist on the stack at the same time (one completes before the other starts).

</details>

---

> **💡 Key Insight:** Space complexity is often overlooked but crucial for large-scale systems. An O(n) algorithm that uses O(n²) space might be worse than an O(n²) algorithm that uses O(1) space when memory is the bottleneck.

---

**Next:** [1.6 Complexity Cheatsheet](./1.6-Complexity-Cheatsheet.md) →
