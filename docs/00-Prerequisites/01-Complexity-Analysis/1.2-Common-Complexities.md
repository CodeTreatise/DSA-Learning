...
O(n log n) →           10,000
O(n²)      →        1,000,000
O(2ⁿ)      → → → → → → → → → (number with 301 digits!)
O(n!)      →→→→→→→→→→→→→→→→→ (number with 2,568 digits!)
```
...
```
1-100: "Greater than 50?" → Yes
51-100: "Greater than 75?" → No
51-75: "Greater than 63?" → Yes
64-75: "Greater than 69?" → No
64-69: "Greater than 66?" → Yes
67-69: "Greater than 68?" → No
67-68: "Is it 68?" → Yes!
...
Level 0:    [5, 2, 8, 1, 9, 3, 7, 4]        ← n elements
             /                    \
Level 1: [5,2,8,1]           [9,3,7,4]      ← n/2 + n/2 = n work
           /    \               /    \
Level 2: [5,2] [8,1]       [9,3] [7,4]      ← n/4 * 4 = n work
          / \   / \         / \   / \
Level 3: [5][2][8][1]     [9][3][7][4]      ← n/8 * 8 = n work
...
**Everyone shaking hands at a party.** With n people, there are n * (n-1) / 2 ≈ n² handshakes.
...
| Input Size | O(n²) Operations | Acceptable? |
|------------|------------------|-------------|
| n ≤ 100 | 10,000 | ✅ Yes |
| n ≤ 1,000 | 1,000,000 | ⚠️ Maybe |
| n ≤ 10,000 | 100,000,000 | ❌ Too slow |
| n ≥ 100,000 | 10,000,000,000+ | ❌ Unusable |

**Rule of thumb:** Most computers handle ~10⁸ operations per second. For interview problems with n ≤ 10⁴, O(n²) might pass.

### Optimizing O(n²) to O(n)

```python
# O(n²) - Check all pairs for duplicate
...
**Deciding what to wear.** With n items of clothing, you have 2ⁿ possible outfit combinations (each item is either worn or not).
...
Only when n is very small (typically n ≤ 20-25) or when you're generating all possible combinations and there's no better approach.

| Problem Type | Acceptable n |
|--------------|--------------|
| Subset generation | n ≤ 20 |
| Brute force password | Very short passwords only |
| Traveling Salesman (exact) | n ≤ 15-20 |
...
**Rule:** If your algorithm is O(n!), it's only usable for n ≤ 10-12.
...
| If n ≤ | These complexities work | Example problems |
|--------|------------------------|------------------|
| 10 | O(n!), O(2ⁿ), O(n⁴) | Brute force anything |
| 20 | O(2ⁿ), O(n³) | Subset generation |
| 100 | O(n³) | Floyd-Warshall |
| 500 | O(n³) | Small matrix operations |
| 5,000 | O(n²) | Bubble sort (just barely) |
| 10⁶ | O(n log n) | Sorting large arrays |
| 10⁸ | O(n) | Single-pass algorithms |
| 10⁹+ | O(log n), O(1) | Binary search, hash lookup |

---

> **💡 Key Insight:** The difference between O(n) and O(n²) isn't just "a bit slower"—at scale, it's the difference between 1 second and 3 hours. Understanding complexity classes helps you immediately know whether an approach is viable before writing a single line of code.

---

**Next:** [1.3 Analyzing Loops](./1.3-Analyzing-Loops.md) →
