...
>
> ⏱️ *Reference document — use as needed*
...
| **O(log n)** | Logarithmic | 3 | 7 | 10 | 13 | ✅ Excellent |
| **O(n)** | Linear | 10 | 100 | 1,000 | 10,000 | ✅ Good |
| **O(n log n)** | Linearithmic | 33 | 664 | 9,966 | 132,877 | ✅ Good |
| **O(n²)** | Quadratic | 100 | 10,000 | 1,000,000 | 100,000,000 | ⚠️ Careful |
| **O(n³)** | Cubic | 1,000 | 1,000,000 | 10⁹ | 10¹² | ❌ Avoid |
| **O(2ⁿ)** | Exponential | 1,024 | 10³⁰ | 10³⁰¹ | ∞ | ❌ Avoid |
| **O(n!)** | Factorial | 3.6M | 10¹⁵⁸ | ∞ | ∞ | ❌ Avoid |
...
- Quick Sort is often fastest in practice despite O(n²) worst case
...
| **Union-Find** | O(α(n)) ≈ O(1) | O(n) | Disjoint sets |

---

## 📝 Common Patterns

### Loop Patterns → Complexity

| Pattern | Complexity | Example |
|---------|------------|---------|
| Single loop | O(n) | `for i in range(n)` |
| Two sequential loops | O(n) | `for x in a; for x in a` |
| Nested loops | O(n²) | `for i in n: for j in n` |
| Nested with different arrays | O(n * m) | `for x in a: for y in b` |
...
| Exponential branching | T(n) = 2T(n-1) + O(1) | O(2ⁿ) |
...
|----------------|----------------|-------------------|
| n ≤ 10 | O(n!), O(2ⁿ) | Brute force, permutations |
| n ≤ 20 | O(2ⁿ) | Subset generation, bitmask DP |
| n ≤ 100 | O(n³) | Floyd-Warshall |
| n ≤ 500 | O(n³) | Matrix operations |
| n ≤ 5,000 | O(n²) | Simple DP, bubble sort |
| n ≤ 10⁵ | O(n log n) | Sorting, balanced BST |
| n ≤ 10⁶ | O(n) | Linear scan, two pointers |
| n ≤ 10⁸ | O(n) | Linear scan (tight) |
| n ≤ 10⁹ | O(log n), O(1) | Binary search, math |
| n > 10⁹ | O(log n), O(1) | Math formulas only |
...
log(a/b) = log(a) - log(b)
log(a^n) = n * log(a)
log_b(x) = log_c(x) / log_c(b)    ← All log bases are O(log n)
```
...
- ❌ "It's O(n) because there's one loop" (check what's inside!)
- ❌ "It's O(n²) because there are two loops" (are they nested or sequential?)
- ❌ "It's O(1) space" (did you forget the recursion stack?)
...
│                    BIG O CHEATSHEET                             │
├─────────────────────────────────────────────────────────────────┤
│ COMPLEXITY RANKING (fastest → slowest):                        │
...
> **💡 Key Insight:** This cheatsheet is your quick reference, but understanding WHY these complexities occur is what impresses interviewers. Know the formulas, but also know the intuition.

---

**Next:** [1.7 Practice Problems](./1.7-Practice-Problems.md) →
