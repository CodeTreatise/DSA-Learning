...
>
> ⏱️ *Estimated reading: 30 minutes*

---

## 🎯 Why Recursion Complexity Is Different

Loops are straightforward: count iterations * work per iteration.
...
**Tree:**
```
countdown(5)     ← 1 unit of work
    │
countdown(4)     ← 1 unit of work
    │
countdown(3)     ← 1 unit of work
    │
countdown(2)     ← 1 unit of work
    │
countdown(1)     ← 1 unit of work
    │
...
- Tree height: ~n (actually Θ(n))
- Calls at level k: ~2^k
- Total calls: 1 + 2 + 4 + ... + 2^n ≈ **O(2ⁿ)**
...
**Analysis:**
- Tree height: log₂(n) (we halve each time)
...
**Tree:**
```
binary_search(n)     ← O(1) work
        │
binary_search(n/2)   ← O(1) work
        │
binary_search(n/4)   ← O(1) work
        │
       ...
        │
binary_search(1)     ← O(1) work
```
...
1. **Identify the number of recursive calls** → multiplier
2. **Identify the subproblem size** → what n becomes
3. **Identify the work done outside recursion** → the "+ O(f(n))" part
...
## 📋 Quick Reference: Common Recursive Algorithms
...
- Master Theorem: a=2, b=2, d=0 → 2 > 2⁰ → Case 3
- O(n^(log₂2)) = **O(n)**
...
T(n)      ← O(n) partition
  │
T(n-1)    ← O(n-1) partition
  │
T(n-2)    ← O(n-2) partition
  ...
...
**Space complexity:** O(n) — n stack frames simultaneously
...
## 📊 Decision Tree: Analyzing Recursion
...
# Time: O(n) ✓
# Space: O(n) ✗ (don't forget the call stack!)
```
...
> **💡 Key Insight:** For recursion, always draw the tree first. It makes the pattern visible: Is work concentrated at the root? Spread evenly? Or concentrated at the leaves?

---

**Next:** [1.5 Space Complexity](./1.5-Space-Complexity.md) →
