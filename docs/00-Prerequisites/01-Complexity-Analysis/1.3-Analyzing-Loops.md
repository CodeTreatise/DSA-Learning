...
>
> ⏱️ *Estimated reading: 25 minutes*

---

## 🎯 The Universal Loop Formula

Most loop complexity follows this pattern:

```
Total Operations = (Number of Iterations) * (Work per Iteration)
```
...
### Pattern 2: Half Loop — Still O(n)

```python
def half_loop(arr):
    for i in range(len(arr) // 2):  # n/2 iterations
        print(arr[i])
    # Total: n/2 * O(1) = O(n/2) = O(n)
```

**Remember:** We drop constants! O(n/2) = O(n)

### Pattern 3: Loop with Step — Still O(n)
...
### Pattern 4: Loop Dividing by 2 — O(log n)
...
**Trace:**
```
n = 16:  16 → 8 → 4 → 2 → 1 → 0 (done)
         1    2   3   4   5 = 5 iterations

n = 32:  32 → 16 → 8 → 4 → 2 → 1 → 0 (done)
         1     2   3   4   5   6 = 6 iterations

Pattern: log₂(n) + 1 iterations = O(log n)
```

### Pattern 5: Loop Multiplying by 2 — O(log n)
...
**Trace:**
```
n = 16:  i = 1 → 2 → 4 → 8 → 16 (stop, 16 ≮ 16)
         4 iterations = log₂(16) = O(log n)
```

### Pattern 6: Loop with Non-Constant Step — O(√n)
...
def sqrt_loop(n):
    ...
    while i * i < n:
        print(i)
        i += 1
    # When i² = n, i = √n, so √n iterations
```

**Trace:**
```
n = 16:  i = 0 → 1 → 2 → 3 → 4 (stop, 4² = 16)
         4 iterations = √16 = O(√n)
```
...
### Pattern 7: Simple Nested Loops — O(n²)
...
**Visualization:**
```
i=0: j runs 0,1,2,...,n-1  → n operations
i=1: j runs 0,1,2,...,n-1  → n operations
i=2: j runs 0,1,2,...,n-1  → n operations
...
i=n-1: j runs 0,1,2,...,n-1  → n operations

Total: n rows * n operations each = n²
```

### Pattern 8: Dependent Inner Loop — O(n²)
...
**Trace:**
```
i=0: j runs 0 times       → 0 operations
i=1: j runs 0             → 1 operation
i=2: j runs 0,1           → 2 operations
i=3: j runs 0,1,2         → 3 operations
...
i=n-1: j runs 0,1,...,n-2 → n-1 operations

Total: 0 + 1 + 2 + ... + (n-1) = n(n-1)/2 = (n² - n)/2 = O(n²)
```

**The sum formula:** 1 + 2 + 3 + ... + n = n(n+1)/2 ≈ n²/2 = O(n²)

### Pattern 9: Two Independent Inputs — O(n * m)
...
def two_arrays(arr1, arr2):
    for x in arr1:          # n iterations
        for y in arr2:      # m iterations
            print(x, y)
    # Total: n * m = O(n * m), NOT O(n²)!
```

**Common mistake:** Assuming O(n²) when inputs are different.

### Pattern 10: Inner Loop with Constant — O(n)
...
### Pattern 11: Outer + Inner Dividing — O(n log n)
...
### Pattern 12: Three Nested Loops — O(n³)
...
### Pattern 20: Amortized O(1) Inside Loop — Still O(n)
...
## 🧮 Mathematical Summation Formulas
...
## ⚡ Quick Reference: Common Loop Complexities
...
| Dependent inner loop | `for i: for j in range(i):` | O(n²) |
| Different arrays | `for x in a: for y in b:` | O(n * m) |
| Outer * log n inner | `for i: while j //= 2:` | O(n log n) |
...
def looks_quadratic(n):
    ...
    # log n outer iterations * log n inner = O(log² n)
```
...
- Outer loop: n iterations
- Inner loop: j triples each time → log₃(n) iterations
- Total: n * log₃(n) = O(n log n)

(Remember: All log bases differ by a constant factor, so log₃(n) = O(log n))
...
> **💡 Key Insight:** Loop analysis is pattern matching. Once you recognize the 10-15 common patterns, you can analyze any loop structure by breaking it down into known components.

---

**Next:** [1.4 Analyzing Recursion](./1.4-Analyzing-Recursion.md) →
