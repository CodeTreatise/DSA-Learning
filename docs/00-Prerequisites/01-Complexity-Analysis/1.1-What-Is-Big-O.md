...
| **O(n²)** - Quadratic | 100x longer | Nested loops |
| **O(2ⁿ)** - Exponential | Impossibly longer | Trying all combinations |
...
You walk directly to that exact spot. Doesn't matter if the venue has 100 or 100,000 seats—you go straight there.
...
**Time: Doubles venue = 1 more step** ✅
...
**Time: 10x more people = 10x longer** ⚠️

### Scenario 4: Asking Everyone About Everyone (O(n²) - Quadratic)
...
With 100 people: 100 * 100 = 10,000 interactions
With 1,000 people: 1,000 * 1,000 = 1,000,000 interactions

**Time: 10x more people = 100x longer** ❌
...
When n is large (n = 1000):
  3(1,000,000) + 5(1000) + 100 = 3,000,000 + 5,000 + 100 ≈ 3,000,000
  The n² term dominates!
```

**That's why we only care about the highest-order term: O(n²)**

---

## 🧮 The Rules of Big O (Simplified)
...
O(n)       10          100         1000
O(2n)      20          200         2000     ← Same growth pattern!
O(100n)   1000       10000       100000     ← Still linear!
```
...
## 📊 Big O, Big Ω, Big Θ (The Full Picture)
...
| **Ω** (Big Omega) | Lower Bound | "At least this fast" | Minimum speed |
| **Θ** (Big Theta) | Tight Bound | "Exactly this fast" | Average speed |
...
- Upper bound is O(n)
- In practice, it performs at Θ(n)

Don't worry about Ω and Θ for interviews—focus on Big O.

---

## ⏱️ Best, Average, Worst Cases
...
| **Average** | Target is somewhere in middle | O(n/2) → O(n) |
...
## 💻 Code Examples: Identifying Big O
...
def print_pairs(arr):
    ...
    for i in range(len(arr)):          # n iterations
        for j in range(len(arr)):      # n iterations for EACH i
            print(arr[i], arr[j])      # n * n = n² operations
...
**Why O(n²)?** For every element, we look at every other element. Nested loops often (but not always!) indicate O(n²).
...
❌ **Wrong thinking:** "This has 3 operations, that has 5, so 3 is better."

✅ **Right thinking:** "Both are O(1), so they're equivalent in Big O terms."

### Mistake 2: Assuming Nested Loops Are Always O(n²)
...
❌ **Wrong:** "Two nested loops, must be O(n²)"

✅ **Right:** Inner loop is constant (5), so it's O(5n) = O(n)
...
> "My solution is O(n) time and O(1) space. Time is linear because we visit each element once with the two-pointer technique—left pointer starts at 0, right at n-1, and they meet in the middle after at most n steps. Space is constant because we only use two pointer variables regardless of input size."
...
| O(n²) | Quadratic | Bubble sort | 100x time |
| O(2ⁿ) | Exponential | All subsets | 1000x time |
...
> **💡 Key Insight:** Big O isn't about exact operation counts—it's about understanding how your algorithm **scales**. An O(n) algorithm will always eventually beat O(n²), no matter how optimized the O(n²) code is.

---

**Next:** [1.2 Common Complexities](./1.2-Common-Complexities.md) →
