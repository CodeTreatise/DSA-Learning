...
> **📌 Note:** This file covers **Hash Map fundamentals** - how hashing works, collision handling, and language-specific APIs. For **Hash Map as an algorithm technique** (Two Sum pattern, frequency counting, complement lookup), see [05 Hash Map Technique](../01-Arrays/1.4-Common-Techniques/05-Hash-Map-Technique.md).
...
> 💡 *When Two Pointers requires sorted input, Hash Map is often the alternative for unsorted data.*
...
- [Time Complexity](../.../00-Prerequisites/00-Prerequisites.md) - Big O understanding
...
│ 2 │ "apple" → 5   │  ← First key
├───┼───────────────┤
│ 3 │ "banana" → 3  │  ← Collision, moved to next slot
├───┼───────────────┤
│ 4 │               │
└───┴───────────────┘
```
...
**Why Hash Map?**
- Array is unsorted → Two Pointers won't work
- Need O(1) lookup for complement
- Trade space for time: O(n) space → O(n) time
...
> **💡 Key Insight:** Hash Maps trade space for time. When you need O(1) lookup and can afford O(n) space, Hash Map is usually the answer. Two Sum unsorted? Hash Map. Frequency counting? Hash Map. Finding duplicates? Hash Set.

> **🔗 Related:** [Two Pointers](../01-Arrays/1.4-Common-Techniques/01-Two-Pointers.md) (sorted alternative) | [Prefix Sum](../01-Arrays/1.4-Common-Techniques/03-Prefix-Sum.md) (combines with Hash Map)
