...
> 💡 *String problems are essentially array problems with characters. Most array techniques (Two Pointers, Sliding Window) apply directly to strings.*
...
string s = "hello";
s[0] = 'H';  // ✅ "Hello" - modifies in place

// C - character arrays
char s[] = "hello";
s[0] = 'H';  // ✅ Modifies in place
```
...
|--------|-------------|---------|
| `s.upper()` | All uppercase | `"hello" → "HELLO"` |
| `s.lower()` | All lowercase | `"HELLO" → "hello"` |
| `s.strip()` | Remove whitespace | `" hi " → "hi"` |
| `s.split(delim)` | Split to list | `"a,b" → ["a", "b"]` |
| `s.join(list)` | Join list to string | `["a","b"] → "a,b"` |
| `s.replace(old, new)` | Replace substring | `"hello" → "jello"` |
| `s.find(sub)` | Find index | `"hello".find("l") → 2` |
| `s.count(sub)` | Count occurrences | `"hello".count("l") → 2` |
| `s.startswith(pre)` | Check prefix | `"hello".startswith("he") → True` |
| `s.endswith(suf)` | Check suffix | `"hello".endswith("lo") → True` |
| `s.isalpha()` | All letters? | `"abc".isalpha() → True` |
| `s.isdigit()` | All digits? | `"123".isdigit() → True` |
| `s.isalnum()` | Alphanumeric? | `"a1".isalnum() → True` |

### JavaScript

| Method | Description | Example |
|--------|-------------|---------|
| `s.toUpperCase()` | All uppercase | `"hello" → "HELLO"` |
| `s.toLowerCase()` | All lowercase | `"HELLO" → "hello"` |
| `s.trim()` | Remove whitespace | `" hi " → "hi"` |
| `s.split(delim)` | Split to array | `"a,b" → ["a", "b"]` |
| `arr.join(delim)` | Join array | `["a","b"] → "a,b"` |
| `s.replace(old, new)` | Replace first | `"hello" → "jello"` |
| `s.replaceAll(old, new)` | Replace all | All occurrences |
| `s.indexOf(sub)` | Find index | `"hello".indexOf("l") → 2` |
| `s.includes(sub)` | Contains? | `"hello".includes("ell") → true` |
| `s.startsWith(pre)` | Check prefix | `"hello".startsWith("he") → true` |
| `s.endsWith(suf)` | Check suffix | `"hello".endsWith("lo") → true` |
| `s.charCodeAt(i)` | Char code at index | `"A".charCodeAt(0) → 65` |
...
> **💡 Key Insight:** Strings in most languages are immutable arrays of characters. Use the same techniques (Two Pointers, Sliding Window) but be mindful of creating new strings in loops - use `''.join()` or `StringBuilder` for efficient concatenation.
