...
>
> ⏱️ Estimated Learning Time: 45-60 minutes
...
```
Before: [node] ⇄ [next_node]
After:  [node] ⇄ [new] ⇄ [next_node]
...
# ✅ Correct - update all 4 connections
...
def bad_delete(self, node):
    ...
    node.next.prev = node.prev  # ✓ But what if we did this first?
...
```python
# ❌ Without sentinels - many null checks
...
# ✅ With sentinels - no special cases
...
- [ ] [LRU Cache](https://leetcode.com/problems/lru-cache/) - LC 146 ⭐⭐⭐ Classic!
...
**When to mention DLL:**
- "I need O(1) deletion with a node reference → DLL"
- "I need to move nodes frequently → DLL with hash map"
- "I need bidirectional traversal → DLL"
...
**Common follow-ups:**
- "Why not just use an array?" → O(n) to move elements
- "Why DLL over singly?" → Need O(1) remove without predecessor
...
|---------|---------|-----------|
| Amazon | LRU Cache | ⭐⭐⭐⭐ Very High |
| Meta | LRU Cache | ⭐⭐⭐⭐ Very High |
| Google | LRU/LFU Cache | ⭐⭐⭐ High |
| Microsoft | LRU Cache | ⭐⭐⭐ High |
...
> **💡 Key Insight:** The power of DLL comes from having access to both neighbors. Combined with a hash map for O(1) node lookup, you get O(1) for all operations - this is exactly what LRU Cache needs!
