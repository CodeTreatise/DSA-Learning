...
```
Input:  1 → 2 → 3 → 4 → null, n = 2 (0-indexed)
Output: Node(3)
```
...
- "Can you do it in one pass?" → Two pointers
- "What if the list has a cycle?" → Fast/slow handles it
- "What about memory usage?" → Iterative vs recursive
...
> **💡 Key Insight:** The two-pointer technique (fast/slow or gap technique) converts many two-pass problems into one-pass solutions. For middle: fast goes 2x speed. For Nth from end: create N gap between pointers.
