...
```python
# ❌ Wrong - stops one position early
for i in range(position):  # Should be position - 1
    current = current.next

# ✅ Correct - traverse to position - 1
...
- "Given a node (not head), delete it" → LC 237
- "Remove nth from end in one pass" → LC 19
- "Remove all occurrences of X" → LC 203

**Questions about trade-offs:**
- "Why is delete from end O(n)?" → Need to find previous node
- "How would you make insert at end O(1)?" → Maintain tail pointer
- "When would you use LL over array?" → Frequent insertions/deletions at front
...
> **💡 Key Insight:** The order of pointer updates is CRITICAL in linked list operations. Always save the "next" reference before breaking any links. When inserting, point the new node to its successor before updating the predecessor's pointer.
