...
def reverse_list_recursive_verbose(head: ListNode) -> ListNode:
    ...
    #   head = 1
    #   head.next = 2
    #   After recursion: 4 → 3 → 2 → null (new_head = 4)
    #   We need: 4 → 3 → 2 → 1 → null
    
    new_head = reverse_list_recursive_verbose(head.next)
    
    # head.next is 2, and 2.next should point to 1
    head.next.next = head  # 2 → 1
    head.next = None       # 1 → null (prevent 1 ↔ 2 cycle)
...
|---------|-----------|------------------|
| Amazon | ⭐⭐⭐⭐⭐ | Basic reversal, k-group |
| Meta | ⭐⭐⭐⭐⭐ | All variations |
| Google | ⭐⭐⭐⭐ | May ask for recursive |
| Microsoft | ⭐⭐⭐⭐ | Standard + edge cases |
...
> **💡 Key Insight:** The order of operations matters! Always (1) save next, (2) reverse link, (3) move prev, (4) move current. If you forget step 1, you lose the rest of the list. Drawing the pointers before coding helps prevent mistakes.
