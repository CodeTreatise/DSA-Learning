...
Any function `f(x) → y` where you repeatedly apply it creates an implicit linked list. If the domain is finite, it MUST eventually cycle!
...
Result: Never reaches 1 → NOT HAPPY ✗
```
...
Node: 2  → 4  → 16 → 37 → 58 → 89 → 145 → 42 → 20 → 4
...
If fast meets slow → cycle → not happy
```
...
```python
# ❌ Wrong - string conversion is slower
def get_next_slow(n):
...
```python
# ❌ Wrong - both start at same position but should differ initially
slow = n
fast = n
while slow != fast:  # Immediately true!

# ✅ Correct - start fast one step ahead OR check condition differently
slow = n
fast = get_next(n)
while fast != 1 and slow != fast:
```
...
```python
# ❌ Wrong - only checking for cycle
while slow != fast:
    slow = get_next(slow)
    fast = get_next(get_next(fast))
return False  # What if fast == 1?

# ✅ Correct - check for both conditions
while fast != 1 and slow != fast:
    slow = get_next(slow)
    fast = get_next(get_next(fast))
return fast == 1
...
```javascript
// ❌ Wrong in JavaScript - floating point division
...
- **Day 30:** Review the insight: any bounded sequence → cycle detection
...
**Follow-up questions:**
- "Why O(1) space?" → Fast/Slow only needs two variables
- "Why does it terminate?" → Bounded sequence must cycle
- "What's the time complexity?" → O(log n) per step, bounded steps
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Tests creative thinking |
| Apple | ⭐⭐⭐ | Number manipulation focus |
| Uber | ⭐⭐⭐ | Pattern recognition |
...
> **💡 Key Insight:** Any deterministic sequence in a bounded range MUST eventually cycle (Pigeonhole Principle). This means we can apply Fast & Slow pointer to detect cycles in mathematical sequences, not just linked lists. The "linked list" is implicit - each value "points to" its successor.
