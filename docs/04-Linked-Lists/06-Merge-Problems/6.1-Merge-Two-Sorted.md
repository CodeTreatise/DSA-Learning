...
**Common follow-ups:**
- "What if lists have different lengths?" → Handled by final `current.next = list1 or list2`
- "Can you do it recursively?" → Yes, but O(n+m) stack space
- "What about K lists?" → Use a min-heap (leads to LC 23)
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common, warm-up question |
| Meta | ⭐⭐⭐⭐ | May ask as part of larger problem |
| Google | ⭐⭐⭐ | Often leads to Merge K |
...
> **💡 Key Insight:** The dummy head pattern eliminates edge cases for the first node. Always consider using it when building a new list node by node.
