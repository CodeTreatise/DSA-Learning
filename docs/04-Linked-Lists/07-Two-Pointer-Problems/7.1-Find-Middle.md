...
# fast = head, fast.next = null → loop doesn't run
# slow stays at head → correct!
```
...
**Common follow-ups:**
- "Can you do it in one pass?" → This IS one pass
- "What if length is known?" → Just traverse to n/2
- "First middle instead?" → Adjust termination condition
...
| Google | ⭐⭐⭐ | May combine with other operations |
...
> **💡 Key Insight:** Fast & Slow works because of the 2:1 speed ratio. When fast completes the full distance, slow has covered exactly half. This principle extends to other problems like cycle detection.
