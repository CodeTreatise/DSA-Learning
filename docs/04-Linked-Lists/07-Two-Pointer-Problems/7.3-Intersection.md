...
## 📐 Approach 1: Two Pointers (Elegant Solution)
...
```
A: a1 → a2 → c1 → c2 → c3 (length 5: 2 unique + 3 common)
B: b1 → b2 → b3 → c1 → c2 → c3 (length 6: 3 unique + 3 common)

Pointer A path: a1→a2→c1→c2→c3→(jump to B)→b1→b2→b3→c1 ← MEET!
Pointer B path: b1→b2→b3→c1→c2→c3→(jump to A)→a1→a2→c1 ← MEET!
...
## 📐 Approach 3: Hash Set (Simple but O(n) Space)
...
```python
# ❌ Wrong - same values don't mean same node!
...
```python
# ❌ Wrong - infinite loop if no intersection
while pA != pB:
    pA = pA.next  # Eventually null, but never equals pB
    pB = pB.next

# ✅ Correct - redirect to other list's head when reaching null
while pA != pB:
...
# A: 1 → 2 → 3
# B: 4 → 5 → 3  ← This 3 must be the SAME node as A's 3

# NOT the same as:
# A: 1 → 2 → 3
# B: 4 → 5 → 3  ← Different node that happens to have value 3
```
...
**Common follow-ups:**
- "Why not use hash set?" → Works, but uses O(n) space
- "Can you modify the lists?" → Shouldn't, but could connect tail to head and detect cycle
...
> **💡 Key Insight:** By having each pointer traverse both lists (A→B and B→A), they travel the same total distance. If there's an intersection, they'll meet there. This is one of the most elegant linked list solutions!
