...
1 == 1 ✓
2 == 2 ✓
3 == 3 ✓ (middle, compare with itself or skip)
Result: true
```
...
# We reverse from slow.next (2 → 1 becomes 1 → 2)
...
**Common follow-ups:**
- "Can you do it without modifying?" → Yes, with stack (O(n/2) space)
- "What about restoring the list?" → Reverse second half again
- "Why this middle finding method?" → Need first middle for correct split
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Meta | ⭐⭐⭐⭐ |" O(1) space expected "|
| Google | ⭐⭐⭐⭐ | May ask about restoration |
...
> **💡 Key Insight:** This problem elegantly combines three patterns: Fast & Slow (find middle), In-Place Reversal (reverse half), and Two Pointers (compare). Recognizing that you need to combine multiple patterns is a key interview skill.
