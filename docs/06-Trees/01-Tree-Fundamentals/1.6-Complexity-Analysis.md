...
## 🔄 Optimizing Tree Solutions

### Hash Map Optimization

```python
# O(n²) - Linear search in each call
...
## ⚠️ Common Complexity Mistakes

### 1. Forgetting About Tree Shape

```python
# ❌ WRONG: "DFS is always O(log n) space"
# Space depends on tree HEIGHT, not just the algorithm

# ✅ CORRECT: "DFS is O(h) space, which is O(log n) for balanced 
...
# ✅ Actually O(n) with list approach
def serialize(node):
    result = []
    def dfs(node):
        if not node:
...
├─────────────────────────────────────────────────────┤
│ Common mistake:                                     │
│   Don't say O(log n) for all trees!                 │
│   Say O(h) and clarify based on tree type.          │
└─────────────────────────────────────────────────────┘
```
...
> **💡 Key Insight:** For most tree problems, time is O(n) because you visit each node. The interesting discussion is usually about space: O(h) for DFS vs O(w) for BFS, and how h varies based on tree balance. Always clarify what h could be in different scenarios.
