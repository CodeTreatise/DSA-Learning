...
Looking for LCA(5, 1):
        ...
        3          ← Both children returned non-null: LCA!
       / \
...
Looking for LCA(5, 4):
        ...
        3          ← Only left returned (5), propagate up
       / \
      5   1        ← Found 5, AND right subtree found 4
...
├── left AND right found something? → Current is LCA
├── left found something? → Propagate left result
├── right found something? → Propagate right result
...
# ❌ WRONG thinking: "return p" means "p is the answer"
# ✅ CORRECT: "return p" means "I found p, let parent decide"
```
...
```python
# ❌ WRONG: Not handling when root is p or q
if not root:
    return None
...
# ✅ CORRECT: Check if root is p or q first
if not root or root == p or root == q:
    return root
```
...
> **💡 Key Insight:** The beauty of this solution is the meaning of the return value: it's not "this is the LCA" but rather "I found something here." The LCA is determined by the first node where both children return non-null, or where the node itself is one of the targets and a child returns the other.
