...
col -2: [4]
col -1: [2]
col  0: [1, 5, 6]  ← 5 and 6 at same row, sorted by value
col +1: [3]
col +2: [7]
...
```python
# ❌ WRONG: Both children same column
if node.left:
...
```python
# ❌ WRONG: Only storing column and value
nodes.append((col, node.val))
# Can't sort by row within column!

# ✅ CORRECT: Store all three
nodes.append((col, row, node.val))
```
...
```python
# ❌ WRONG: Not sorting columns
for col in column_map:  # Hash map order is arbitrary!

# ✅ CORRECT: Iterate in column order
for col in range(min_col, max_col + 1):
