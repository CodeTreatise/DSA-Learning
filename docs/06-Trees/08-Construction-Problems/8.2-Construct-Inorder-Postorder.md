...
**Explanation:**
- Inorder: [9, 3, 15, 20, 7] → Left subtree, root, right subtree
- Postorder: [9, 15, 7, 20, 3] → Left subtree, right subtree, root (root is LAST)
...
**Steps:**
1. Take **last** element of postorder → this is the root
...
# ✅ CORRECT: Right first, then left (reverse of postorder)
root.right = build(mid + 1, right)
root.left = build(left, mid - 1)
```
...
```python
# ❌ WRONG: Moving forward like preorder
postorder_idx[0] += 1

# ✅ CORRECT: Moving backwards from end
postorder_idx[0] -= 1
```
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Common |
| Google | ⭐⭐⭐⭐ | Asked with LC 105 |
| Meta | ⭐⭐⭐ | Sometimes appears |
...
> **💡 Key Insight:** The key difference from preorder+inorder is that we read postorder backwards (getting Root-Right-Left order), so we must build the right subtree before the left. This ensures we consume postorder elements in the correct order.
