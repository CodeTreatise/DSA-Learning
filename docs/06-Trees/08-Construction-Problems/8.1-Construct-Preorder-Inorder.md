...
**Explanation:**
- Preorder: [3, 9, 20, 15, 7] → Root first, then left subtree, then right subtree
- Inorder: [9, 3, 15, 20, 7] → Left subtree, then root, then right subtree
...
Find root in inorder → everything left of it is left subtree, everything right is right subtree.
...
**Steps:**
1. Take first element of preorder → this is the root
...
Step 3: Right subtree
        ...
        Left of 20: [15] → Node 15
        Right of 20: [7] → Node 7
...
```python
# ❌ WRONG: Building right before left
root.right = build(mid + 1, right)  # Wrong! Consumes wrong preorder elements
root.left = build(left, mid - 1)

# ✅ CORRECT: Left first, then right (matches preorder order)
root.left = build(left, mid - 1)
root.right = build(mid + 1, right)
```
...
```python
# ❌ WRONG: Including root in left subtree
root.left = build_tree(preorder[1:mid+2], inorder[:mid+1])

# ✅ CORRECT: Root is at index mid, exclude it
...
```python
# ❌ SLOW: Linear search each time
mid = inorder.index(root_val)  # O(n) per call → O(n²) total

# ✅ FAST: Build hash map once
inorder_map = {val: idx for idx, val in enumerate(inorder)}
mid = inorder_map[root_val]  # O(1) per call → O(n) total
```
...
> **💡 Key Insight:** Preorder gives us the root (first element), while inorder lets us determine which nodes belong to left vs right subtrees. This combination uniquely determines the tree structure. Always build the left subtree first to maintain correct preorder consumption.
