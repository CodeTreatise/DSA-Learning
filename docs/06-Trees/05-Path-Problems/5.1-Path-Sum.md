...
```
              5
             /
            4       ← NOT a leaf (has child)
           /
          2         ← IS a leaf (no children)
...
```python
# ❌ WRONG: Checking remaining at any node
def has_path_sum(root, target_sum):
    if not root:
        return target_sum == 0  # Wrong! Null is not a leaf

# ✅ CORRECT: Only check at actual leaves
def has_path_sum(root, target_sum):
    if not root:
        return False
...
```python
# ❌ WRONG: Checking before subtracting current value
def has_path_sum(root, target_sum):
    if not root.left and not root.right:
        return target_sum == 0  # Didn't subtract root.val!

# ✅ CORRECT: Subtract first, then check
remaining = target_sum - root.val
if not root.left and not root.right:
    return remaining == 0
```
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Meta | ⭐⭐⭐⭐ | Often leads to Path Sum II |
...
> **💡 Key Insight:** The critical part is checking at LEAF nodes only. A null node is NOT a leaf. A node with no children IS a leaf. Subtracting from target is cleaner than accumulating sum - at each leaf, just check if remaining equals 0.
