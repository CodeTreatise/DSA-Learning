...
At 5 (leaf): current_num = 49*10 + 5 = 495 → add to sum
At 1 (leaf): current_num = 49*10 + 1 = 491 → add to sum
At 0 (leaf): current_num = 40 → add to sum
...
# ✅ CORRECT: Only add at leaf nodes
def dfs(node, current_num):
    if not node:
        return 0
    current_num = current_num * 10 + node.val
...
```python
# ❌ WRONG: Only checking both None
if node.left is None and node.right is None:
    return current_num
# Then calling dfs on None child

# ✅ CORRECT: Check None at start
if not node:
    return 0
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐ | Common |
| Meta | ⭐⭐⭐ | Standard question |
| Microsoft | ⭐⭐ | Occasionally asked |
...
> **💡 Key Insight:** Building a number as you traverse is as simple as `current * 10 + digit`. This same technique works for binary numbers (`current * 2 + bit`) or any base. The key is only collecting the result at leaf nodes.
