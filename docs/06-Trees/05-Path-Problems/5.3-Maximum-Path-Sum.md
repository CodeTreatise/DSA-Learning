...
>
> **Difficulty:** Hard | **Frequency:** ⭐⭐⭐⭐⭐ Very Common (Famous!)
...
```
      ...
      9    20
          /  \            path_through(20) = 15 + 20 + 7 = 42 ← max!
...
```python
# ❌ WRONG: Always including branches
...
# ✅ CORRECT: Ignore negative branches
left_gain = max(0, max_gain(node.left))
right_gain = max(0, max_gain(node.right))
```
...
# ✅ CORRECT: Start with negative infinity
max_sum = float('-inf')
```
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Meta | ⭐⭐⭐⭐⭐ | Favorite hard problem |
| Google | ⭐⭐⭐⭐⭐ | Classic interview |
| Microsoft | ⭐⭐⭐⭐ | Often asked |
...
> **💡 Key Insight:** The core insight is distinguishing between "path through this node" (can use both branches, for global answer) and "gain to parent" (can only use one branch, for continuing the path). This pattern - tracking a global maximum while returning something different - appears in many tree problems.
