...
1. node2_left.val (2) == node2_right.val (2) ✓
2. mirror(3, 3) - outer pair → 3.val == 3.val ✓
3. mirror(4, 4) - inner pair → 4.val == 4.val ✓
4. All leaf children are null pairs ✓
```
...
```python
# ❌ WRONG: Same comparison as Same Tree
...
```python
# ❌ WRONG: Comparing root with itself
...
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Meta | ⭐⭐⭐⭐⭐ | Often asked |
| Google | ⭐⭐⭐⭐ | May ask iterative |
| Microsoft | ⭐⭐⭐⭐ | Classic interview |
| Apple | ⭐⭐⭐⭐ | Standard question |
...
> **💡 Key Insight:** Symmetric Tree is Same Tree with swapped comparisons. Instead of comparing `(left.left, right.left)`, compare `(left.left, right.right)`. This "mirror swap" pattern is useful whenever you need to check for reflection symmetry.
