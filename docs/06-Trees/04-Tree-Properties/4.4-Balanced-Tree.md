...
1. **Naive:** Calculate height separately for each node → O(n²)
2. **Optimal:** Calculate height and check balance in one pass → O(n)
...
| Nearly balanced | Allow diff ≤ k | Generalized |
...
```python
# ❌ SLOW: Recalculating height for each node
...
# ✅ FAST: Single pass with sentinel
def check_height(node):
    if not node:
        return 0
...
# ✅ CORRECT: The recursive check handles all nodes
...
# ✅ CORRECT: Diff of 0 or 1 is okay
if abs(left_height - right_height) > 1:
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Meta | ⭐⭐⭐⭐ | Often asks O(n) solution |
| Google | ⭐⭐⭐⭐ | May ask about AVL |
...
> **💡 Key Insight:** The sentinel pattern (-1 for failure) is powerful for tree problems. It allows you to: (1) combine computation with validation, (2) enable early termination, and (3) maintain O(n) complexity. Use it whenever you need to check a condition at every node.
