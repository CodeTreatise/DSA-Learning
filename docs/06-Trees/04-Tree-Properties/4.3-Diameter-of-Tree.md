...
2. Diameter through this node (update global):
   = left_height + right_height
        ...
        1           height=3, diameter_here=2+1=3 ← max!
       / \
...
# ✅ CORRECT: Return height, update diameter separately
def height(node):
    left = height(node.left)
    right = height(node.right)
...
```python
# ❌ WRONG: Only checking diameter through root
def diameter(root):
...
> **💡 Key Insight:** The diameter problem teaches a fundamental pattern: sometimes you need to track a "global maximum" while computing something else (height) at each node. The answer isn't what you return from recursion, but what you accumulate across all recursive calls.
