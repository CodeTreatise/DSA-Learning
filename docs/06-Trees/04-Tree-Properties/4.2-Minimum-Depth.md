...
# For tree [1, null, 2]:
#   min(0, 1) = 0 → returns 1 (WRONG!)
#   Actual answer is 2

# ✅ CORRECT: Handle null children specially
def min_depth(root):
    if not root:
        return 0
    if not root.left:
...
# ✅ CORRECT: Start at 1 (root is depth 1)
queue = deque([root])
depth = 1
```
...
```python
# ❌ WRONG: Checking only one side
if not node.left:  # This doesn't mean it's a leaf!
    return depth

# ✅ CORRECT: Both must be null for leaf
if not node.left and not node.right:
    return depth
```
...
**Follow-up handling:**
- "What if tree is very wide?" → DFS uses less space
- "What if tree is very deep?" → BFS guaranteed to find min quickly
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Often paired with max depth |
| Meta | ⭐⭐⭐⭐ | Test edge case awareness |
| Google | ⭐⭐⭐ | May ask for BFS + DFS |
| Microsoft | ⭐⭐⭐ | Focus on edge cases |
...
> **💡 Key Insight:** The critical difference from max depth is handling null children. `min(0, something) = 0` is wrong because a null child doesn't make the parent a leaf. Always ask: "Does this node have ANY children?" If yes, it's not a leaf.
