...
- [Subtree of Another Tree](#) - Uses Same Tree as helper
...
Step 1: p=1, q=1 → values match, check children
Step 2: p.left=2, q.left=2 → values match, check children
Step 3: p.left.left=null, q.left.left=null → both null, true
Step 4: p.left.right=null, q.left.right=null → both null, true
Step 5: p.right=3, q.right=3 → values match, check children
Step 6: p.right.left=null, q.right.left=null → both null, true
Step 7: p.right.right=null, q.right.right=null → both null, true
...
```python
# ❌ WRONG: Only checking left subtree
def is_same(p, q):
    if not p and not q:
        return True
    if not p or not q:
...
```python
# ❌ WRONG: Object comparison, not value
return p == q  # This compares object references!

# ✅ CORRECT: Compare values and structure
return p.val == q.val and is_same(p.left, q.left) and is_same(p.right, q.right)
```
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Often paired with Symmetric |
| Meta | ⭐⭐⭐⭐ | May ask iterative version |
| Google | ⭐⭐⭐ | Used as helper for harder problems |
| Microsoft | ⭐⭐⭐ | Classic warm-up |
...
> **💡 Key Insight:** Same Tree is a fundamental building block. The pattern of checking "both null → true, one null → false, then compare values and recurse" appears in many tree comparison problems. Master this pattern once, apply it everywhere.
