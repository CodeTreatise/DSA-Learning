...
for node in all_nodes:
    result[node] = dfs_from(node)  # O(n) each = O(n²) total
```
...
2. Identify what information flows UP (child → parent)
...
4. **Rerooting** transforms O(n²) brute force into O(n) with two passes
5. **Tree structure** makes DP clean: each node only needs children's info

---

> **💡 Key Pattern:** Most Tree DP follows this template:
> ```python
> def dfs(node):
...
> **🔗 Related:** [Tree Problems](../../06-Trees.md) | [Recursion Patterns](../.../02-Recursion-Backtracking/02-Recursion-Backtracking.md) | [DP Fundamentals](../01-DP-Fundamentals/1.1-What-Is-DP.md)
