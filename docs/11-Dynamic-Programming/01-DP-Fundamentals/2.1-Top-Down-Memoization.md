...
**Key relationship:**
```
Recursion → Add memo → Memoization (Top-Down DP)
                           ↓ (convert)
...
```
Step 1: Check if already computed → Return cached result
Step 2: If base case → Return base value
Step 3: Compute recursively → Cache and return result
```
...
│   │   └── fib(1) → base case → return 1
│   │   → memo[3] = 2, return 2
│   └── fib(2) → IN MEMO → return 1 ✓ (no recomputation!)
│   → memo[4] = 3, return 3
└── fib(3) → IN MEMO → return 2 ✓ (no recomputation!)
→ memo[5] = 5, return 5
...
| Array memo | Dense integer states | dp[i] where 0 ≤ i ≤ n |
...
> **💡 Key Insight:** Memoization is just recursion with a cache. If you can write a recursive solution, you can convert it to DP by adding 3 lines: (1) check if in cache, (2) store result before returning, (3) return cached result.

> **🔗 Related:** [What Is DP](./1.1-What-Is-DP.md) | [Bottom-Up Tabulation](./2.2-Bottom-Up-Tabulation.md) | [Space Optimization](./2.3-Space-Optimization.md)
