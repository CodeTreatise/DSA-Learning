...
**Signals in problems:**
- Clear iteration order (small → large)
...
Order: 5 → 4 → 3 → 2 → 1 → 0 (then back up)
```
...
Order: 0 → 1 → 2 → 3 → 4 → 5 (always forward)
```
...
| Unique Paths | O(m*n) | O(m*n) | Can optimize to O(n) |
| LCS | O(m*n) | O(m*n) | Can optimize to O(min(m,n)) |
| Coin Change | O(amount * coins) | O(amount) | Already 1D |
...
```python
# Memoization: memo[(i, j)] → 2D state
# Tabulation:  dp[i][j]      → 2D array
```
...
```python
# Memoization base cases → Initial values in dp table
```
...
def lcs_tab(s1, s2):
    ...
    # 2D state → 2D table (add 1 for base case row/col)
    dp = [[0] * (n + 1) for _ in range(m + 1)]
...
| Convert memo → tab | 15-20 min | Mechanical process |
| Derive tabulation directly | 25-35 min | Requires practice |
| Master pattern | 5-7 problems | Until automatic |

---

> **💡 Key Insight:** Tabulation builds the solution from the ground up. Instead of asking "what do I need?" (top-down), you ask "what can I compute with what I have?" (bottom-up). This makes it easier to see space optimization opportunities.

> **🔗 Related:** [Top-Down Memoization](./2.1-Top-Down-Memoization.md) | [Space Optimization](./2.3-Space-Optimization.md) | [Problem-Solving Framework](./3.0-DP-Problem-Solving-Framework.md)
