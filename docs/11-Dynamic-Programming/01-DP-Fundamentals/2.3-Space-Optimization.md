...
```
dp[i] depends on dp[i-1], dp[i-2]     → O(1) space possible
dp[i][j] depends on dp[i-1][j], dp[i][j-1] → O(n) space possible
dp[i][j] depends on dp[i-2][j], dp[i][j-2] → O(2n) space possible
```
...
```
Full Table O(n*m)
     ↓ Optimize row dependency
Two Rows O(2*m) = O(m)
     ↓ Optimize to single row
Single Row O(m)
     ↓ Optimize to variables
Few Variables O(1)
```
...
### Technique 1: 1D → O(1) (Rolling Variables)
...
### Technique 3: 2D → O(n) with Careful Iteration
...
|---------|--------|-------|-----------|
| Fibonacci | O(n) | O(1) | n → 1 |
| Climbing Stairs | O(n) | O(1) | n → 1 |
| Unique Paths | O(m*n) | O(min(m,n)) | m*n → n |
| LCS | O(m*n) | O(min(m,n)) | m*n → n |
| Edit Distance | O(m*n) | O(min(m,n)) | m*n → n |
| 0/1 Knapsack | O(n*W) | O(W) | n*W → W |
...
**❌ Wrong (overwrites before reading):**
```python
for i in range(1, m + 1):
    for j in range(1, n + 1):
        if text1[i-1] == text2[j-1]:
...
- **Day 3:** Optimize Unique Paths from O(m*n) to O(n)
...
> "I can optimize this from O(m*n) space to O(n) space because each row only depends on the previous row. Would you like me to show that?"
...
| Optimize 1D → O(1) | 10-15 min |
| Optimize 2D → O(n) | 20-30 min |
| Master backward iteration | 2-3 problems |

---

> **💡 Key Insight:** Space optimization works because DP is fundamentally about remembering just enough. Look at your recurrence: if dp[i] only needs dp[i-1] and dp[i-2], you don't need the whole history—just the last 2 values.

> **🔗 Related:** [Bottom-Up Tabulation](./2.2-Bottom-Up-Tabulation.md) | [0/1 Knapsack](../04-Knapsack-01/6.1-Knapsack-01-Pattern.md) | [Unbounded Knapsack](../05-Knapsack-Unbounded/6.3-Unbounded-Knapsack-Pattern.md)
