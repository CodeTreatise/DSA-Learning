...
> **When your state is a subset.** Bitmask DP uses binary representation to track which elements have been selected. Perfect for problems with small sets (n ≤ 20) where we need to track "which elements are used."
...
- **n ≤ 20** (otherwise 2^n states too large)
...
|---------|------|-------|
| TSP | O(n² * 2ⁿ) | O(n * 2ⁿ) |
| Partition to K | O(n * 2ⁿ) | O(2ⁿ) |
| Hat Assignment | O(40 * n * 2ⁿ) | O(2ⁿ) |

**When is Bitmask DP feasible?**
- n ≤ 15: Very comfortable
- n ≤ 20: Usually OK
- n ≤ 25: Might need optimization
...
2. **2^n states**: Only feasible for n ≤ 20
...
> **💡 Key Pattern:** When you see "assign N items to N slots" or "visit all N nodes" with N ≤ 20, think Bitmask DP. The mask tracks which elements are "used," and transitions add one more element.

> **🔗 Related:** [Backtracking](../../02-Recursion-Backtracking/02-Backtracking/2.1-Backtracking-Basics.md) | [Graph Algorithms](../.../10-Graphs/10-Graphs.md) | [DP Fundamentals](../01-DP-Fundamentals/1.1-What-Is-DP.md)
