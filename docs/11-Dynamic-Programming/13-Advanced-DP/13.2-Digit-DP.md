...
Position 0: Can place 0-5
  If we place 5 → tight=True (still bounded)
  If we place 0-4 → tight=False (already less than limit)
...
Position 2 (after placing 52): Can place 0-3
  If we place 3 → exactly 523
  If we place 0-2 → 520, 521, 522
```
...
**Total: O(d * S * 2 * 2 * 10) = O(d * S * 40)**

**Examples:**
- Digit sum ≤ K: S = K+1, Total = O(d * K)
- Distinct digits: S = 2^10 = 1024, Total = O(d * 1024)
- Divisibility: S = K (remainder), Total = O(d * K)
...
> **💡 Key Pattern:** Digit DP builds numbers left-to-right, tracking whether we're still "tight" (bounded by the limit) and whether we've "started" (placed a non-zero digit). The state captures whatever property we're counting.

> **🔗 Related:** [Bitmask DP](./13.1-Bitmask-DP.md) | [DP Fundamentals](../01-DP-Fundamentals/1.1-What-Is-DP.md) | [Binary Search](../../03-Sorting-Searching/03-Sorting-Searching.md)
