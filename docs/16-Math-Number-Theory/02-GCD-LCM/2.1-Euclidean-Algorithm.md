...
So we need at most `2 * log(min(a, b))` iterations.
...
```python
# ❌ WRONG - might think order matters
def gcd_wrong(a, b):
    if a < b:
        a, b = b, a  # Unnecessary!
    # ...
...
| Need all divisors | Iterate to √n |
...
> **💡 Key Insight:** GCD is one of the oldest algorithms (300 BC, Euclid) and still one of the most efficient. The key is that `gcd(a, b) = gcd(b, a % b)` - the remainder preserves all common divisors.

> **🔗 Related:** [Extended GCD →](./2.2-Extended-GCD.md) | [GCD Applications →](./2.3-GCD-Applications.md) | [Modular Inverse](../05-Modular-Inverse/5.1-Modular-Inverse-Basics.md)
