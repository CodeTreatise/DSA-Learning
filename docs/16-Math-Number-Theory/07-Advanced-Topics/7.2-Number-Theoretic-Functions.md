...
flowchart TD
    ...
    B -->|"Count coprimes"| C["Euler's Totient φ(n)"]
    B -->|"Count divisors"| D["Divisor Count τ(n)"]
    B -->|"Sum divisors"| E["Divisor Sum σ(n)"]
    B -->|"Inclusion-Exclusion"| F["Möbius μ(n)"]
    
    C --> C1["φ(n) = n * Π(1 - 1/p)"]
    D --> D1["τ(n) = Π(eᵢ + 1)"]
    E --> E1["σ(n) = Π(p^(e+1)-1)/(p-1)"]
...
**Why O(√n) for single value:**
- All functions require prime factorization
- Trial division up to √n suffices
...
```python
# ❌ WRONG - not checking for squared factors
if n % p == 0:
    n //= p
    count += 1
...
| [Ugly Number II](https://leetcode.com/problems/ugly-number-ii/) | 🟡 Medium | Prime factors |
| [Count Primes](https://leetcode.com/problems/count-primes/) | 🟡 Medium | Sieve (related) |
...
> **🔗 Related:** [Modular Inverse](../05-Modular-Inverse/5.1-Modular-Inverse-Basics.md) | [Sieve of Eratosthenes](../03-Prime-Numbers/3.2-Sieve-of-Eratosthenes.md) | [Prime Factorization](../03-Prime-Numbers/3.1-Prime-Basics.md)
