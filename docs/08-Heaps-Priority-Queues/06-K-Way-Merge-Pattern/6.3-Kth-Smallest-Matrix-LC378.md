...
Approach 2: Binary Search on Value
  - Binary search for value where count(≤ value) = k
...
|-----------|---------------|
| k is small (k ≈ n) | K-Way Merge |
| k is large (k ≈ n²) | Binary Search |
...
Extract minimum, push next element from same row:
  Pop (1, 0, 0) → push (5, 0, 1)
  Pop (5, 0, 1) → push (9, 0, 2)
  Pop (9, 0, 2) → no more in row 0
  Pop (10, 1, 0) → push (11, 1, 1)
  ...
...
For each mid value, count how many elements ≤ mid:
  ...
  If current ≤ mid: add row+1, move right
  Else: move up

If count ≥ k, x might be answer or smaller → search left
If count < k, x too small → search right
```
...
def kthSmallest_binary(matrix: list[list[int]], k: int) -> int:
    ...
    Find smallest value x where count(≤ x) ≥ k.
    ...
    def count_less_or_equal(mid):
        """Count elements ≤ mid using matrix structure."""
        count = 0
...
```python
# ❌ Wrong: May push same cell multiple times
min_heap = [(matrix[0][0], 0, 0)]
while heap:
...
```python
# ❌ Wrong: Using count >= k to go left
if count >= k:
    right = mid - 1  # Might miss the answer!

# ✅ Correct: Use right = mid (inclusive)
if count >= k:
...
```python
# ❌ Wrong: Assuming left is always in matrix
while left < right:
    mid = (left + right) // 2
...
# Actually ✅ Correct: The algorithm guarantees left IS in matrix
...
```python
# ❌ Wrong: Only initializing with first element
heap = [(matrix[0][0], 0, 0)]
# Then only adding right neighbors → misses elements!

# ✅ Correct: Add both right AND down neighbors
...
**Opening:**
"I see two approaches here. First, K-Way Merge: treat each row as a sorted list and merge them using a min-heap, stopping at the kth element. Second, Binary Search on value: search for the smallest value where the count of elements ≤ that value is at least k."
...
**For binary search:**
"I'll binary search between min and max values. For each mid, I count elements ≤ mid by starting from bottom-left and moving right when ≤ mid (adding row+1 to count) or up when > mid."
...
|---------|-----------|-------------------|
| Google | ⭐⭐⭐⭐⭐ | Both, discuss trade-offs |
| Amazon | ⭐⭐⭐⭐ | Heap approach |
| Meta | ⭐⭐⭐ | Either |
| Microsoft | ⭐⭐⭐ | Binary search |
