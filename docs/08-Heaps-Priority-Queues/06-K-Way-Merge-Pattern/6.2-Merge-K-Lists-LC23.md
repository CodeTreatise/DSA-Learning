...
```python
# ❌ Wrong: ListNode objects can't be compared directly
heapq.heappush(heap, (node.val, node))  # TypeError when values equal!

# ✅ Correct: Add unique tiebreaker (list index)
...
# ✅ Correct: Push next node from same list
while min_heap:
    val, idx, node = heapq.heappop(min_heap)
    current.next = node
    current = current.next
...
```python
# ❌ Wrong: Not handling odd number of lists
for i in range(0, len(lists), 2):
    merged.append(mergeTwoLists(lists[i], lists[i+1]))  # IndexError!

# ✅ Correct: Check bounds for second list
for i in range(0, len(lists), 2):
    l1 = lists[i]
    l2 = lists[i+1] if i+1 < len(lists) else None
    merged.append(mergeTwoLists(l1, l2))
...
|---------|-----------|-------|
| Google | ⭐⭐⭐⭐⭐ | Very commonly asked |
| Amazon | ⭐⭐⭐⭐⭐ | Frequently asked |
| Meta | ⭐⭐⭐⭐ | Standard interview question |
| Microsoft | ⭐⭐⭐⭐ | Common |
