...
```
K Sorted Lists → Min-Heap → Single Sorted Output

List 1: [1, 4, 7]
List 2: [2, 5, 8]        Min-Heap         Merged Output
List 3: [3, 6, 9]   →   (smallest)   →   [1, 2, 3, 4, 5, 6, 7, 8, 9]
                            │
...
```python
# ❌ Wrong: Assuming all lists have elements
for i, arr in enumerate(arrays):
...
```python
# ❌ Wrong: Only popping, never pushing next
while min_heap:
    val, arr_idx, elem_idx = heapq.heappop(min_heap)
    result.append(val)
    # Missing: push next element from same list!

# ✅ Correct: Always check and push next
while min_heap:
    val, arr_idx, elem_idx = heapq.heappop(min_heap)
    result.append(val)
...
```python
# ❌ Wrong: Using elem_idx instead of elem_idx + 1
...
|---------|-----------|-------|
| Google | ⭐⭐⭐⭐⭐ | LC 23 very common |
| Amazon | ⭐⭐⭐⭐ | Often asked |
| Meta | ⭐⭐⭐⭐ | Standard heap problem |
| Microsoft | ⭐⭐⭐ | Less frequent |
...
> **K-way merge keeps exactly K elements in the heap—one from each list.** The minimum is always the next element in sorted order. After extracting the minimum, we push the next element from the same list, maintaining the invariant. This gives O(N log K) time, which is optimal when lists are already sorted.
