...
| Range with count ≥ k | Sliding window |
...
```python
# ❌ Wrong: Not updating max when pushing new element
next_val = nums[list_idx][elem_idx + 1]
heapq.heappush(min_heap, (next_val, list_idx, elem_idx + 1))
# Forgot to update current_max!

# ✅ Correct: Update max on each push
heapq.heappush(min_heap, (next_val, list_idx, elem_idx + 1))
current_max = max(current_max, next_val)
```
...
```python
# ❌ Wrong: Continuing when a list is exhausted
while min_heap:  # Wrong condition!
    current_min, list_idx, elem_idx = heapq.heappop(min_heap)
    # Range now doesn't cover all lists!

# ✅ Correct: Stop when any list is exhausted
...
# ✅ Correct: Compare width, then left endpoint
...
```python
# ❌ Wrong: Assuming all lists non-empty
for i in range(k):
    heapq.heappush(min_heap, (nums[i][0], i, 0))

# ✅ Correct: Check for empty lists
for i in range(k):
    if nums[i]:
        heapq.heappush(min_heap, (nums[i][0], i, 0))
...
|---------|-----------|-------|
| Google | ⭐⭐⭐⭐⭐ | Hard favorite |
| Amazon | ⭐⭐⭐⭐ | Merge pattern |
| Meta | ⭐⭐⭐ | K-way merge |
| Apple | ⭐⭐⭐ | Range problems |
