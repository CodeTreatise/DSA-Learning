...
- Smaller half → MAX-heap (get maximum easily)
- Larger half → MIN-heap (get minimum easily)
...
- Balance: len(small) ∈ {len(large), len(large) + 1}
- Invariant: max(small) ≤ min(large)
```
...
Add 5:
  small: [-5]    large: []
  Sizes: 1, 0 → balanced (small can be 1 larger)
  Median = 5

Add 2:
  2 ≤ 5, add to small
  small: [-5, -2]    large: []
  Sizes: 2, 0 → UNBALANCED! Move 5 to large
  small: [-2]    large: [5]
  Sizes: 1, 1 → balanced
  Median = (2 + 5) / 2 = 3.5
...
Add 8:
  8 > 2, add to large
  small: [-2]    large: [5, 8]
  Sizes: 1, 2 → UNBALANCED! Move 5 to small
  small: [-5, -2]    large: [8]
  Sizes: 2, 1 → balanced
  Median = 5
...
Add 3:
  3 ≤ 5, add to small
  small: [-5, -3, -2]    large: [8]
  Sizes: 3, 1 → UNBALANCED! Move 5 to large
  small: [-3, -2]    large: [5, 8]
  Sizes: 2, 2 → balanced
  Median = (3 + 5) / 2 = 4.0
...
Add 6:
  ...
  Sizes: 2, 3 → UNBALANCED! Move 5 to small
...
```python
# ❌ Wrong: Python heapq is min-heap by default
...
```python
# ❌ Wrong: Allowing large to be bigger than small
# This breaks the convention for odd counts

# ✅ Correct convention:
# len(small) ∈ {len(large), len(large) + 1}
...
**Follow-up handling:**
- *"What if elements can be removed?"* → Lazy deletion with invalidation map
- *"What about sliding window?"* → Track invalid elements, clean during rebalance
- *"Can we optimize for integers in a range?"* → Bucket counting
...
| Microsoft | ⭐⭐⭐⭐ | Common data stream question |
| Google | ⭐⭐⭐⭐ | May ask follow-ups |
| Meta | ⭐⭐⭐ | Less common but possible |
