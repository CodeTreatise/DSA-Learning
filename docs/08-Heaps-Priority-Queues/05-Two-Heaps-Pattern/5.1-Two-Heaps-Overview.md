...
- Smaller half in MAX-heap → get maximum in O(1)
- Larger half in MIN-heap → get minimum in O(1)
...
3. Balance sizes: |max_heap| - |min_heap| ≤ 1
...
Rule 1: Sizes differ by at most 1
        len(max_heap) - len(min_heap) ∈ {0, 1}
        
Rule 2: All elements in max_heap ≤ all elements in min_heap
...
```
addNum(num):
    1. If num ≤ max of smaller half (or smaller half empty):
...
class MedianFinder:
    ...
    Invariant: 0 ≤ len(max_heap) - len(min_heap) ≤ 1
    """
...
```python
# ❌ Wrong: Treating small as min-heap
heapq.heappush(self.small, num)

# ✅ Correct: Negate for max-heap behavior in Python
heapq.heappush(self.small, -num)
...
```python
# ❌ Wrong: max_heap can be smaller than min_heap
# This breaks the convention for odd counts

# ✅ Correct: max_heap should have the extra element
# len(max_heap) ∈ {len(min_heap), len(min_heap) + 1}
```

### 3. Not Handling Empty Heaps

```python
# ❌ Wrong: Accessing empty heap
def findMedian(self):
    return (-self.small[0] + self.large[0]) / 2  # Crashes if one is empty

# ✅ Correct: Check sizes first
def findMedian(self):
    if len(self.small) > len(self.large):
        return -self.small[0]
    return (-self.small[0] + self.large[0]) / 2
...
```python
# ❌ Wrong: Comparing with empty max-heap
if num <= -self.max_heap[0]:  # IndexError!

# ✅ Correct: Check if heap is empty first
if not self.max_heap or num <= -self.max_heap[0]:
    heapq.heappush(self.max_heap, -num)
```
...
**Follow-up questions:**
- *"What if we need to remove elements?"* → Lazy deletion with hash map
- *"What about sliding window?"* → Track invalid elements, clean during rebalance
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | LC 295 commonly asked |
| Google | ⭐⭐⭐⭐ | May ask follow-ups |
| Microsoft | ⭐⭐⭐ | Standard median problem |
| Apple | ⭐⭐⭐ | Data stream questions |
