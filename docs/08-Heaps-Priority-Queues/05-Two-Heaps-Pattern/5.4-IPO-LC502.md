...
You can only start a project if your current capital is ≥ its required capital. After completing a project, its profit is added to your capital.
...
2. For each of k iterations:
   a. Move all affordable projects (capital ≤ w) to max_heap
...
Step 2: First project
  Move affordable (capital ≤ 0) to max_heap:
    (0,1) → max_heap = [(-1)]
  min_heap = [(1,2), (1,3)]
...
Step 3: Second project
  Move affordable (capital ≤ 1) to max_heap:
    (1,2), (1,3) → max_heap = [(-3), (-2)]
  min_heap = []
...
- Total: O((n + k) log n) = O(n log n) since k ≤ n
...
| Minimum profit threshold | Only take if profit ≥ X | Filter in max_heap pop |
...
```python
# ❌ Wrong: Sorting once doesn't account for changing capital
projects = sorted(zip(capital, profits))
for cap, profit in projects:
    if cap <= w:
        w += profit  # Wrong! Should pick max profit among affordable

# ✅ Correct: Use max_heap to always pick best affordable
while min_heap and min_heap[0][0] <= w:
    cap, profit = heapq.heappop(min_heap)
    heapq.heappush(max_heap, -profit)
w += -heapq.heappop(max_heap)
...
```python
# ❌ Wrong: Assuming there's always an affordable project
for _ in range(k):
    while min_heap and min_heap[0][0] <= w:
        ...
...
```python
# ❌ Wrong: Only moving affordable after pop
for _ in range(k):
    if min_heap and min_heap[0][0] <= w:  # Only checks one!
        ...
    w += -heapq.heappop(max_heap)

# ✅ Correct: Move ALL affordable at once (while loop)
for _ in range(k):
    while min_heap and min_heap[0][0] <= w:
        cap, profit = heapq.heappop(min_heap)
        heapq.heappush(max_heap, -profit)
...
```python
# ❌ Wrong: Using max-heap for capital (should be min-heap)
# We want smallest capital first (to check affordability)

# ✅ Correct:
# min_heap for capital: smallest capital at top → check if affordable
# max_heap for profit: largest profit at top → select best
```
...
**Walking through:**
"In each iteration, I first move all projects with capital ≤ my current w into the available pool. Then I greedily take the most profitable one and add its profit to my capital."
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Resource allocation theme |
| Google | ⭐⭐⭐ | Hard problems |
| Meta | ⭐⭐⭐ | Two heaps pattern |
| Bloomberg | ⭐⭐⭐⭐ | Financial domain |
