...
```
KthLargest class:
├── __init__(k, nums) - Initialize with k and initial stream
├── add(val) - Add new element, return kth largest
```
...
- New element < root means at least K elements are larger
  → It can never be in top K, safe to ignore
```
...
|----------|------|-----|-------|
| Min-heap size K ⭐ |" O(n log K) "|" O(log K) "|" O(K) "|
...
```python
# ❌ Wrong: Max-heap doesn't give Kth largest directly
max_heap = []
for num in nums:
...
```python
# ❌ Wrong: Assumes heap always has K elements
def add(self, val):
...
```python
# ❌ Subtly wrong: What if val equals heap[0]?
if val > self.heap[0]:
    heapq.heapreplace(self.heap, val)
...
```python
# ❌ Wrong: What to return when heap.size < k?
def add(self, val):
    heapq.heappush(self.heap, val)
