...
> 💡 **Core Insight:** Instead of sorting the entire array O(n log n), use a heap of size K to achieve O(n log k) time complexity.
...
| K ≈ n | Heap loses advantage | Just sort |
...
Final heap: [7, 8, 9] ← The 3 largest elements!
Root (7) = 3rd largest = Kth largest ✓
```
...
| Full sort | O(n log n) | O(1) or O(n) | k ≈ n |
...
Heap:   O(n log k) = O(1M × log 10) ≈ 3.3M operations
Sort:   O(n log n) = O(1M × log 1M) ≈ 20M operations
...
**Common follow-ups:**
- "What if K is close to n?" → "Then sorting might be simpler with same complexity"
- "Can you do better?" → "QuickSelect gives O(n) average for single Kth element"
- "What about streaming data?" → "Heap is perfect, just maintain size K"
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | LC 215, 347, 973 very common |
| Meta | ⭐⭐⭐⭐ | Top K Frequent variants |
| Google | ⭐⭐⭐⭐ | K Closest, custom comparators |
| Microsoft | ⭐⭐⭐ | Basic Top K |
