...
2. Get top K by frequency → O(n log k) with heap
...
Final heap: [(2, 2), (3, 1)]
Elements: [2, 1] or [1, 2] ← Top 2 most frequent
```
...
- **Bucket sort:** When you need O(n) and max frequency ≤ n
...
```python
# ❌ Risky: When frequencies are equal, Python compares next element
heap = []
heapq.heappush(heap, (2, [1, 2]))  # Lists aren't comparable!

# ✅ Safe: Add unique tiebreaker
counter = 0
...
```python
# ❌ Wrong: Returning (freq, num) tuples
return min_heap  # [(2, 2), (3, 1)] - wrong!

# ✅ Correct: Extract just the elements
...
```python
# ❌ Missing: k equals number of unique elements
if k == len(freq):
...
# ✅ Correct: n+1 buckets for frequencies 0 to n
buckets = [[] for _ in range(n + 1)]
```
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Meta | ⭐⭐⭐⭐⭐ | Frequent + follow-ups |
| Google | ⭐⭐⭐⭐ | May ask for bucket sort |
| Microsoft | ⭐⭐⭐ | Standard |
...
> **Two-step pattern:** First count with a hash map, then apply Top K pattern on the frequencies. This combines two fundamental operations into a clean solution. The bucket sort optimization shows that when values are bounded (frequency ≤ n), we can achieve O(n) time.
