...
Step 1: Push 3 → [3]
Step 2: Push 2 → [2, 3]    (size = k = 2)
Step 3: Push 1 → 1 < root(2), skip (would be popped anyway)
        Actually: Push then pop → [2, 3]
Step 4: Push 5 → [2, 3, 5] → pop → [3, 5]
Step 5: Push 6 → [3, 5, 6] → pop → [5, 6]
Step 6: Push 4 → 4 < root(5), skip
        Actually: Push then pop → [5, 6]
...
**Which to choose:**
```
k << n (k is small):   Min-heap O(n log k) ≈ O(n)
k ≈ n (k is large):    Sort O(n log n) - simpler
...
```python
# ❌ Wrong: Using max-heap then popping K times
...
```python
# ❌ Wrong: Confusing index vs rank
return sorted(nums)[k]  # This is (k+1)th smallest!

# ✅ Correct: k-1 for 0-indexed
...
```python
# ❌ Risky: Fixed pivot can cause O(n²)
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common, often first heap problem |
| Meta | ⭐⭐⭐⭐⭐ | Asked frequently |
| Google | ⭐⭐⭐⭐ | May ask for QuickSelect follow-up |
| Microsoft | ⭐⭐⭐⭐ | Standard heap question |
