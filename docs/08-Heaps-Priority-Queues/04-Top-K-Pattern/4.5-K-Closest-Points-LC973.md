...
**Key insight:**
- K smallest distances → MAX-heap of size K
...
```
For K LARGEST → MIN-heap of size K
For K SMALLEST → MAX-heap of size K
...
```python
# ❌ Unnecessary: Computing actual distance
import math
...
# ✅ Correct: Max-heap for K smallest (negate distance)
max_heap = []
for point in points:
...
```python
# ❌ Risky: When distances are equal, lists compared
heapq.heappush(heap, (-dist, [x, y]))  # [x,y] comparison may fail

# ✅ Safe: Use tuple instead of list
...
"I'll compare squared distances instead of actual distances to avoid unnecessary square root computations. Since we only care about relative ordering, √a < √b ⟺ a < b."
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common, often asked |
| Meta | ⭐⭐⭐⭐ | Standard heap problem |
| Google | ⭐⭐⭐⭐ | May ask for QuickSelect |
| LinkedIn | ⭐⭐⭐⭐ | Frequently asked |
| Uber | ⭐⭐⭐⭐ | Geo/location relevance |
