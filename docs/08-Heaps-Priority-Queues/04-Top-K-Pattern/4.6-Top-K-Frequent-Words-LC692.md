...
```python
# ❌ Wrong: Alphabetically later first
sorted_words = sorted(count.keys(), key=lambda w: (-count[w], w), reverse=True)

# ✅ Correct: Alphabetically earlier first
sorted_words = sorted(count.keys(), key=lambda w: (-count[w], w))
```
...
```python
# ❌ Wrong: Earlier word pops first (should stay!)
def __lt__(self, other):
    if self.freq != other.freq:
        return self.freq < other.freq
    return self.word < other.word  # Wrong for min-heap!

# ✅ Correct: Later word pops first (we keep earlier)
def __lt__(self, other):
    if self.freq != other.freq:
        return self.freq < other.freq
...
```python
# ❌ Wrong: Returning min-heap order (worst first)
result = []
while heap:
    result.append(heapq.heappop(heap).word)
return result  # Reversed order!

# ✅ Correct: Reverse to get best first
return result[::-1]
```
...
```python
# ❌ Wrong: Using just tuple without wrapper
