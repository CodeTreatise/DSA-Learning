...
Time 0: Meeting [0,30] starts → need 1 room
        Heap: [30]  (end time of room 1)

Time 5: Meeting [5,10] starts
        Earliest free room ends at 30 > 5 → need new room
        Heap: [10, 30]  (2 rooms)

Time 15: Meeting [15,20] starts
         Earliest free room ends at 10 ≤ 15 → reuse!
...
```python
# ❌ Wrong: Strict greater than
if start > end_times[0]:  # Misses when end == start

# ✅ Correct: Can reuse if meeting ended
...
```python
# ❌ Wrong: Just push without checking reuse
heapq.heappush(end_times, end)

# ✅ Correct: Pop-then-push for reuse
if start >= end_times[0]:
...
```python
# ❌ Wrong: Start before end at same time
events.sort()  # (10, 1) before (10, -1) → wrong count

# ✅ Correct: End before start at same time
...
|---------|-----------|-------|
| Google | ⭐⭐⭐⭐⭐ | Classic interview question |
| Meta | ⭐⭐⭐⭐⭐ | Very common |
| Amazon | ⭐⭐⭐⭐⭐ | Multiple variations |
| Microsoft | ⭐⭐⭐⭐ | Standard |
