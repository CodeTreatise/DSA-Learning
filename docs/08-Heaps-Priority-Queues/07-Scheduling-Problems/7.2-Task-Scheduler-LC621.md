...
```python
# ❌ Wrong: May return less than tasks length
return (max_count - 1) * (n + 1) + num_max

# ✅ Correct: At minimum, need len(tasks)
...
```python
# ❌ Wrong: Checking <=, might add back too early
if cooling and cooling[0][0] <= time:

# ✅ Correct: Exactly when cooldown expires
if cooling and cooling[0][0] == time:
```
...
A _ _ A  ← 2 gaps between A's, total 4 time units for 2 A's
  1 2
```
...
|---------|-----------|-------|
| Meta | ⭐⭐⭐⭐⭐ | Very common |
| Amazon | ⭐⭐⭐⭐⭐ | Real-world relevance |
| Microsoft | ⭐⭐⭐⭐ | Common |
| Google | ⭐⭐⭐ | Sometimes with variations |
