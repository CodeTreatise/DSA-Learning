...
|----------|------|-------|-------|
| Max-Heap + Prev | O(n log k) | O(k) | k = unique chars (≤ 26) |
...
- k ≤ 26 for lowercase letters, so effectively O(n log 26) = O(n)
...
```python
# ❌ Wrong: May try to construct impossible result
def reorganize(s):
...
```python
# ❌ Wrong: Using positive counts in max-heap
prev_count = neg_count - 1  # This makes it more negative!

# ✅ Correct: Add 1 to negative count (reduces magnitude)
...
**Impossibility:**
"First, I check if it's even possible. If any character appears more than ⌈n/2⌉ times, there's no valid arrangement. For example, 'aaab' has 4 characters, max allowed per char is 2, but 'a' appears 3 times."
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Google | ⭐⭐⭐⭐ | Multiple variations |
| Meta | ⭐⭐⭐⭐ | Common |
| Microsoft | ⭐⭐⭐ | Sometimes asked |
...
> **The greedy approach works because placing the most frequent character first gives the most flexibility for the remaining positions.** The impossibility condition `max_count > (n + 1) // 2` comes from the pigeonhole principle: in a string of length n, we can only have at most ⌈n/2⌉ of the same character in non-adjacent positions.
