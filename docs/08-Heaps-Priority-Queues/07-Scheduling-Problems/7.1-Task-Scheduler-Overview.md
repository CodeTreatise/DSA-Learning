...
| Reorganize String | O(n log k) | O(k) | k = unique chars (≤26) |
...
- **Reorganize:** Each char processed once, heap op O(log k) where k ≤ 26
...
```python
# ❌ Wrong: Just using max-heap without cooldown
while max_heap:
    task = heappop(max_heap)
    # Where does task go after cooldown?

# ✅ Correct: Track cooldown separately
...
```python
# ❌ Wrong: Assuming heap always has tasks
for each time step:
    task = heappop(max_heap)  # Error if empty!

# ✅ Correct: Check if heap empty (idle)
if max_heap:
    task = heappop(max_heap)
else:
...
```python
# ❌ Wrong: Min-heap for task frequency
min_heap = [count for count in counts]  # Gets LEAST frequent first!

# ✅ Correct: Max-heap for greedy (most frequent first)
