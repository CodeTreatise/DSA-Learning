...
├── Index 0: Root (highest priority)
├── Left child of i: 2*i + 1
├── Right child of i: 2*i + 2
└── Parent of i: (i-1) // 2

Operations:
├── enqueue → push to end, bubble up
├── dequeue → swap root with last, pop, bubble down
├── peek → return arr[0]
└── size → return len(arr)
```
...
Option 1: Tuples (priority, data)
  ...
  Problem: If priorities equal, compares data → error if not comparable
...
# ❌ Wrong: Modifying item's priority after insertion
entry = [3, "task"]
heap = [entry]
heapq.heapify(heap)
entry[0] = 1  # Changed priority, but heap not updated!

# ✅ Correct: Use indexed PQ or re-insert
...
```javascript
// ❌ Wrong: Expected max-heap but got min-heap
const pq = new PriorityQueue((a, b) => a.priority - b.priority);
// This is min-heap: smaller priority wins

// ✅ Correct: For max-heap, reverse comparison
const maxPQ = new PriorityQueue((a, b) => b.priority - a.priority);
...
| Google | ⭐⭐⭐⭐ | Edge cases, optimization |
| Meta | ⭐⭐⭐ | Correctness |
| Startups | ⭐⭐⭐⭐⭐ | From scratch implementation |
