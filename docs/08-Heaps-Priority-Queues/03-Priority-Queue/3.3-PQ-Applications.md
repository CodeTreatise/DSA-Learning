...
Max-Priority Queue execution order:
  P1 (priority 3) → P3 (priority 2) → P2 (priority 1)
...
tasks = ["A", "A", "A", "B", "B", "B"]
n = 2
print(task_scheduler(tasks, n))  # 8: A→B→idle→A→B→idle→A→B
```
...
```python
# ❌ Wrong: Processing same node multiple times at different distances
while pq:
    dist, node = heapq.heappop(pq)
    # Process node...

# ✅ Correct: Skip if we've already found a shorter path
while pq:
    dist, node = heapq.heappop(pq)
...
- "Why not just sort?" → Sorting is O(n log n) upfront; PQ is O(log n) per operation, better for streaming
- "What about BST?" → PQ has O(1) peek vs O(log n), simpler implementation
- "Space complexity?" → O(n) for heap, or O(K) if maintaining fixed size
