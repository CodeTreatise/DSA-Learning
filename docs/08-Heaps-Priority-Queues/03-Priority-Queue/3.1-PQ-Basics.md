...
Enqueue: A → B → C                 Enqueue: A(3) → B(1) → C(2)
...
Binary Heap         O(n log n)       ⭐ Fast, simple
Unsorted Array      O(n²)            ❌ Too slow
Sorted Array        O(n²)            ❌ Too slow
...
enqueue("Task C", 2):
   ...
   → After heapify: [Task B(1), Task C(2), Task A(3)] (conceptually)
...
# ❌ Wrong: Objects without comparison
class Task:
...
# ✅ Correct: Use tuple with priority
...
# ⚠️ Priority queues don't support decrease-key efficiently
pq = [(5, "node1"), (3, "node2")]
heapq.heapify(pq)
...
|---------|-----------|-----------------|
| Amazon | ⭐⭐⭐⭐⭐ | Scheduling, Top K |
| Meta | ⭐⭐⭐⭐ | Stream processing |
| Google | ⭐⭐⭐⭐ | Graph algorithms |
| Microsoft | ⭐⭐⭐⭐ | Task scheduling |
