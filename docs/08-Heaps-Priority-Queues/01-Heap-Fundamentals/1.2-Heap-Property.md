...
100 ≥ 90 ✓  100 ≥ 80 ✓           1 ≤ 5 ✓   1 ≤ 3 ✓
90 ≥ 70 ✓   90 ≥ 60 ✓            5 ≤ 10 ✓  5 ≤ 8 ✓
```
...
|-----------|----------|----------|
| **Min-Heap** | Parent ≤ children | Priority queue (min first) |
| **Max-Heap** | Parent ≥ children | Priority queue (max first) |
...
# ✅ Correct: Compare parent with its children
def is_min_heap(arr):
...
```python
# ❌ Wrong: 1-indexed formulas with 0-indexed array
parent = i // 2              # Wrong for 0-indexed
left = 2 * i                 # Wrong for 0-indexed

# ✅ Correct: 0-indexed formulas
parent = (i - 1) // 2
left = 2 * i + 1
right = 2 * i + 2
```
...
```python
# ❌ Wrong: Checking leaf nodes too (wasteful)
for i in range(len(arr)):  # Includes leaves
    
# ✅ Correct: Only check internal nodes (have children)
