...
- Max-heap: parent ≥ children (always)
- Min-heap: parent ≤ children (always)
...
```python
# ❌ Wrong: Using min-heap for "K largest"
for num in arr:
...
```python
# ❌ Wrong: Changing elements breaks heap property
heap = [1, 3, 2]
heapq.heapify(heap)
...
|---------|-----------|---------------------|
| Amazon | ⭐⭐⭐⭐⭐ | Top K, scheduling |
| Google | ⭐⭐⭐⭐⭐ | K-way merge, median |
| Meta | ⭐⭐⭐⭐ | Priority queue |
| Microsoft | ⭐⭐⭐⭐ | Heap fundamentals |
...
> **A heap gives O(1) access to the extreme element (min or max) with O(log n) updates.** The magic comes from the complete binary tree property (efficient array storage) combined with the heap property (extreme always at root). For Top K problems, remember: **K largest → min-heap of size K; K smallest → max-heap of size K.**
