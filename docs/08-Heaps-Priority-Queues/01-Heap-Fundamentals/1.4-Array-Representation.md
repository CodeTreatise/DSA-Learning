...
- **Simple navigation** → Index formulas
- **No wasted space** → Complete tree fills array
...
If child is at index i:
  - If left child:  2*parent + 1 = i  → parent = (i-1)/2
  - If right child: 2*parent + 2 = i  → parent = (i-2)/2
...
```python
# ❌ Wrong: Accessing without bounds check
...
```python
# ❌ Wrong: Removing from middle (creates gap)
def bad_remove(heap, i):
    del heap[i]  # Breaks completeness!

# ✅ Correct: Always remove from end, then fix
...
# ✅ Correct: Collect elements first, then process
