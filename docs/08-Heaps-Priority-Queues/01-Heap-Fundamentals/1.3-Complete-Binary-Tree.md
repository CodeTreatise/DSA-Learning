...
Last level fills L→R ✓           Gap in last level ✗
```
...
- Height = **⌊log₂(n)⌋** (minimum possible for n nodes)
...
Perfect → Complete ✓        Full ≠ Complete          Complete ≠ Full
Complete → NOT necessarily  (gap at right)           (node 3 has 1 child)
   Perfect
```
...
If we have 6 nodes:
       ...
       4 5 6            Position 7 is empty, but that's OK
                        (last level fills L→R, can be partial)
```
...
|-----------|------------|-----------|
| **Complete** | All levels full except last, fills L→R | Yes |
| **Perfect** | All levels completely full | Yes (subset) |
| **Full** | Every node has 0 or 2 children | Not necessarily |
| **Balanced** | Height difference ≤ 1 at each node | Not necessarily |
...
All nodes have 0 or 2 children  Last level fills L→R
...
Still complete ✓               Special case of complete
```

### 3. Assuming All Heaps Are Perfect

```python
# ❌ Wrong assumption: heap always has 2^k - 1 elements
heap_size = 2**k - 1

# ✅ Correct: heap can have any number of elements
...
### Hard (Master concept)
- [ ] Prove height of complete tree is ⌊log₂(n)⌋
