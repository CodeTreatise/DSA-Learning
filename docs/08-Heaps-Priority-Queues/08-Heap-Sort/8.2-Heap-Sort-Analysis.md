...
```
         Level    Nodes    Heapify Cost    Total Work
          ...
          ...
          h      n/4          1             n/4 * 1
          h+1    n/2          0             n/2 * 0  ← Half the nodes, 0 work!

Most nodes are at the bottom → least work each
Few nodes at top → most work each
Net effect: O(n)
```
...
|-----------|---------|-------------|
| **Heap Sort** | ❌ No | Swaps can reorder equals |
| Quick Sort | ❌ No | Partition can reorder |
| Merge Sort | ✅ Yes | Merge preserves order |
| Tim Sort | ✅ Yes | Based on Merge Sort |
...
- Python's `heapq` + list → slower than `sorted()`
...
```
❌ Wrong: "Each heapify is O(log n), we do n/2, so O(n log n)"

✅ Correct: Most nodes are near leaves with height ≈ 0
...
```
❌ Wrong: "Just track original indices"
   (Doesn't help - heap operations still reorder)

✅ Correct: Heap Sort is fundamentally unstable
...
```
❌ Wrong: "Heap Sort has guaranteed O(n log n), Quick Sort O(n²)"

✅ Correct: Quick Sort is typically 2-3x faster on average
