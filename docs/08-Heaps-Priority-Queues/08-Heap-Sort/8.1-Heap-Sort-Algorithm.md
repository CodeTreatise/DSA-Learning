...
```python
# ❌ Wrong: Starting from root
for i in range(0, n):  # Inefficient, still works but not O(n)
    heapify_up(arr, i)

# ✅ Correct: Start from last non-leaf, go backwards
...
```python
# ❌ Wrong: Using full array length
for i in range(n - 1, 0, -1):
    arr[0], arr[i] = arr[i], arr[0]
    heapify_down(arr, n, 0)  # Wrong! Uses full n

# ✅ Correct: Reduce heap size each iteration
for i in range(n - 1, 0, -1):
    arr[0], arr[i] = arr[i], arr[0]
...
```python
# ❌ Wrong: Min-heap gives descending order
# Extract min → place at end → smallest at end!

# ✅ Correct: Use max-heap for ascending
# Extract max → place at end → largest at end → ascending ✓
```

### 4. Forgetting the Sort is In-Place

```python
# ❌ Wrong: Creating new array
def heap_sort(arr):
    # ... sorting ...
...
|---------|-----------|-------|
| All | ⭐⭐⭐ | Understanding expected, rarely implement |
| Amazon | ⭐⭐⭐⭐ | May ask implementation |
| System Design | ⭐⭐⭐⭐ | External/distributed sorting |
