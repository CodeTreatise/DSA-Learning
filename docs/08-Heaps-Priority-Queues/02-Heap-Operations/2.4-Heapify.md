...
Unordered array:      →      Valid Min-Heap:
[5, 3, 8, 1, 2]              [1, 2, 8, 5, 3]
...
- Have all elements upfront → heapify
- Elements arrive one by one → insert
...
Total work:
  Σ (nodes at level i) * (bubble-down distance)
  = Σ 2^i * (h - i)  for i = 0 to h-1
  
Let k = h - i (height from bottom):
  = Σ 2^(h-k) * k  for k = 1 to h
  = 2^h * Σ (k / 2^k)  for k = 1 to h
  = n * Σ (k / 2^k)
  
Σ (k / 2^k) converges to 2 (infinite series)
...
```python
# ❌ Wrong: Starting from index 0 (includes leaves unnecessarily)
...
# ✅ Safe: Work on copy if original needed
arr = [5, 3, 8, 1, 2]
heap = arr.copy()
heapq.heapify(heap)
...
```python
# ❌ Wrong: Using len(arr) inside bubble_down when n differs
def bubble_down(arr, i):
    n = len(arr)  # Problem during heap sort!
    
# ✅ Correct: Pass n as parameter
...
|---------|-----------|-------|
| All | ⭐⭐⭐⭐ | Fundamental operation |
| Google | ⭐⭐⭐⭐ |" May ask for O(n) proof "|
| Amazon | ⭐⭐⭐⭐ | Applied in Top K |
| Meta | ⭐⭐⭐ | Used in priority queue |
