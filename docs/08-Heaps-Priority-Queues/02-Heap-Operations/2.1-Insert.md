...
**Key insight:**
```
Insert adds at the END → O(1) for placement
Then fixes heap property → O(log n) worst case
Total: O(log n)
```
...
Insert at index 6:
  Path: 6 → 2 → 0 (at most 3 steps for 7 elements)
```
...
print(heap)  # [0, 3, 1, 7, 4, 2]  ← 0 bubbled to root
```
...
print(heap)  # [15, 7, 10, 3, 5, 8]  ← 15 bubbled to root
```
...
Example: n = 1,000,000
Height = ⌊log₂(1,000,000)⌋ = 19 comparisons max
```
...
```python
# ❌ Wrong: Using min-heap comparison for max-heap
while i > 0:
    parent = (i - 1) // 2
    if heap[parent] > heap[i]:  # This is for MIN-heap!
        swap...

# ✅ Correct: For MAX-heap, parent should be larger
while i > 0:
    parent = (i - 1) // 2
...
```python
# ❌ Wrong: Not moving to parent position
while i > 0:
    parent = (i - 1) // 2
    if heap[parent] > heap[i]:
        heap[parent], heap[i] = heap[i], heap[parent]
...
# ✅ Correct: Update i to continue bubbling
while i > 0:
    parent = (i - 1) // 2
    if heap[parent] > heap[i]:
        heap[parent], heap[i] = heap[i], heap[parent]
...
```python
# ❌ Wrong: Always going to root (wastes time)
while i > 0:
    parent = (i - 1) // 2
    if heap[parent] > heap[i]:
        heap[parent], heap[i] = heap[i], heap[parent]
...
# ✅ Correct: Break when property satisfied
while i > 0:
    parent = (i - 1) // 2
    if heap[parent] > heap[i]:
        heap[parent], heap[i] = heap[i], heap[parent]
