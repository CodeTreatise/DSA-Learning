...
7 4 5               7 4 5
                    ...
                    Step 3: Bubble down
                    5 vs children (3, 2): 2 < 3 < 5 → swap with 2
                        2
                       / \
                      3   5
                     /\
...
Maximum path length = height of tree = ⌊log₂(n)⌋
...
```python
# ❌ Wrong: Using min comparison for max-heap
if heap[left] < heap[largest]:  # Should be >
    largest = left

# ✅ Correct: Max-heap uses > comparison
...
```python
# ❌ Wrong: Assumes heap has more than 1 element
def bad_extract(heap):
    root = heap[0]
...
```python
# ❌ Wrong: Always goes to leaf (wastes time)
while left < n:
    if heap[left] < heap[i]:
...
→ "That's more complex - we'd need to find it O(n), move last to that position, then either bubble up or down depending on the replacement value."
