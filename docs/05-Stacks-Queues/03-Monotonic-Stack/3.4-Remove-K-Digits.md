...
>
> **Difficulty:** Medium | **Frequency:** ⭐⭐⭐⭐ Common Greedy Stack Problem
...
Explanation:
Remove the leading 1, remaining = "0200" → "200"
...
'0' < '1' → pop '1', k=0
Stack: ['0', '2', '0', '0']
Result: "0200" → Strip leading zeros → "200"

Case 2: num = "9999", k = 2
No digit is smaller than previous → never pop during iteration
...
```python
# ❌ WRONG: Only removes during iteration
for digit in num:
    while k > 0 and stack and digit < stack[-1]:
        stack.pop()
        k -= 1
...
```python
# ❌ WRONG: Using <= removes equal digits unnecessarily
while k > 0 and stack and digit <= stack[-1]:

# ✅ CORRECT: Only remove strictly larger
while k > 0 and stack and digit < stack[-1]:
```
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Common greedy problem |
| Google | ⭐⭐⭐⭐ | Tests greedy thinking |
| Microsoft | ⭐⭐⭐ | May ask variations |
| Bloomberg | ⭐⭐⭐ | String manipulation |
