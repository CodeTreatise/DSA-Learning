...
>
> **Priority:** ⭐⭐⭐⭐⭐ HIGH - Covers ~15% of stack interview problems
>
...
i=2: arr[2]=2
     2 > arr[1]=1, pop 1 → result[1]=2
     2 = arr[0]=2, no pop (not strictly greater)
     Push 2
     Stack: [0, 2]

i=3: arr[3]=4
     4 > arr[2]=2, pop 2 → result[2]=4
...
│      ├── Next Greater  → Decreasing stack, iterate L→R     │
│      ├── Next Smaller  → Increasing stack, iterate L→R     │
│      ├── Prev Greater  → Decreasing stack, iterate L→R     │
│      └── Prev Smaller  → Increasing stack, iterate L→R     │
│                                                             │
│  Q2: What should I store in the stack?                      │
│      ├── Need index for answer  → Store indices             │
│      └── Need value only        → Store values              │
│                                                             │
│  Q3: When do I record the answer?                           │
│      ├── Next Greater/Smaller → On POP (current is answer)  │
│      └── Prev Greater/Smaller → On PUSH (top is answer)     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
|-----------|------------|-----------|-----------|
| Next Greater | Decreasing | L→R | Pop |
| Next Smaller | Increasing | L→R | Pop |
| Previous Greater | Decreasing | L→R | Push |
| Previous Smaller | Increasing | L→R | Push |
...
# ✅ CORRECT: Decreasing stack for next greater
...
```python
# ❌ WRONG: Can't access original positions
stack.append(nums[i])
...
```python
# ❌ WRONG: Not handling elements with no next greater
def incomplete(nums):
...
**Common follow-ups:**
- "Can you solve it in O(1) space?" → Usually not for this pattern
- "What if elements can be equal?" → Clarify strict vs non-strict comparison
- "What about circular arrays?" → Use modulo or iterate twice
...
|---------|-----------|------------------|
| Amazon | ⭐⭐⭐⭐⭐ | Daily Temperatures, Stock Span |
| Meta | ⭐⭐⭐⭐ | Trapping Rain Water, Histogram |
| Google | ⭐⭐⭐⭐ | 132 Pattern, complex variations |
| Microsoft | ⭐⭐⭐⭐ | Next Greater Element, Remove K Digits |
| Bloomberg | ⭐⭐⭐⭐⭐ | Stock-related problems |
...
> **💡 Key Insight:** The monotonic stack works because we're exploiting the property that once we find a greater/smaller element, all the elements we pop will never need to be considered again for that direction. Each element is processed exactly once.
