...
Operations:
appendleft(0):  [0, 1, 2, 3, 4, 5]  ← Added to front
append(6):      [0, 1, 2, 3, 4, 5, 6]  ← Added to back
popleft():      [1, 2, 3, 4, 5, 6]  ← Removed 0 from front
pop():          [1, 2, 3, 4, 5]  ← Removed 6 from back
```
...
// ✅ CORRECT: Use proper deque implementation
...
|---------|-----------|------------|
| Google | ⭐⭐⭐⭐⭐ | Sliding Window Max |
| Amazon | ⭐⭐⭐⭐ | Monotonic problems |
...
> **💡 Key Insight:** The deque's power is flexibility - it's a stack, a queue, or both, all with O(1) operations. For sliding window problems, this flexibility allows us to maintain candidates at one end while expiring them from the other, achieving what would otherwise require O(n) per window.
