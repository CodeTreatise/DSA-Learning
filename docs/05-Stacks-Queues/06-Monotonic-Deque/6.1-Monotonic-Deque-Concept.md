...
>
> **Interview Frequency:** ⭐⭐⭐⭐ High | **Difficulty:** Medium-Hard
...
| Variable window size | Different expiration logic | Shortest Subarray with Sum ≥ K |
...
```python
# ❌ WRONG: Storing values, can't check expiration
dq.append(val)

# ✅ CORRECT: Store indices, lookup values
dq.append(i)
...
```python
# ❌ WRONG: Removing from wrong end
dq.popleft()  # For monotonic removal

# ✅ CORRECT: Remove from back for monotonic, front for expiration
while dq and nums[dq[-1]] < val:
...
```python
# ❌ WRONG: Incorrect window boundary
if dq[0] <= i - k:  # Wrong boundary
    dq.popleft()

# ✅ CORRECT: Element at index i-k+1 is the first valid
...
|---------|-----------|-------|
| Google | ⭐⭐⭐⭐⭐ | LC 239 very common |
| Amazon | ⭐⭐⭐⭐ | Sliding window problems |
...
> **💡 Key Insight:** The monotonic deque is essentially a "priority queue with expiration." It maintains the potential maximum/minimum candidates in sorted order, and automatically discards elements that either fall outside the window (from front) or will never be the answer (from back). Each element is added once and removed once, giving O(n) total time.
