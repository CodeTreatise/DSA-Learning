...
| O(1) lookup | ✅ | ❌ O(n) | ✅ |
| O(1) insert | ✅ | ✅ | ✅ |
| O(1) delete | ✅ | ❌ O(n) to find | ✅ |
| O(1) move to front | ❌ No order | ✅ | ✅ |
| O(1) find LRU | ❌ No order | ✅ (tail) | ✅ |
...
│ ┌──────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐      │
│ │ HEAD │←→│ Node(3,C)│←→│ Node(1,A)│←→│ Node(2,B)│←→│TAIL││
│ └──────┘   └──────────┘   └──────────┘   └──────────┘      │
...
2. Move Node(1,A) to front → O(1)

On put(4, D) when at capacity:
1. Remove Node(2,B) from tail → O(1)
2. Remove key=2 from Hash Map → O(1)
3. Add Node(4,D) to front → O(1)
4. Add key=4 to Hash Map → O(1)
```
...
```python
# ❌ HARDER: Without dummy nodes, many edge cases
...
# ✅ EASIER: Dummy nodes eliminate edge cases
def _add_to_front(self, node):
    node.prev = self.head
    node.next = self.head.next
    self.head.next.prev = node
...
|---------|------------|-----------|
| Design HashMap | 🟢 Easy | Hash basics |
| LRU Cache | 🟡 Medium | This problem |
| LFU Cache | 🔴 Hard | Frequency tracking |
| Insert Delete GetRandom | 🟡 Medium | Similar pattern |
...
> **💡 Key Insight:** Neither hash map nor doubly linked list alone provides O(1) for all operations. Combined: hash map gives O(1) node lookup, DLL gives O(1) reordering. Store the key in each node so you know what to delete from the hash map when evicting.

> **🔗 Related:** [Design Overview](./8.1-Design-Overview.md) | [RandomizedSet](./8.5-RandomizedSet-LC380.md) | [Hash Map Operations](../02-Hash-Map-Set/2.1-Hash-Map-Operations.md)
