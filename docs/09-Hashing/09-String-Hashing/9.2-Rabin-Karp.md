...
```
Single pattern: O(n + m) average — competitive with KMP
Multiple patterns: O(n * k) — better than running KMP k times
...
Remove leftmost: subtract char * b^(m-1)
...
#              = 65*256³ + 65*256² + 66*256 + 65
#              = ... (some value)
#
# i=0: window "AABA"
#      hash matches → verify → ✓ Match at 0
# i=1: window "ABAA"
#      roll hash → no match
# i=2: window "BAAC"
#      roll hash → no match
# ...
# i=9: window "AABA"
#      roll hash → hash matches → verify → ✓ Match at 9
# i=10: window "ABAA"
#       roll hash → no match
# ...
#
...
def rabin_karp_multiple(text: str, patterns: list[str]) -> dict[str, list[int]]:
    ...
    - KMP: O(n * k) where k is number of patterns
    - Rabin-Karp: O(n * avg_len + total_pattern_length)
...
| Worst | O(n * m) | Many hash collisions (pathological) |
...
Rolling updates: O(1) * n positions = O(n)
Verification (rare): O(m) * expected collisions ≈ O(m * n/mod) ≈ O(1)
...
|-----------|---------|-------|-------|-------------------|
| Naive | O(n * m) | O(n * m) | O(1) | O(n * m * k) |
| **Rabin-Karp** | O(n + m) | O(n * m) | O(1) | O(n + Σm) ✓ |
| KMP | O(n + m) | O(n + m) | O(m) | O((n + m) * k) |
| Boyer-Moore | O(n/m) best | O(n * m) | O(m + σ) | O(n * k) |
| Aho-Corasick | - | O(n + Σm + occ) | O(Σm) | ✓ (built for it) |
...
```python
# ❌ WRONG: Hash collision = false positive
if window_hash == pattern_hash:
    result.append(i)
...
```javascript
// ❌ RISKY: Can overflow for long patterns
...
# ✅ CORRECT: Include last valid window
...
# ✅ CORRECT: Only roll if not at last position
for i in range(n - m + 1):
    # Check match
    ...
...
|---------|------------|-------|
| Implement strStr() | 🟢 Easy | Basic Rabin-Karp |
| Repeated DNA Sequences | 🟡 Medium | Fixed length hash |
| Find All Anagrams | 🟡 Medium | Character frequency hash |
| Longest Duplicate Substring | 🔴 Hard | Binary search + Rabin-Karp |
...
> "Unlike KMP which guarantees O(n+m), Rabin-Karp can degrade to O(n*m) in worst case. However, Rabin-Karp is simpler and excels at multiple pattern search."
...
|---------|-----------|-------|
| Google | ⭐⭐⭐ | String matching questions |
| Amazon | ⭐⭐⭐ | Practical applications |
| Microsoft | ⭐⭐⭐ | Algorithm knowledge |
