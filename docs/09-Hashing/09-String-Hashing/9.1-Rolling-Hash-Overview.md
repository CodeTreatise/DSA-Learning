...
```
Naive: Check each position → O(n * m) comparisons
       Each comparison is O(m) for m-length pattern

Rolling: Compute hash incrementally → O(n + m)
...
hash_new = (hash_old - s[i] * base^(m-1)) * base + s[i+m]
```
...
Rolling update:
1. Remove 'a': 731 - 1*676 = 55
2. Shift left (*26): 55 * 26 = 1430
3. Add 'd': 1430 + 4 = 1434

hash = 1434 = b*26² + c*26¹ + d*26⁰
            = 2*676 + 3*26 + 4*1 ✓
```
...
# pattern_hash = hash("abc") = 1*31² + 2*31¹ + 3*31⁰ = 961 + 62 + 3 = 1026
#
# i=0: window "abc", hash = 1026 == 1026 ✓, verify → match at 0
# i=1: window "bca", roll → different hash, skip
# i=2: window "cab", roll → different hash, skip
# i=3: window "abc", roll → 1026 == 1026 ✓, verify → match at 3
#
# Result: [0, 3]
```
...
class DoubleRollingHash:
    ...
    Probability of collision: 1/MOD1 * 1/MOD2 ≈ 10^-18
    """
...
**Worst case:** O(n * m) if many hash collisions (rare with good primes)
...
```python
# ❌ WRONG: Negative result possible
...
```python
# ❌ WRONG: May overflow for long patterns
power = base ** (length - 1)  # Huge number!

# ✅ CORRECT: Use modular exponentiation
...
```python
# ❌ WRONG: Hash collision can cause false positive
if text_hash == pattern_hash:
...
```python
# ❌ WRONG: Power for removal
power = pow(base, length, mod)  # Should be length-1!

# ✅ CORRECT: Power is base^(length-1)
power = pow(base, length - 1, mod)

# Because: hash = char[0] * base^(m-1) + ... + char[m-1] * base^0
...
|---------|------------|-------------|
| Implement strStr() | 🟢 Easy | Basic pattern matching |
| Repeated DNA Sequences | 🟡 Medium | 10-char rolling hash |
| Longest Duplicate Substring | 🔴 Hard | Binary search + rolling hash |
| Distinct Echo Substrings | 🔴 Hard | Double hash |
...
|---------|-----------|-------|
| Google | ⭐⭐⭐⭐ | Rabin-Karp variants |
| Amazon | ⭐⭐⭐ | Pattern matching |
| Meta | ⭐⭐⭐ | String problems |
...
> **💡 Key Insight:** Rolling hash trades mathematical elegance for simplicity. By representing strings as numbers (polynomial hash), we can compare substrings in O(1) by comparing their hash values. The "rolling" update removes the old character and adds the new in O(1), enabling O(n) string matching.

> **🔗 Related:** [Rabin-Karp Algorithm](./9.2-Rabin-Karp.md) | [Hash Functions](../01-Hash-Fundamentals/1.2-Hash-Functions.md) | [Sliding Window](../../01-Arrays-Strings/01-Arrays/1.4-Common-Techniques/02-Sliding-Window.md)
