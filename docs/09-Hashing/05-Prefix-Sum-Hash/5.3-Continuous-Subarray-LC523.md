...
> **Find a subarray of length ≥2 whose sum is a multiple of k using prefix sums with modular arithmetic.**
...
**Problem signals:**
- "Sum is multiple of k" → modular arithmetic
- "Subarray of size at least two" → need length constraint
- "Continuous subarray" → prefix sum applicable
...
- Length constraint (≥2 in this case)
...
Output: true
Explanation: [2,4] sums to 6, which is 1*6

Example 2:
Input: nums = [23,2,6,4,7], k = 6
Output: true
Explanation: [23,2,6,4,7] sums to 42, which is 7*6

Example 3:
Input: nums = [23,2,6,4,7], k = 13
Output: false
No subarray of length ≥2 has sum that's a multiple of 13
```
...
class Solution:
    def checkSubarraySum(self, nums: List[int], k: int) -> bool:
        """
        Check if subarray of length ≥2 has sum that's multiple of k.
...
def checkSubarraySum(nums: List[int], k: int) -> bool:
    ...
    # prefix % k = 0, and we need index -1 to get length ≥ 2
    remainder_index = {0: -1}
...
```python
# ❌ WRONG: Not checking length
if remainder in remainder_index:
    return True  # Might be length 1!

# ✅ CORRECT: Check length ≥ 2
if remainder in remainder_index:
    if i - remainder_index[remainder] >= 2:
        return True
```
...
```python
# ❌ WRONG: Updating index when found
if remainder in remainder_index:
    if i - remainder_index[remainder] >= 2:
        return True
...
```python
# ❌ WRONG: Empty map or wrong index
remainder_index = {}  # Missing 0
remainder_index = {0: 0}  # Wrong index

# ✅ CORRECT
remainder_index = {0: -1}  # -1 so that length = i - (-1) = i+1 ≥ 2
```
...
# Modern LeetCode guarantees k ≥ 1
...
|---------|------------|-------------|------|
| Continuous Subarray Sum | 🟡 Medium | Remainder + length | [LC 523](https://leetcode.com/problems/continuous-subarray-sum/) |
| Subarray Sum Equals K | 🟡 Medium | No modulo | [LC 560](https://leetcode.com/problems/subarray-sum-equals-k/) |
| Subarray Sums Divisible by K | 🟡 Medium | Count all | [LC 974](https://leetcode.com/problems/subarray-sums-divisible-by-k/) |
| Make Sum Divisible by P | 🟡 Medium | Remove minimum | [LC 1590](https://leetcode.com/problems/make-sum-divisible-by-p/) |
| Check If Array Pairs Divisible | 🟡 Medium | Pair remainders | [LC 1497](https://leetcode.com/problems/check-if-array-pairs-are-divisible-by-k/) |
...
> "I store the first index where I see each remainder. This maximizes the gap, ensuring I can find length ≥ 2 subarrays."
...
> **💡 Key Insight:** Two prefix sums with the same remainder mod k means their difference is divisible by k. Store the first occurrence of each remainder to maximize potential subarray length.

> **🔗 Related:** [Prefix Hash Overview](./5.1-Prefix-Hash-Overview.md) | [Subarray Sum K](./5.2-Subarray-Sum-K-LC560.md) | [LC 974 - Count Divisible](https://leetcode.com/problems/subarray-sums-divisible-by-k/)
