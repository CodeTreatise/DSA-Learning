...
- K is smaller than n (if k ≈ n, just sort)
...
| k ≈ n | Overhead not worth it | Simple sorting |
...
bucket[5]: empty
bucket[4]: empty
bucket[3]: [1] → add to result
bucket[2]: [2] → add to result
...
- Counting: O(n)
- Bucket creation: O(unique elements) ≤ O(n)
- Collection: O(n) in worst case
- Total: O(n)

**Heap Analysis:**
- Counting: O(n)
- Heap operations: O(unique * log k)
- Total: O(n log k) when unique ≈ n
...
```python
# ❌ WRONG: Max-heap removes largest, keeping smallest
...
# ❌ WRONG: Just sort by frequency
# ✅ CORRECT: Sort by (-freq, word) for tie-breaking
```

### 3. Off-by-One in Bucket Sort

```python
# ❌ WRONG: Buckets of size n
buckets = [[] for _ in range(n)]  # Indices 0 to n-1

# ✅ CORRECT: Buckets of size n+1 (frequency can be n)
...
```python
# ❌ WRONG: Return (count, num) pairs
return heapq.nlargest(k, freq.items(), key=lambda x: x[1])

# ✅ CORRECT: Return just the numbers
...
|---------|------------|------|-------|
| Top K Frequent Elements | 🟡 Medium | [LC 347](https://leetcode.com/problems/top-k-frequent-elements/) | This problem |
| Top K Frequent Words | 🟡 Medium | [LC 692](https://leetcode.com/problems/top-k-frequent-words/) | Tie-breaking |
| Kth Largest Element | 🟡 Medium | [LC 215](https://leetcode.com/problems/kth-largest-element-in-an-array/) | QuickSelect |
| Sort Characters by Frequency | 🟡 Medium | [LC 451](https://leetcode.com/problems/sort-characters-by-frequency/) | Build result string |
| K Closest Points | 🟡 Medium | [LC 973](https://leetcode.com/problems/k-closest-points-to-origin/) | Distance ranking |
...
**Clarify:**
> "Are all inputs valid? Is k always ≤ number of unique elements? Can I return elements in any order?"
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Very common |
| Meta | ⭐⭐⭐⭐ | Often asks follow-up |
| Google | ⭐⭐⭐⭐ | Expects O(n) solution |
| Microsoft | ⭐⭐⭐⭐ | Common Top K problem |
...
> **💡 Key Insight:** Bucket sort achieves O(n) because frequencies are bounded by n. Using frequency as array index lets us "sort" without comparison, avoiding O(n log n).

> **🔗 Related:** [Frequency Overview](./3.1-Frequency-Overview.md) | [Heap Patterns](../../08-Heaps-Priority-Queues/04-Top-K-Pattern/4.1-Top-K-Overview.md) | [Kth Largest](https://leetcode.com/problems/kth-largest-element-in-an-array/)
