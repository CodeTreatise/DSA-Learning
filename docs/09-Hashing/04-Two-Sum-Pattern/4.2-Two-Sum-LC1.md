...
```python
# ❌ WRONG: j starts from 0, may use same element
for i in range(n):
    for j in range(n):  # Should be range(i + 1, n)
        if nums[i] + nums[j] == target:
            return [i, j]

# ✅ CORRECT: j starts from i + 1
for i in range(n):
    for j in range(i + 1, n):
        if nums[i] + nums[j] == target:
            return [i, j]
...
```python
# ❌ WRONG in JavaScript (checks if value is iterable, not membership)
...
|---------|------------|------|-------|
| Two Sum | 🟢 Easy | [LC 1](https://leetcode.com/problems/two-sum/) | This problem |
| Two Sum II | 🟡 Medium | [LC 167](https://leetcode.com/problems/two-sum-ii-input-array-is-sorted/) | Two Pointers |
| 3Sum | 🟡 Medium | [LC 15](https://leetcode.com/problems/3sum/) | Build on Two Sum |
| 4Sum | 🟡 Medium | [LC 18](https://leetcode.com/problems/4sum/) | Recursive pattern |
| Two Sum IV - BST | 🟢 Easy | [LC 653](https://leetcode.com/problems/two-sum-iv-input-is-a-bst/) | Tree variant |
...
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐⭐ | Phone screen favorite |
| Google | ⭐⭐⭐⭐⭐ | Expect O(n) solution |
| Meta | ⭐⭐⭐⭐⭐ | Must solve in < 10 min |
| Microsoft | ⭐⭐⭐⭐ | Common warm-up |
| All Companies | ⭐⭐⭐⭐⭐ | **Most asked LC problem** |
...
> **💡 Key Insight:** The magic of Two Sum is recognizing that "find a + b = target" becomes "for each a, check if (target - a) exists" - transforming a search problem into a lookup problem that hash maps solve in O(1).

> **🔗 Related:** [Two Sum Overview](./4.1-Two-Sum-Overview.md) | [Contains Duplicate II](./4.3-Contains-Duplicate-II-LC219.md) | [3Sum](https://leetcode.com/problems/3sum/)
