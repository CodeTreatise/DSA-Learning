...
**Why it matters:**
- α = 0.5 → 50% full → few collisions
- α = 0.7 → 70% full → acceptable
- α = 1.0 → 100% full → many collisions
- α > 1.0 → only possible with chaining

**Industry standards:**
- Java HashMap: resize at α = 0.75
- Python dict: resize at α = 0.67
...
|-------------|------|------|----------|
| α < 0.5 | Very fast | Memory wasteful | Speed critical |
| α = 0.5-0.7 | Balanced | Good trade-off | General use |
| α = 0.75 | Standard | Default choice | Most cases |
| α > 0.9 | Memory efficient | Slower | Memory constrained |

## ❌ When Load Factor Matters Most

- **Open addressing:** Must resize before α = 1.0 (table full)
- **Chaining:** Can exceed 1.0, but performance degrades
- **Real-time systems:** Keep α low for predictable performance
- **Memory-constrained:** Accept higher α for space savings
...
α = 0.5: ~1.5 probes on average
α = 0.7: ~2.2 probes
α = 0.9: ~5.5 probes
α = 0.99: ~50 probes!

Formula: E[probes] ≈ 1/(1-α) for unsuccessful search
```
...
α = 0.25  ▓░░░░░░░░░░░░░░░░░░░  Few collisions
α = 0.50  ▓▓▓▓▓░░░░░░░░░░░░░░░  Some collisions
α = 0.75  ▓▓▓▓▓▓▓▓▓▓▓░░░░░░░░░  More collisions
α = 0.90  ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓░░░  Many collisions
α = 0.99  ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓  Severe clustering
```
...
Initial: m = 16, n = 0, α = 0

After insertions:
┌──────┬──────┬────────┬─────────────────┐
│ n    │ m    │ α      │ Action          │
├──────┼──────┼────────┼─────────────────┤
│ 1    │ 16   │ 0.06   │ Normal insert   │
│ 8    │ 16   │ 0.50   │ Normal insert   │
│ 12   │ 16   │ 0.75   │ Threshold!      │
│ 12   │ 32   │ 0.375  │ After resize    │
│ 24   │ 32   │ 0.75   │ Threshold again │
│ 24   │ 64   │ 0.375  │ After resize    │
└──────┴──────┴────────┴─────────────────┘
```
...
After Resize (m=8, n=3, α=0.375):
┌───────────────────────────────┐
│ [0]: "dog"                    │  ← Rehashed!
│ [1]: empty                    │
│ [2]: empty                    │
│ [3]: "ant"                    │  ← Same or different
│ [4]: empty                    │
│ [5]: "cat"                    │  ← No longer colliding!
│ [6]: empty                    │
│ [7]: "bat"                    │  ← Split from chain
└───────────────────────────────┘
...
for i in range(10):
    ...
    print(f"n={ht.size}, m={ht.capacity}, α={load:.2f}")
```
...
| Lookup | O(1) | O(n) | Rare with good α |
...
|-------------|------------------|-------|
| α = 0.25 | 25% utilized | Wasteful |
| α = 0.50 | 50% utilized | Acceptable |
| α = 0.75 | 75% utilized | Good trade-off |

**Memory usage:**
- Chaining: O(n) for elements + O(m) for buckets + pointer overhead
- Probing: O(m) for array (m ≥ n/α)
...
|----------|---------------|------|------|
| Double size | α > threshold | Simple, effective | Memory spikes |
...
```python
# ❌ WRONG: Resize only when completely full
if self.size == self.capacity:
    self._resize()  # Too late! Performance already degraded

# ✅ CORRECT: Resize before too full
...
```python
# ❌ WRONG: Hash with old capacity
def put(self, key, value):
    self._resize_if_needed()
    index = hash(key) % self.old_capacity  # Bug!

# ✅ CORRECT: Always use current capacity
def put(self, key, value):
    self._resize_if_needed()
...
|---------|------------|-------|------|
| Design HashMap | 🟢 Easy | Basic resizing | [LC 706](https://leetcode.com/problems/design-hashmap/) |
| Design HashSet | 🟢 Easy | Set with resize | [LC 705](https://leetcode.com/problems/design-hashset/) |
| LRU Cache | 🟡 Medium | Hash + resizing | [LC 146](https://leetcode.com/problems/lru-cache/) |
| Randomized Set | 🟡 Medium | O(1) operations | [LC 380](https://leetcode.com/problems/insert-delete-getrandom-o1/) |
...
> **💡 Key Insight:** Load factor balances space and time. The magic of amortized O(1) comes from doubling capacity - we pay O(n) for resize but get n free insertions, averaging to O(1) per insert.

> **🔗 Related:** [Collision Handling](./1.3-Collision-Handling.md) | [Design HashMap](../08-Design-Problems/8.2-Design-HashMap-LC706.md) | [Amortized Analysis](../.../00-Prerequisites/00-Prerequisites.md#complexity)
