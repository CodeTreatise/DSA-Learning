...
- [Big O notation](../.../00-Prerequisites/00-Prerequisites.md#complexity)
...
```
Data → Hash Function → Hash Code → Array Index → Stored Value

"apple" → hash("apple") → 97342158 → 97342158 % 100 → 58 → bucket[58]
```
...
"dog" → (100+111+103) % 10 = 314 % 10 = 4
"ant" → (97+110+116) % 10 = 323 % 10 = 3
"bat" → (98+97+116) % 10 = 311 % 10 = 1
...
│                                                              │
│   ┌──────────┐     ┌─────────────┐     ┌─────────────────┐  │
│   │   Key    │ →→→ │ Hash        │ →→→ │  Bucket Array   │  │
│   │ "hello"  │     │ Function    │     │  [0][1][2]...   │  │
│   └──────────┘     └─────────────┘     └─────────────────┘  │
│                                                              │
│   Hash Function:                                             │
│   - Takes any key (string, int, object)                     │
│   - Returns an integer (hash code)                          │
│   - hash_code % array_size = bucket index                   │
│                                                              │
│   Bucket Array:                                              │
...
```python
# ❌ MISCONCEPTION: Always O(1)
# ✅ REALITY: O(1) average, O(n) worst case
...
```python
# ❌ MISCONCEPTION: Hash tables maintain order
# ✅ REALITY: Only Python 3.7+ dict maintains insertion order
...
```python
# ❌ MISCONCEPTION: Any object works as key
# ✅ REALITY: Keys must be hashable (immutable)
...
# Invalid keys
# d[[1, 2]] = 4       # ✗ list (mutable)
# d[{1: 2}] = 5       # ✗ dict (mutable)
```
...
|---------|------------|-------|------|
| Two Sum | 🟢 Easy | Basic lookup | [LC 1](https://leetcode.com/problems/two-sum/) |
| Contains Duplicate | 🟢 Easy | Set usage | [LC 217](https://leetcode.com/problems/contains-duplicate/) |
| Valid Anagram | 🟢 Easy | Frequency counting | [LC 242](https://leetcode.com/problems/valid-anagram/) |
| Design HashMap | 🟢 Easy | Implementation | [LC 706](https://leetcode.com/problems/design-hashmap/) |
| Group Anagrams | 🟡 Medium | Hash as key | [LC 49](https://leetcode.com/problems/group-anagrams/) |
...
**Red flags to avoid:**
- ❌ Not considering space trade-off
- ❌ Forgetting hash tables are unordered
- ❌ Using hash when array indices work
- ✅ Explaining why O(1) average, not guaranteed
...
|---------|------------------|-------|
| All | ⭐⭐⭐⭐⭐ | Fundamental skill |
| Meta | ⭐⭐⭐⭐⭐ | Very common |
| Google | ⭐⭐⭐⭐⭐ | Expected knowledge |
| Amazon | ⭐⭐⭐⭐⭐ | System design too |
...
> **💡 Key Insight:** Hashing trades space for time, converting O(n) lookups to O(1) by using a smart indexing scheme. It's one of the most important techniques to master for coding interviews.

> **🔗 Related:** [Hash Functions](./1.2-Hash-Functions.md) | [Collision Handling](./1.3-Collision-Handling.md) | [Hash Map Operations](../02-Hash-Map-Set/2.1-Hash-Map-Operations.md)
