...
|--------------|----------------|---------|
| Single Missing | Range [0,n], n nums | [0,1,3] → 2 |
| Single Duplicate | Range [1,n], n+1 nums | [1,3,4,2,2] → 2 |
| All Missing | Range [1,n], n nums | [4,3,2,7,8,2,3,1] → [5,6] |
| All Duplicates | Range [1,n], n nums | Same array → [2,3] |
| Missing + Duplicate | One each | [1,2,2,4] → miss:3, dup:2 |
| First Missing Positive | Any integers | [3,4,-1,1] → 2 |
...
|----------|------|-------|-----------------|
| **Math (Sum/Product)** | O(n) | O(1) | ❌ No |
| **XOR** | O(n) | O(1) | ❌ No |
| **Cyclic Sort/Marking** | O(n) | O(1) | ✅ Yes |

Choose based on constraints:
- Array immutable → Math or XOR
- Array can be modified → Cyclic Sort (most versatile)
- Multiple missing/duplicates → Cyclic Sort or Marking
...
# How it works:
# [4, 3, 2, 7, 8, 2, 3, 1]
# See 4 → mark index 3 negative: [4, 3, 2, -7, 8, 2, 3, 1]
# See 3 → mark index 2 negative: [4, 3, -2, -7, 8, 2, 3, 1]
# See 2 → mark index 1 negative: [4, -3, -2, -7, 8, 2, 3, 1]
# See 7 → mark index 6 negative: [4, -3, -2, -7, 8, 2, -3, 1]
# See 8 → mark index 7 negative: [4, -3, -2, -7, 8, 2, -3, -1]
# See 2 → index 1 already negative → 2 is duplicate!
# See 3 → index 2 already negative → 3 is duplicate!
# See 1 → mark index 0 negative: [-4, -3, -2, -7, 8, 2, -3, -1]
```
...
def find_disappeared_marking(nums: list[int]) -> list[int]:
    ...
    # Indices still positive → those numbers missing
...
### Problem 6: First Missing Positive (LC 41) ⭐ HARD
...
1. "Can I modify the array?" → Cyclic sort/marking
2. "Is there overflow concern?" → XOR over sum
3. "Need to find ONE value?" → Consider Floyd's
4. "Need to find ALL values?" → Marking technique
...
- Bit Manipulation - XOR techniques
