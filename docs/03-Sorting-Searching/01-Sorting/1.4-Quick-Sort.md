...
> **Interview Value:** ⭐⭐⭐⭐⭐ - Most asked sorting algorithm  
...
✅ **Better (Tail Recursion Optimization):**
```python
def sort(low, high):
    while low < high:
        p = partition(low, high)
...
**Common follow-ups:**
- "What's the worst case?" → Already sorted with bad pivot
- "How to avoid worst case?" → Random pivot or median-of-three
- "Is it stable?" → No, swapping breaks relative order
