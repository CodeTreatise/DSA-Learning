...
> **Interview Value:** ⭐⭐⭐ - Know for specific cases  
...
├── Yes, range k < n → Counting Sort
├── Yes, large range but fixed digits → Radix Sort
├── Yes, uniformly distributed → Bucket Sort
└── No → Comparison sort (Quick/Merge)
```
...
| Radix |" O(d(n + k)) "|" O(n + k) "| ✅ | Fixed-digit integers |
...
1. **Frequency counting problems** → Counting Sort concept
2. **Sorting integers with limited range** → Counting Sort
3. **Finding maximum gap** → Bucket Sort pigeonhole
