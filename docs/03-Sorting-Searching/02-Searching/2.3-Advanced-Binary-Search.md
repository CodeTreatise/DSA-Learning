...
> **Interview Value:** ⭐⭐⭐⭐⭐ - Google favorite  
...
Check: max(left) ≤ min(right)?
       max(3, 18) = 18 ≤ min(8, 19) = 8?  NO! 18 > 8
...
Check valid:
- nums1[1] = 3 ≤ nums2[4] = 21? YES
- nums2[3] = 19 ≤ nums1[2] = 8? NO! 19 > 8
...
```
Goal: Split both arrays so left half ≤ right half
...
Check: max(3, 11) ≤ min(8, 18)
       11 ≤ 8?  NO → Move partition1 right

nums1: [1, 3, 8] | [9, 15]
nums2:       [7] | [11, 18, 19, 21, 25]

Check: max(8, 7) ≤ min(9, 11)
       8 ≤ 9?  YES! Valid partition.
...
"Binary search on partition position. Valid when max(left) ≤ min(right)."
...
> "I need to partition both arrays such that all elements in the left partition are ≤ all elements in the right partition. The number of elements in the left partition is (m+n+1)/2."
...
Valid when: max(left1, left2) ≤ min(right1, right2)
```
