...
> **Interview Value:** ⭐⭐⭐⭐⭐ - ~15% of interview problems use Binary Search  
...
```
Linear Search: [1, 2, 3, 4, 5, 6, 7, 8, 9] → Check all 9 elements → O(n)
Binary Search: [1, 2, 3, 4, 5, 6, 7, 8, 9] → Check 4 elements max → O(log n)
```
...
log₂(1000) ≈ 10 comparisons maximum!
```
...
| **Lower bound** | First element ≥ target | [Boundary Search](./2.2-Binary-Search-Patterns/02-Boundary-Search.md) |
...
- "What if there are duplicates?" → Use boundary search
- "What if the array is rotated?" → Modified binary search
- "Can you do it recursively?" → Yes, but O(log n) space
