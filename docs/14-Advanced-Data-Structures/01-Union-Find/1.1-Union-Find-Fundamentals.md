...
| **Grouping by equivalence** | Transitive relationships (a~b, b~c → a~c) |
...
- [Kruskal's MST Algorithm](../../10-Graphs/10-Graphs.md#6-minimum-spanning-tree-mst) - Uses Union-Find
...
Interpretation:
- parent[0] = 0 → 0 is root of its set
- parent[1] = 0 → 1's parent is 0
- parent[2] = 2 → 2 is root of its set
- parent[3] = 2 → 3's parent is 2
- parent[4] = 4 → 4 is root of its set
...
| Operation | Time | Space | Notes |
|-----------|------|-------|-------|
| find(x) | O(α(n)) ≈ O(1) | O(1) | With path compression |
| union(x, y) | O(α(n)) ≈ O(1) | O(1) | With union by rank |

Where α(n) is the **inverse Ackermann function** - grows so slowly that α(n) < 5 for any practical n.
...
```python
# ❌ Wrong: Connecting elements directly
def union(self, x, y):
    self.parent[x] = y  # Wrong! Doesn't merge sets properly

# ✅ Correct: Connect roots
def union(self, x, y):
    root_x, root_y = self.find(x), self.find(y)
    if root_x != root_y:
        self.parent[root_x] = root_y
...
> "To ensure efficiency, I'll add path compression during find, which flattens the tree, and union by rank, which keeps trees balanced. Together these give O(α(n)) which is effectively O(1)."

**Company Frequency:**
| Company | Frequency | Focus |
|---------|-----------|-------|
| Amazon | ⭐⭐⭐⭐ | Connected components |
| Meta | ⭐⭐⭐⭐ | Accounts merge pattern |
| Google | ⭐⭐⭐ | May combine with other DS |
...
> **💡 Key Insight:** Union-Find brilliantly reduces "are X and Y connected?" from O(n) traversal to nearly O(1) by maintaining set representatives. The magic is in the optimizations: path compression makes trees flat, and union by rank keeps them balanced.

> **🔗 Related:** [Optimizations](./1.3-Optimizations.md) | [Connected Components](./1.4-Advanced-Patterns/01-Connected-Components.md) | [Graph Traversal](../../10-Graphs/10-Graphs.md)

---

**Next:** [Basic Implementation Details →](./1.2-Basic-Implementation.md)
