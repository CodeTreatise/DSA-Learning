...
prefix_sum(diff, 3) = 1 + 11 + 1 = 13 = original[3] + 10 ✓
```
...
Value at index i:
A[i] = Σ D[j] for j = 1 to i

Sum from 1 to i:
Σ A[k] for k = 1 to i
= Σ (Σ D[j] for j = 1 to k) for k = 1 to i
= Σ D[j] * (i - j + 1) for j = 1 to i
= (i + 1) * Σ D[j] - Σ (j * D[j])
...
## 🎯 Application: Count Smaller After Self
...
# ✅ Correct: Use set to remove duplicates first
...
```python
# ❌ Wrong: Including current element
count = bit.prefix_sum(rank[num])  # Includes elements = to num!

# ✅ Correct: Strictly smaller
...
| Range query | O(log n) |
| 2D operations | O(log n * log m) |

---

> **💡 Key Insight:** BIT's power comes from its flexibility. By storing differences instead of values, we flip the roles of update and query. By using two BITs, we can handle both range updates and range queries.

> **🔗 Related:** [BIT Fundamentals](./4.1-BIT-Fundamentals.md) | [BIT vs Segment Tree →](./4.3-BIT-vs-Segment-Tree.md)

---

**Next:** [BIT vs Segment Tree →](./4.3-BIT-vs-Segment-Tree.md)
